...
*Note: 由于浏览器发出的https请求只能获取去域名，无法获得路径信息，所以浏览器的非http请求只能确保对域名匹配生效，如果要让上述各种匹配方式适用https请求，需要用到下面的https转http的方功能，后面再详细讲*

# 基本功能

下面只以 **域名匹配方式** 说明whistle基本功能，**其它匹配方式(路径、正则)也同样适用**，下面每个操作的配置信息都是写在本地的文件，后面讲UI操作的时候，再讲如何在页面配置。

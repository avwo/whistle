...
var DebugRule = React.createClass({
  ...
  onAbortResChange: function(e) {
    this.onStateChange('abortRes', e.target.checked);
  },
  onWeinreIdChange: function(e) {
    this.onStateChange('weinreId', removeSpaces(e.target.value));
  },
  onLogIdChange: function(e) {
    this.onStateChange('logId', removeSpaces(e.target.value));
  },
  ...
  render: function() {
    ...
    return (
      <div className={'w-rules-form' + (hide ? ' w-hide' : '')}>
        ...
        <FormItem>
          <label>
            {renderBox(state.abortReq, 'abortReq', self.onEnableCheckChange)}
            Abort Request
        ...
        <FormItem>
          <label>
            {renderBox(state.abortRes, 'abortRes', self.onEnableCheckChange)}
            Abort Response
            <HelpIcon docsUrl="rules/enable.html" className="ml-10" />
          </label>
        </FormItem>

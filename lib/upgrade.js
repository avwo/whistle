...
function upgradeHandler(req, socket) {
  ...
  getPluginNameByReq(isWs ? req : null, function(err, uiPort) {
    ...
    var clientIp, clientPort, remoteAddr, remotePort;
    if (isWs) {
      ...
      remoteAddr = clientInfo[2] || util.getRemoteAddr(req);
      remotePort = clientInfo[3] || util.getRemotePort(req);
    ...
    if (!isWs || uiPort) {
      if (!isWs) {
        ...
        headers[util.REMOTE_ADDR_HEAD] = remoteAddr;
        headers[util.REMOTE_PORT_HEAD] = remotePort;

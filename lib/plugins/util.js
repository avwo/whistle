...
function setPlugin(plugins, pkg) {
    ...
    files.forEach(function(file) {
      ...
      try {
        ...
        if (Date.now() - ctn[0] < MAX_EXPIRE && ctn[1]) {
          var root = ctn[1];
          setPlugin(plugins, fse.readJsonSync(path.join(root, 'package.json')));
        } else {
          fs.unlinkSync(file);
        }
      } catch (e) {}
    ...
    files.forEach(function(file) {
      ...
      readFile(file, function(_, ctn) {
        ...
        if (ctn && Date.now() - ctn[0] < MAX_EXPIRE && ctn[1]) {
          var root = ctn[1];
          readFile(path.join(root, 'package.json'), function(err, pkg) {
            if (pkg) {
              try {
                setPlugin(plugins, JSON.parse(pkg));
              } catch (e) {}
            }

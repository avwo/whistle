...
var appendTrailers = function(_res, res, newTrailers) {
  common.addTrailerNames(_res.headers, newTrailers);
...
function formatRawHeaders(headers, req) {
  ...
  var rawNames = Array.isArray(rawHeaders) ? getRawHeaderNames(rawHeaders) : null;
  if (headers && headers.trailer && (!rawNames || !rawNames.trailer)) {
    rawNames = rawNames || {};
      ...
      server.on('request', function(req, res) {
        ...
        case PLUGIN_HOOKS.HTTP:
          if (httpServer) {
            ...
            req.appendTrailers = res.appendTrailers = function(_res, newTrailers) {
              appendTrailers(_res, res, newTrailers);
            };
            req.request = function(uri, cb, opts) {
              ...
              var client = (args[2] ? httpsRequest : httpRequest)(uri, function(_res) {
                ...
                if (typeof cb === 'function') {
                  ...
                  appendTrailers(_res, res, opts && opts.trailers);
                  res.writeHead();
                  _res.pipe(res);
                }
              });

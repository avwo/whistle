      ...
      server.on('connect', function(req, socket, head) {
        ...
        case PLUGIN_HOOKS.TUNNEL:
          if (tunnelServer) {
            initConnectReq(req, socket);
            req.sendEstablished(function() {
              req.connect = function(tunnelPath, cb, opts) {
                var headers = extractHeaders(req, pluginKeyMap);
                var policy = headers['x-whistle-policy'];
                if (TUNNEL_HOST_RE.test(tunnelPath)) {
                  headers.host = tunnelPath;
                } else if (typeof tunnelPath === 'function') {
                  opts = cb;
                  cb = tunnelPath;
                } else if (tunnelPath) {
                  if (TUNNEL_HOST_RE.test(tunnelPath.url)) {
                    if (tunnelPath.headers) {
                      headers = tunnelPath.headers;
                      if (policy && !headers['x-whistle-policy']) {
                        headers['x-whistle-policy'] = policy;
                      }
                    }
                    headers.host = tunnelPath.url;

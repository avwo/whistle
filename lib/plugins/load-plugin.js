...
module.exports = function(options, callback) {
  ...
  var initServers = function(_ctx) {
    ...
    var startTunnelServer = getFunction(execPlugin.pluginServer || execPlugin.tunnelServer || execPlugin.connectServer);
    ...
    getServer(function(server, _port) {
      ...
      if (startReqDecoder) {
        ...
        startTunnelServer(reqDecoderServer, options);
        reqDecoderPort = _port;
      }

      if (startReqEncoder) {
        reqEncoderServer = createServer();
        startTunnelServer(reqEncoderServer, options);
        reqEncoderPort = _port;
      }

      if (startResDecoder) {
        resDecoderServer = createServer();
        startTunnelServer(resDecoderServer, options);
        resDecoderPort = _port;
      }

      if (startResEncoder) {
        resEncoderServer = createServer();
        startTunnelServer(resEncoderServer, options);
      ...
      server.on('upgrade', function(req, socket) {
        ...
        case PLUGIN_HOOKS.UI:
          if (uiServer) {
            setContenxt(req);
            uiServer.emit('upgrade', req, socket);
          }
          break;
        case PLUGIN_HOOKS.WS:
          if (httpServer) {
            initWsReq(req, socket);
            httpServer.emit('upgrade', req, socket);
          }
          break;
        default:
          socket.destroy();
      ...
      server.on('connect', function(req, socket) {
        ...
        case PLUGIN_HOOKS.TUNNEL:
          if (tunnelServer) {
            ...
            tunnelServer.emit('connect', req, socket);
          }
          break;
        case PLUGIN_HOOKS.REQ_ENCODER:
          if (reqEncoderServer) {
            initConnectReq(req, socket);
            reqEncoderServer.emit('connect', req, socket);
          }
          break;
        case PLUGIN_HOOKS.REQ_DECODER:
          if (reqDecoderServer) {
            initConnectReq(req, socket);
            reqDecoderServer.emit('connect', req, socket);
          }
          break;
        case PLUGIN_HOOKS.RES_ENCODER:
          if (resEncoderServer) {
            initConnectReq(req, socket);
            resEncoderServer.emit('connect', req, socket);
          }
          break;
        case PLUGIN_HOOKS.RES_DECODER:
          if (resDecoderServer) {
            initConnectReq(req, socket);
            resDecoderServer.emit('request', req, socket);
          }
          break;
        default:
          socket.destroy();

...
var inspectOpts, inspectTimer, inspectPending;
var reqCallbacks = {};
var resCallbacks = {};
var inpsectCallbacks = {};
var framesCallbacks = [];
var MAX_LENGTH = 100;
var TIMEOUT = 1000;
...
var addInspectApi = function(req, conn, status, reqId) {
  status = status.split(',').forEach(function(name) {
    setStatus(req, name);
  });
  req.addClientFrame = function(data, ignore) {

  };
  req.addServerFrame = function(data, ingore) {
...
var initReq = function(req, res) {
  ...
  var customInpsect = req.headers[pluginOpts.CUSTOM_INSPECT_HEADER];
  setContenxt(req);
  oReq.clientIp = req.clientIp;
  if (customInpsect && typeof customInpsect === 'string') {
    addInspectApi(req, res, customInpsect, reqId);
    req.customInpsect = oReq.customInpsect = true;
  }
...
var retryInspect = function(time) {
  if (!inspectTimer) {
    inspectTimer = setTimeout(inspect, time || TIMEOUT);
  }
};
...
var inspect = function() {
  ...
  var idList = Object.keys(inpsectCallbacks);
  if (!idList.length) {
    return;
  }
  inspectPending = true;
  ...
  requestData(inspectOpts, function(err, result) {
...
module.exports = function(options, callback) {
  ...
  inspectOpts = url.parse(baseUrl + 'inspect?');
  inspectOpts.headers = headers;
  inspectOpts.method = 'POST';

...
var initReq = function(req, res) {
  ...
  req[REQ_ID_KEY] = oReq.id = getValue(req, pluginOpts.REQ_ID_HEADER);
...
var requestFrames = function() { 
  ...
  request(framesOpts, function(err, result) {
    framesPending = false;
    if (err || !result) {
    ...
    if (Array.isArray(frames)) {
      ...
      req[FRAME_KEY] = last && last.frameId;
    }
...
var getFrames = function(req, cb) {
  ...
  getSession(req, function(session) {
    if (!session || session.reqError || session.resError) {
      return framesCallbacks.forEach(function(_cb) {
        _cb('');
      });
    }
    requestFrames();

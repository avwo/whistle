...
var setContext = function (req) {
  ...
  req.clientIp = getValue(req, common.CLIENT_IP_HEADER) || '127.0.0.1';
};
...
var initReq = function (req, res, isServer) {
  ...
  var reqId = getValue(req, common.REQ_ID_HEADER);
  ...
  if (isServer) {
    var parseStatus = headers[common.CUSTOM_PARSER_HEADER];
    req.customParser = oReq.customParser = notEmptyStr(parseStatus);
    req.customParser && addParserApi(req, res, parseStatus, reqId);
    if (parseStatus) {
      delete headers[common.CUSTOM_PARSER_HEADER];
    }
  }
  var conf = pluginOpts.config;
  req[REQ_ID_KEY] = oReq.id = reqId;
  addSessionStorage(req, reqId);
  oReq.isHttp2 = oReq.isH2 = !!headers[conf.ALPN_PROTOCOL_HEADER];
  oReq.existsCustomCert = headers[common.CUSTOM_CERT_HEADER] == '1';
  oReq.isUIRequest = req.isUIRequest =
    headers[common.UI_REQUEST_HEADER] == '1';
  oReq.enableCapture = headers[common.ENABLE_CAPTURE_HEADER] == '1';
  oReq.isFromPlugin = req.fromPlugin = oReq.fromPlugin = headers[common.PLUGIN_REQUEST_HEADER] == '1';
  oReq.ruleValue = getValue(req, common.RULE_VALUE_HEADER);
  oReq.ruleUrl = getValue(req, common.RULE_URL_HEADER);
  oReq.pipeValue = getValue(req, common.PIPE_VALUE_HEADER);
  oReq.sniValue = getValue(req, common.SNI_VALUE_HEADER);
  oReq.hostValue = getValue(req, common.HOST_VALUE_HEADER);
  oReq.extraUrl = getValue(req, common.EXTRA_URL_HEADER);
  oReq.ruleProtocol = getValue(req, common.RULE_PROTO_HEADER);
  var fullUrl = getValue(req, common.FULL_URL_HEADER);
  var pattern = getValue(req, common.PATTERN_HEADER);
  oReq.url = oReq.fullUrl = req.fullUrl = fullUrl;
  req.isHttps = oReq.isHttps = HTTPS_RE.test(fullUrl);
  oReq.remoteAddress = headers[REMOTE_ADDR_HEAD] || '127.0.0.1';
  oReq.remotePort = parseInt(headers[REMOTE_PORT_HEAD], 10) || 0;
  req.notDecompressed = oReq.notDecompressed = headers[common.WS_DECOMPRESS_HEADER] === '1';
  req.fromTunnel = oReq.fromTunnel = headers[FROM_TUNNEL_HEADER] === '1';
  req.fromComposer = oReq.fromComposer = headers[FROM_COM_HEADER] === '1';
  delete headers[common.UI_REQUEST_HEADER];
  delete headers[FROM_TUNNEL_HEADER];
  delete headers[REMOTE_ADDR_HEAD];
  delete headers[REMOTE_PORT_HEAD];
  delete headers[FROM_COM_HEADER];
  delete headers[common.ENABLE_CAPTURE_HEADER];
  delete headers[common.PLUGIN_REQUEST_HEADER];
  if (pattern && pattern[1] === ',') {
    oReq.isRexExp = oReq.isRegExp = pattern[0] === '1';
    oReq.pattern = pattern.substring(2);
  }
  oReq.originHost = headers['x-whistle-origin-host']; // for web ui
  oReq.servername = oReq.serverName = getValue(req, common.SERVER_NAME_HEADER);
  var certCacheInfo = getValue(req, common.CERT_CACHE_INFO);
  ...
  var sniType = headers[SNI_TYPE_HEADER];
  var isSNI;
  if (sniType) {
    delete headers[SNI_TYPE_HEADER];
    if (sniType === '1') {
      isSNI = true;
  ...
  oReq.commonName = getValue(req, common.COMMON_NAME_HEADER);
  oReq.realUrl = getValue(req, common.REAL_URL_HEADER) || oReq.url;
  oReq.relativeUrl = getValue(req, common.RELATIVE_URL_HEADER);
  oReq.method = getValue(req, common.METHOD_HEADER) || 'GET';
  oReq.clientPort = getValue(req, common.CLIENT_PORT_HEADER);
  oReq.globalValue = getValue(req, common.GLOBAL_VALUE_HEADER);
  oReq.proxyValue = getValue(req, common.PROXY_VALUE_HEADER);
  oReq.pacValue = getValue(req, common.PAC_VALUE_HEADER);
  oReq.serverIp = oRes.serverIp = getValue(req, common.HOST_IP_HEADER) || '';
  oReq.statusCode = oRes.statusCode = getValue(req, common.STATUS_CODE_HEADER);
  oReq.pluginVars = getPluginVars(req, PLUGIN_VARS_HEAD);
  oReq.globalPluginVars = getPluginVars(req, GLOBAL_PLUGIN_VARS_HEAD);
  oReq.headers = req.headers;
};
...
module.exports = async function (options, callback) {
  ...
  FROM_COM_HEADER = config.FROM_COM_HEADER;

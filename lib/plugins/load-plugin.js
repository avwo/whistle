...
var initConnectReq = function(req, res) {
  ...
  req.sendEstablished = function(err, cb) {
    ...
    res.write(resCtn, function() {
      typeof cb === 'function' && setTimeout(cb, 16);
    });
  };
  initWsReq(req, res);
};
...
function addFrameHandler(req, socket, maxWsPayload, fromClient, toServer) {
  ...
  if (fromClient === toServer) {
    return hanldeWsSignal(receiver, sender);
  }
  return {
    receiver: receiver,
    sender: sender
...
function formatRawHeaders(headers, req) {
  return Array.isArray(req.rawHeaders) ? formatHeaders(headers, getRawHeaderNames(req.rawHeaders)) : headers;
}
...
function hanldeWsSignal(receiver, sender) {
...
module.exports = function(options, callback) {
  ...
  if (options.debugMode) {
    ...
    var handleGlobalError = function(err) {
  ...
  options.LRU = LRU;
  var cgiHeaders = {};
  cgiHeaders[PROXY_ID_HEADER] = options.shortName;
  var parseCgiUrl = function(url) {
    var opts = parseUrl(url);
    opts.headers = headers;
    return opts;
  };
  var rulesUrlOpts = parseCgiUrl(baseUrl + 'rules/list');
  var valuesUrlOpts = parseCgiUrl(baseUrl + 'values/list');
  var certsInfoUrlOpts = parseCgiUrl(baseUrl + 'get-custom-certs-info');
  
  var requestCgi = function(opts, cb) {
    var timer;
    var execCallback = function(err, data) {
      if (timer === null) {
        return;
      }
      clearTimeout(timer);
    ...
    timer = setTimeout(execCallback, 30000);
    var client = httpRequest(opts, function(res) {
      ...
      res.on('end', function() {
        ...
        try {
          execCallback(null, JSON.parse(body + ''));
        } catch (e) {
          execCallback();
        }
      });
  ...
  options.getCustomCertsInfo = function(cb) {
    requestCgi(certsInfoUrlOpts, cb);
  };
    ...
    getServer(function(server, _port) {
      ...
      server.on('request', function(req, res) {
        ...
        case PLUGIN_HOOKS.HTTP:
          if (httpServer) {
            initReq(req, res);
            var curUrl = req.originalReq.url;
            var svrRes = '';
            ...
            req.request = function(uri, cb) {
              ...
              cb = args[1];
              uri.agent = false;
              var client = httpRequest(uri, function(_res) {
                svrRes = _res;
        ...
        case PLUGIN_HOOKS.HTTP:
          if (httpServer) {
            ...
            req.writeHead = socket.writeHead = function(code, msg, headers) {
              ...
              headers = [
                'HTTP/1.1 ' + code + ' ' + (msg || STATUS_CODES[code] || ''),
            ...
            req.request = function(uri, cb, noParse) {
              ...
              client.on('upgrade', function(_res, resSocket) {
                ...
                if (noParse !== true) {
                  ...
                  hanldeWsSignal(serverParser.receiver, clientParser.sender);
                }
                if (typeof cb === 'function') {

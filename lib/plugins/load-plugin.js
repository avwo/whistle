...
function drainIfEnd(socket) {
  var end = socket.end;
  socket.end = function() {
    var args = arguments;
    drain(socket, function() {
      end.apply(socket, args);
    });
    return this;
  };
}
      ...
      server.on('connect', function(req, socket, head) {
        ...
        case PLUGIN_HOOKS.REQ_READ:
          if (reqRead) {
            ...
            req.sendEstablished(function() {
              drainIfEnd(socket);
              reqRead.emit('connect', req, socket);
            });
          }
          break;
        case PLUGIN_HOOKS.REQ_WRITE:
          if (reqWrite) {
            ...
            req.sendEstablished(function() {
              drainIfEnd(socket);
              reqWrite.emit('connect', req, socket);
            });
          }
          break;
        case PLUGIN_HOOKS.RES_READ:
          if (resRead) {
            ...
            req.sendEstablished(function() {
              drainIfEnd(socket);
              resRead.emit('connect', req, socket);
            });
          }
          break;
        case PLUGIN_HOOKS.RES_WRITE:
          if (resWrite) {
            ...
            req.sendEstablished(function() {
              drainIfEnd(socket);
              resWrite.emit('connect', req, socket);
            });
          }
          break;

...
var proxyIp, proxyPort, pluginName;
...
process._handlePforkUncaughtException = function(msg, e) {
  ...
  fs.writeFileSync(path.join(process.cwd(), pkgName + '.log'), '\r\n' + msg + '\r\n', {flag: 'a'});
  if (typeof process.handleUncaughtErrorMessage === 'function') {
    return process.handleUncaughtErrorMessage(msg, e);
  }
};
...
function createSocket(opts, cb, isHttp) {
  var done;
  var client;
  var headers = common.lowerCaseify(opts.headers);
  if (isHttp) {
    headers['x-whistle-policy'] = 'capture';
  } else if (headers['x-whistle-policy'] !== 'connect') {
    headers['x-whistle-policy'] = opts.justConnect ? 'connect' : 'tunnel';
  }
  headers['x-whistle-request-from'] = pluginName;
  headers.host = opts.path;
  var clientIp = removeIPV6Prefix(opts.clientIp);
...
var isStreamObj = function(obj) {
  return typeof obj.on === 'function' && typeof obj.pipe === 'function';
};
...
var connect = function(opts, cb) {
  return createSocket({
    ...
    clientIp: opts.clientIp || opts.remoteAddress,
    clientPort: opts.clientPort || opts.remotePort
  }, cb);
};
var sendRequest = function(opts, cb) {
  ...
  if (common.isConnect(uri)) {
    return connect({
      host: uri.hostname,
      port: uri.port,
      headers: extend({}, opts.proxyHeaders || opts.headers)
    }, cb);
  }
  ...
  var body;
  var isStream;
  var handleCb = function(err, res, socket) {
    if (!done) {
      done = true;
      cb && cb(err, res, socket);
    }
  };
  if (isWs) {
    isHttps = protocol === 'wss:';
    protocol = isHttps ? 'https:' : 'http:';
    opts.url = common.replaceProtocol(opts.url, protocol);
  } else {
    body = opts.body;
    if (body) {
      if (isStreamObj(body)) {
        isStream = true;
        body.on('error', handleCb);
      } else {
        body = toBinary(body);
      }
    } else if (isStreamObj(opts)) {
      isStream = true;
      body = opts;
    }
  }
  createSocket({
    path: uri.hostname + ':' + (uri.port || (isHttps ? 443 : 80)),
    clientIp: opts.clientIp || opts.remoteAddress,
    clientPort: opts.clientPort || opts.remotePort,
    ...
    client.once(isWs ? 'upgrade' : 'response', function(res, socket) {
      socket.on('error', handleCb);
      handleCb(null, res, socket);
    });
    client.on('error', handleCb);
    if (isStream) {
      body.pipe(client);
...
module.exports = async function(options, callback) {
  ...
  options.request = function(opts, cb) {
    sendRequest(opts, function(err, res, socket) {
      if (socket && !opts.needRawData) {
        wrapTunnelReader(socket);
        wrapTunnelWriter(socket);
      }
      return cb && cb(err, res, socket);
    });
  };

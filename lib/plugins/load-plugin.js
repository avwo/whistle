...
var initStatus = function(req, name) {
  switch(name) {
  case 'pauseSend':
...
var addParserApi = function(req, conn, status, reqId) {
  ...
  if (req.curSendStatus) {
    if (isFunction(req.onSendStateChange)) {
      process.nextTick(req.onSendStateChange.bind(req)); 
    }
  }
  if (req.curReceiveStatus) {
    if (isFunction(req.onReceiveStateChange)) {
  ...
  var emitDisconnect = function(err) {
    ...
    isFunction(req.onDisconnect) && req.onDisconnect(err);
  };
  ...
  parserCallbacks[reqId] = function(data) {
    ...
    if (curSendStatus != sendStatus) {
      req.curSendStatus = sendStatus;
      try {
        isFunction(req.onSendStateChange) && req.onSendStateChange();
      } catch(e) {}
    }
    var curReceiveStatus = req.curReceiveStatus;
    if (curReceiveStatus != receiveStatus) {
      req.curReceiveStatus = receiveStatus;
      try {
        isFunction(req.onReceiveStateChange) && req.onReceiveStateChange();
      } catch(e) {}
    }
    if (Array.isArray(data.toClient)) {
      data.toClient.forEach(function(frame) {
        ...
        try {
          if (buf && isFunction(req.onSendToClient)) {
            req.onSendToClient(buf, frame.binary);
          }
        } catch(e) {}
      });
    }
    if (Array.isArray(data.toServer)) {
      data.toServer.forEach(function(frame) {
        var buf = toBuffer(frame.base64);
        try {
          if (buf && isFunction(req.onSendToServer)) {
            req.onSendToServer(buf, frame.binary);
          }
        } catch(e) {}
      });
    }

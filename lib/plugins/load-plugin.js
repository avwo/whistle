    ...
    getServer(function(server, _port) {
      ...
      server.on('upgrade', function(req, socket, head) {
        ...
        case PLUGIN_HOOKS.WS_REQ_READ:
          if (wsReqRead) {
            initWsReq(req, socket);
            wsReqRead.emit('upgrade', req, socket, head);
          }
          break;
        case PLUGIN_HOOKS.WS_REQ_WRITE:
          if (wsReqWrite) {
            initWsReq(req, socket);
            wsReqWrite.emit('upgrade', req, socket, head);
          }
          break;
        case PLUGIN_HOOKS.WS_RES_READ:
          if (wsResRead) {
            initWsReq(req, socket);
            wsResRead.emit('upgrade', req, socket, head);
          }
          break;
        case PLUGIN_HOOKS.WS_RES_WRITE:
          if (wsResWrite) {
            initWsReq(req, socket);
            wsResWrite.emit('upgrade', req, socket, head);
          }
          break;
        default:
          socket.destroy();
      ...
      server.on('connect', function(req, socket, head) {
        ...
        case PLUGIN_HOOKS.TUNNEL_RES_WRITE:
          if (tunnelResWrite) {
            initConnectReq(req, socket);
            tunnelResWrite.emit('request', req, socket, head);
          }
          break;
        default:
          socket.destroy();

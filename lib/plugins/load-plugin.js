...
var addParserApi = function(req, conn, state, reqId) {
  ...
  if (req.curSendState) {
    process.nextTick(function() {
      req.emit('sendStateChange', req.curSendState);
    });
  }
  if (req.curReceiveState) {
    process.nextTick(function() {
      req.emit('receiveStateChange', req.curReceiveState);
    });
  }
  var disconnected;
  ...
  parserCallbacks[reqId] = function(data) {
    ...
    if (curSendState != sendState) {
      var prevSendState = req.curSendState;
      req.curSendState = sendState;
      try {
        req.emit('sendStateChange', req.curSendState, prevSendState);
      } catch(e) {}
    }
    var curReceiveState = req.curReceiveState;
    if (curReceiveState != receiveState) {
      var prevReceiveState = req.curReceiveState;
      req.curReceiveState = receiveState;
      try {
        req.emit('receiveStateChange', req.curReceiveState, prevReceiveState);
      } catch(e) {}
    }
  ...
  requestData(framesOpts, function(err, result) {
    ...
    if (Array.isArray(frames)) {
      ...
      if (frameId) {
        ...
        req[CLOSED] = !!(last.closed || last.err);
      }
    }

...
module.exports = async function (options, callback) {
  ...
  options.getRules = function (cb, outputArray) {
    if (typeof cb !== 'function') {
      var temp = cb;
      cb = outputArray;
      outputArray = temp;
    }
    requestCgi(outputArray === true ? rulesListUrlOpts : rulesUrlOpts, cb);
  };
  options.getValues = function (cb, outputArray) {
    if (typeof cb !== 'function') {
      var temp = cb;
      cb = outputArray;
      outputArray = temp;
    }
    requestCgi(outputArray === true ? valuesListUrlOpts : valuesUrlOpts, cb);
  };
            ...
            req.passThrough = function (uri, newTrailers) {
                ...
                var client = req.request(_uri || uri, function (_res) {
                  ...
                  if (handleRes) {
                    common.readStream(_res, function(rawBuffer) {
                      handleRes(rawBuffer, function(result) {
                        if (result) {
                          req.setResRules(result.rules);
                          getCustomBody(result.body, _res, function(body) {
              ...
              if (handleReq) {
                common.readStream(req, function(rawBuffer) {
                  handleReq(rawBuffer, function(result) {
                    if (result) {
                      req.setReqRules(result.rules);
                      getCustomBody(result.body, req, function(body) {

...
var initStatsReq = function(req, res) {
  initReq(req, res);
  res.end();
};

var initServerReq = function(req, res) {
...
module.exports = function(options, callback) {
  ...
  var initServers = function(_ctx) {
    ...
    var hasServer = startServer || startStatsServer || startResStatsServer || startUIServer || startRulesServer
      || startResRulesServer || startTunnelRulesServer || startTunnelServer;
    ...
    if (startServer) {
      ++count;
      getServer(function(server, _port) {
        server.on('request', initServerReq);
        server.on('upgrade', initServerReq);
        server.on('connect', initConnectReq);
    ...
    if (startStatsServer) {
      ++count;
      getServer(function(server, _port) {
        server.on('request', initStatsReq);

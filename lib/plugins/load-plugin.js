...
var initReq = function(req, res, isServer) {
  ...
  req.customParser = oReq.customParser = notEmptyStr(customParserHeader);
  if (isServer && req.customParser) {
...
var getFrames = function(req, cb) {
  ...
  if (!isTunnel && !/^ws/.test(url) && !req.customParser) {
    return;
  }
  cb[REQ_KEY] = req;
  framesCallbacks.push(cb);
...
module.exports = async function(options, callback) {
  ...
  var authPort, port, statsPort, resStatsPort, uiPort, rulesPort, resRulesPort, tunnelRulesPort, tunnelPort;
  ...
  var initServers = function(_ctx) {
    ...
    var hasServer = execAuth || startServer || startStatsServer || startResStatsServer || startUIServer || startRulesServer
    ...
    getServer(async function(server, _port) {
      var maxWsPayload;
      var authServer, uiServer, httpServer, statsServer, resStatsServer;
      ...
      if (execAuth) {
        ...
        authServer.on('request', async function(req, res) {
          ...
          try {
            ...
            res.end(result === false ? 'Forbidden' : '');
          } catch (e) {

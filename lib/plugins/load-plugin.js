...
var initReq = function (req, res, isServer) {
  ...
  if (isServer) {
    var parseStatus = headers[pluginOpts.CUSTOM_PARSER_HEADER];
  ...
  oReq.existsCustomCert = headers[pluginOpts.CUSTOM_CERT_HEADER] == '1';
  oReq.isUIRequest = req.isUIRequest =
    headers[pluginOpts.UI_REQUEST_HEADER] == '1';
  oReq.enableCapture = headers[pluginOpts.ENABLE_CAPTURE_HEADER] == '1';
  oReq.isFromPlugin = req.fromPlugin = oReq.fromPlugin = headers[pluginOpts.PLUGIN_REQUEST_HEADER] == '1';
  ...
  req.notDecompressed = oReq.notDecompressed = headers['x-whistle-disable-ws-decompress'] === '1';
  ...
  if (pattern) {
    delete headers['x-whistle-raw-pattern_'];
    if (pattern[1] === ',') {

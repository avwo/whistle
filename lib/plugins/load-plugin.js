...
var getBody = function() {
  return toBuffer(this.base64);
};
var getText = function() {
  var body = toBuffer(this.base64) || '';
  if (body && !isUtf8(body)) {
    try {
...
var execCallback = function(id, cbs, item) {
  ...
  if (cbs === reqCallbacks || (!item || item.endTime)) {
    ...
    if (item) {
      if (item.req) {
        item.req.getBody = getBody;
        item.req.getText = getText;
      }
      if (item.res) {
        item.res.getBody = getBody;
        item.res.getText = getText;
      }
    }
  }
};

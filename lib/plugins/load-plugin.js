...
function addFrameHandler(req, socket, maxWsPayload, toServer) {
  socket.wsExts = req.headers['sec-websocket-extensions'] || '';
  var receiver = wsParser.getReceiver(socket, !toServer, maxWsPayload);
  var emit = socket.emit;
  var write = socket.write;
  var end = socket.end;
...
module.exports = function(options, callback) {
  ...
  var normalizeArgs = function (uri, cb, req, curUrl, isWs) {
    ...
    uri.method = isWs ? 'GET' : req.method;
        ...
        case PLUGIN_HOOKS.HTTP:
          if (httpServer) {
            ...
            req.request = function(uri, cb) {
              ...
              uri = args[0];
              cb = args[1];
              var client = http.request(uri, cb);
        ...
        case PLUGIN_HOOKS.HTTP:
          if (httpServer) {
            ...
            req.writeHead = socket.writeHead = function(code, msg, headers) {
              ...
              origWrite.call(socket, headers.join('\r\n'));
            };
            req.request = function(uri, cb) {
              var args = normalizeArgs(uri, cb, req, curUrl, true);
              uri = args[0];
              cb = args[1];
              var client = http.request(uri);
        ...
        case PLUGIN_HOOKS.WS_REQ_READ:
          if (wsReqRead) {
            ...
            req.sendEstablished(function() {
              addFrameHandler(req, socket, maxWsPayload, true);
          ...
          if (tunnelServer) {
            ...
            req.sendEstablished(function() {
              req.connect = function(cb) {
                ...
                client.on('connect', function(_res, socket) {

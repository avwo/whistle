...
var addRemoteInfo = function(req, headers) {
  ...
  if (oReq) {
    headers[REMOTE_ADDR_HEAD] = oReq.remoteAddress;
    headers[REMOTE_PORT_HEAD] = oReq.remotePort;
  }
};
...
var getValue = function (req, name, keep) {
...
var setContext = function (req, flags) {
  if (ctx) {
    req.ctx = ctx;
  }
  ...
  if (!flags) {
    req.clientIp = getValue(req, common.CLIENT_IP_HEADER) || LOCALHOST;
  }
};
...
var initReq = function (req, res, isServer) {
  ...
  var flags = common.getFlags(headers);
  setContext(req, flags);
  ...
  req._isUpgrade = flags._isUpgrade === '1';
  oReq.clientId = getValue(req, common.CLIENT_ID_HEADER, true);
  oReq.isHttp2 = oReq.isH2 = !!headers[conf.ALPN_PROTOCOL_HEADER];
  oReq.existsCustomCert = flags._existsCustomCert == '1';
  oReq.isUIRequest = req.isUIRequest = flags._isUIRequest == '1';
  oReq.enableCapture = flags._enableCapture == '1';
  oReq.isFromPlugin = req.fromPlugin = oReq.fromPlugin = flags.isPluginReq == '1';
  ...
  req.clientIp = oReq.clientIp = flags.clientIp || LOCALHOST;
  req.clientPort = oReq.clientPort = +flags.clientPort || 0;
  oReq.remoteAddress = flags._remoteAddr || LOCALHOST;
  oReq.remotePort = +flags._remotePort || 0;
  req.notDecompressed = oReq.notDecompressed = flags._noDecompress === '1';
  req.fromTunnel = oReq.fromTunnel = flags.fromTunnel === '1';
  req.fromComposer = oReq.fromComposer = flags.fromComposer === '1';

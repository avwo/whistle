...
function formatRawHeaders(headers, req) {
  var rawHeaders = req.rawHeaders || req;
  return Array.isArray(rawHeaders) ? formatHeaders(headers, getRawHeaderNames(rawHeaders)) : headers;
}
...
module.exports = function(options, callback) {
  ...
  if (options.debugMode && !options._inspect) {
    var cacheLogs = [];
  ...
  var normalizeArgs = function (uri, cb, req, curUrl, isWs, opts, sessionId, alpnProtocol) {
    ...
    if (!opts || !notEmptyStr(opts.host)) {
      ...
      if (isHttps) {
        ...
        if (sessionId && alpnProtocol) {
          headers[config.SESSION_ID_HEADER] = sessionId;
    ...
    getServer(function(server, _port) {
      ...
      server.on('request', function(req, res) {
        ...
        case PLUGIN_HOOKS.HTTP:
          if (httpServer) {
            initReq(req, res);
            var sessionId = req.headers[config.SESSION_ID_HEADER];
            ...
            req.request = function(uri, cb, opts) {
              var args = normalizeArgs(uri, cb, req, curUrl, false, opts, sessionId, alpnProtocol);
              uri = args[0];
              cb = args[1];
              uri.agent = false;

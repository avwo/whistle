...
var appendTrailers = function (_res, res, newTrailers, req) {
  ...
  common.onReadEnd(_res, function () {
...
var setupConnect = function (req, socket, isServer) {
  if (req.originalReq && req.originalRes) {
    return;
  }
  var established;
  initReq(req, socket, isServer);
  req.sendEstablished = function (err, cb) {
    ...
    socket.once('data', function (chunk) {
      if (!req._hasError) {
        ...
        chunk.length > 1 && socket.unshift(chunk.slice(1));
        cb();
      }
    });
    return socket.write(resCtn.join('\r\n'));
  };
};
      ...
      var emitHttpPipe = function (httpServer, req, socket) {
        if (httpServer) {
          ...
          req.sendEstablished(function () {
            ...
            decoder.destroy = encoder.destroy = destroy;
            decoder.on('error', destroy);
            encoder.on('error', destroy);

...
process._handlePforkUncaughtException = function(msg, e) {
  msg = [
    ...
    msg].join('\n');
    pluginInted && debugMode && console.error(msg); // eslint-disable-line
...
function wrapTunnelWriter(socket, toServer) {
  ...
  if (toServer) {
    socket.closeWebSocket = function(code, data, mask, cb) {
      sender.close(code || 1000, data, mask, cb);
    };
    socket.ping = function(data, opts) {
      return sender.ping(data, opts);
    };
  }
...
module.exports = async function(options, callback) {
  ...
  options.wrapWsReader = function(socket, maxPayload) {
    wrapTunnelReader(socket, true, maxPayload);
    return socket;
  };
  options.wrapWsWriter = function(socket) {

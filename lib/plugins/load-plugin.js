...
var initReq = function(req, res, isServer) {
  ...
  oReq.pluginVar = getValue(req, pluginOpts.PLUGIN_VARS_HEAD);
  oRes.serverIp = getValue(req, pluginOpts.HOST_IP_HEADER) || '127.0.0.1';
  oRes.statusCode = getValue(req, pluginOpts.STATUS_CODE_HEADER);
  var globalPluginVars = toBuffer(req.headers[pluginOpts.GLOBAL_PLUGIN_VARS_HEAD]);
  if (globalPluginVars) {
    delete req.headers[pluginOpts.GLOBAL_PLUGIN_VARS_HEAD];
    try {
      globalPluginVars = JSON.parse(globalPluginVars.toString());
      if (Array.isArray(globalPluginVars)) {
        oReq.pluginVars = globalPluginVars;
      }
    } catch (e) {}
  }
  oReq.pluginVars = oReq.globalPluginVars = oReq.pluginVars || [];
};

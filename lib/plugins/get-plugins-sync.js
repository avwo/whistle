    ...
    try {
      ...
      if (pkg && pkg.version) {
        ...
        var plugin = {
          isSys: isSys,
          moduleName: pkg.name,
          networkMenus: util.getPluginMenu(conf.networkMenus, simpleName),
          rulesMenus: util.getPluginMenu(conf.rulesMenus, simpleName),
          valuesMenus: util.getPluginMenu(conf.valuesMenus, simpleName),
        ...
        plugin[util.PLUGIN_VALUES] = pluginUtil.parseValues(util.readFileSync(path.join(dir, '_values.txt')));

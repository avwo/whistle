	...
	Object.keys(plugins).forEach(function(name) {
		...
		try {
			...
			if (pkg && pkg.version) {
				...
				var rulesPackage = !!pkg.rulesPackage;
				var rulesText = comUtil.trim(comUtil.readFileSync(path.join(dir, 'rules.txt')));
				var _rulesText = comUtil.trim(comUtil.readFileSync(path.join(dir, '_rules.txt')));
				var plugin = {
						path: dir,
						pkgPath: pkgPath,
				...
				if (rulesPackage || rulesText) {
					_rules[simpleName] = plugin;
				}
			}
		} catch(e) {}
	});

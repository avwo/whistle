...
function readReqRules(dir, callback) {
  readFile(path.join(path.join(dir, '_rules.txt')), UTF8_OPTIONS, function(err, rulesText) {
    if (err) {
      readFile(path.join(path.join(dir, 'reqRules.txt')), UTF8_OPTIONS, function(_, rulesText) {
    ...
    if (!pkg || pkg.path != newPkg.path || pkg.mtime != newPkg.mtime) {
      ...
      readJson(path.join(newPkg.path, 'package.json'), function(_, result) {
        if (result && result.version) {
          ...
          readFile(path.join(path.join(newPkg.path, 'rules.txt')), UTF8_OPTIONS, function(err, rulesText) {
            ...
            readReqRules(newPkg.path, function(rulesText) {
              ...
              readFile(path.join(path.join(newPkg.path, '_values.txt')), UTF8_OPTIONS, function(err, rulesText) {
                newPkg[util.PLUGIN_VALUES] = pluginUtil.parseValues(util.renderPluginRules(rulesText, result, simpleName));
                readFile(path.join(path.join(newPkg.path, 'resRules.txt')), UTF8_OPTIONS, function(err, rulesText) {

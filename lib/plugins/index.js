...
function getRules(req, port, callback) {
    ...
    if (req.rules.rule) {
      options.headers[CUR_RULE_HEADER] = encodeURIComponent(req.rules.rule.rawPattern + ' ' + req.rules.rule.matcher);
    }
...
pluginMgr.getTunnelRules = function(req, callback) {
	...
	(function getTunnelRulesMgr() {
		loadPlugin(plugin, function(err, ports) {
			plugin.ports = ports;
			getTunnelRules(req, ports && ports.tunnelRulesPort, function(err, body) {
				if (retryCount++ < 5 && err) {
				...
				if (err || !body) {
					...
					if (body != plugin.__tunnelRules) {
						var rulesMgr = new RulesMgr();
						rulesMgr.parse(body + (plugin._rules ? '\n' + plugin._rules : ''), plugin.path);
						 plugin._tunnelRulesMgr = rulesMgr;
						plugin.__tunnelRules = body;
					}
					callback(plugin._tunnelRulesMgr, plugin);
				}
			});
		});
	})();
...
function resolvePluginRule(req) {
  ...
  if (rules.plugin) {
    ...
    if (PLUGIN_RULE_RE.test(value) || PLUGIN_RULE_RE2.test(value)) {
      ...
      pluginMgr = _getPlugin(RegExp.$1 + ':');
    }
  }
  
  var plugin = getPluginByRuleUrl(comUtil.rule.getUrl(rules.rule));
  if (pluginMgr) {
    if (plugin && pluginMgr != plugin) {
      plugin = null;
      delete rules.rule;
    }
  } else {

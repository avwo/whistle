...
function getRulesFromPlugins(type, req, res, plugins, callback) {
  ...
  loadPlugins(plugins, function(ports) {
    ...
    ports.forEach(function(item, i) {
      ...
      requestRules(extend({}, options), function(err, body, values, raw) {
...
function getOptions(req, res, type) {
  ...
  if (req.ruleValue) {
    headers[RULE_VALUE_HEADER] = encodeURIComponent(req.ruleValue);
  }
  ...
  if (typeof req.clientIp == 'string') {
    headers[CLIENT_IP_HEADER] = encodeURIComponent(req.clientIp);
  }
  ...
  if (isResRules) {
    ...
    headers[HOST_IP_HEADER] = encodeURIComponent(req.hostIp || LOCALHOST);
...
function getRulesMgr(type, req, res, callback) {
  ...
  getRulesFromPlugins(type, req, res, [plugin, req._pluginMgr], callback);
}
...
pluginMgr.getTunnelRules = function(req, callback) {
  getRulesMgr('tunnelRules', req, callback);
};

...
var HAS_CERT_CACHE = 'x-whistle-has-cert-cache';
var INTERVAL = 6000;
...
pluginMgr.loadCert = function(req, plugin, callback) {
  loadPlugin(plugin, function(err, ports) {
    ...
    if (req.hasCertCache) {
      options.headers[HAS_CERT_CACHE] = req.hasCertCache;
    }
...
pluginMgr.loadAuthPlugins = function(req, callback) {
  ...
  _loadPlugins(authPlugins, function(ports) {
    ...
    authReq(true, ports, req, options, function(forbidden) {
      if (forbidden) {
        if (req._redirectUrl) {
          return callback(req._redirectUrl);
        }
        var status = forbidden === 'Forbidden' ? (req._showLoginBox ? 401 : 403) : 502;
  ...
  loadPlugins(plugins, function(ports) {
    ...
    authReq(!isResRules, ports, req, options, function(forbidden) {
      if (forbidden) {
        ...
        if (req._redirectUrl) {
          mgr.parse('* redirect://' + req._redirectUrl);
        } else {
          var status = forbidden === 'Forbidden' ? (req._showLoginBox ? 401 : 403) : 502;
          mgr.parse('* status://' + status + ' resBody://{msg} resType://html resCharset://utf8');
        }
        return callback(mgr);
      }

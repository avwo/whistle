...
var REMOTE_RULES_RE =
  /^\s*@(`?)(whistle\.[a-z\d_\-]+(?:\/[^\s#]*)?|(?:https?:\/\/|[a-z]:[\\/]|~?\/)[^\s#]+)\s*\1(?:#.*)?$/im;
...
function addPluginVars(req, headers, rule) {
  ...
  if (rule.rawPattern) {
    headers['x-whistle-raw-pattern_'] = encodeURIComponent(
      (rule.isRegExp ? 1 : 0) + ',' + rule.rawPattern
    );
  }
...
var MAX_VALUE_LEN = 1024 * 1024 * 32;
...
function requestValue(url, callback, isBin) {
  var options = resolveKey(url);
  if (!options) {
    return;
  }
  options.needRawData = isBin;
  var handleCallback = function(_, body, res) {
    callback(res && res.statusCode == 200 ? body : '');
  };
...
pluginMgr.requestText = function(url, callback) {
  return requestValue(url, callback);
};

pluginMgr.requestBin =function(url, callback) {
  return requestValue(url, callback, true);
};

pluginMgr.requestStream =function(url, callback) {
  var options = resolveKey(url);
  if (!options) {
    return;
  }
  httpMgr.requestStream(extend({}, options), function(err, res) {
    if (err) {
      return httpMgr.requestStream(options, callback);
    }
    callback(err, res);
  });
  return options;

...
function getPluginsRules(options, ports, callback) {
  ...
  var execCallback = function() {
    ...
    body = body.filter(emptyFilter).join('\n');
    raw = raw.filter(emptyFilter).join('\n');
    var vals;
    values.reverse().forEach(function(item) {
      if (item) {
        vals = vals ? extend(vals, item) : item;
      }
    });
    callback(null, body, vals, raw);
  };
  ...
  ports.forEach(function(port, i) {
    ...
    _options.port = port;
    requestRules(_options, function(err, _body, _values, _raw) {
      if (_body) {
        body[i] = _body;
      }
      if (_values) {
...
function getRules(req, port, _port, callback) {
  if (!port) {
    return callback();
  }
  var rules = req.rules;
  ...
  getPluginsRules(options, [port, _port], callback);
}
...
pluginMgr.getRules = function(req, callback) {
  ...
  loadPlugins([plugin, req._pluginMgr], function(ports, _ports) {
    var retryCount = 0;
    (function getRulesMgr() {
      getRules(req, ports && ports.rulesPort, _ports && _ports.rulesPort, function(err, body, values, raw) {
        if (retryCount++ < 5 && err) {
...
function getResRules(req, res, port, _port, callback) {
  if (!port) {
    return callback();
  }
  var options = url.parse(req.fullUrl);
...
pluginMgr.getResRules = function(req, res, callback) {
  ...
  loadPlugins([plugin, req._pluginMgr], function(ports, _ports) {
    var retryCount = 0;
    (function getRulesMgr() {
      getResRules(req, res, ports && ports.resRulesPort, _ports && _ports.resRulesPort, function(err, body, values, raw) {
        if (retryCount++ < 5 && err) {
          setTimeout(getRulesMgr, 60);
          return;
        }
...
function postStatus(req, data) {
  ...
  loadPlugins([plugin, req._pluginMgr], function(ports, _ports) {
    sendStatus(ports && ports.statusPort);
    sendStatus(_ports && _ports.statusPort);
  });
}

pluginMgr.postStatus = postStatus;

function getTunnelRules(req, port, _port, callback) {
  if (!port) {
    return callback();
  }
  var options = {
...
pluginMgr.getTunnelRules = function(req, callback) {
  ...
  loadPlugins([plugin, req._pluginMgr], function(ports, _ports) {
    ...
    (function getTunnelRulesMgr() {
      getTunnelRules(req, ports && ports.tunnelRulesPort, _ports && _ports.tunnelRulesPort, function(err, body, values, raw) {
        if (retryCount++ < 5 && err) {
...
function resolvePluginRule(req) {
  ...
  if (_plugin) {
    if (plugin) {
      req._pluginMgr = _plugin.plugin;
      req._ruleValue = _plugin.value;
    } else {
      req.pluginMgr = _plugin.plugin;
      req.ruleValue = _plugin.value;
    }
  }

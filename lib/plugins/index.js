...
var PLUGIN_VAR_HEAD = 'x-whistle-plugin-var' + config.uid;
...
function addPluginVars(req, headers, rule) {
  if (!rule) {
    ...
    delete headers[PLUGIN_VAR_HEAD];
    return;
  }
  ...
  value = req._pluginVarMap && req._pluginVarMap[name];
  if (value) {
    headers[PLUGIN_VAR_HEAD] = encodeURIComponent(value);
  } else {
    delete headers[PLUGIN_VAR_HEAD];
  }
}
...
function loadPlugin(plugin, callback) {
  ...
  util.getBoundIp(config.host, function(host) {
    ...
    p.fork({
      ...
      PLUGIN_VAR_HEAD: PLUGIN_VAR_HEAD,

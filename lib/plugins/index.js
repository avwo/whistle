...
var propertiesStorage = require('../rules/util').properties;
...
function pluginIsDisabled(name) {
	var disabledPlugins = propertiesStorage.get('disabledPlugins') || {};
	return disabledPlugins[name];
}

function pluginsRulesIsDisabled(name) {
	var disabledPluginsRules = propertiesStorage.get('disabledPluginsRules') || {};
	return disabledPluginsRules[name];
}

pluginMgr.getPlugin = function(protocol) {
	return pluginsRulesIsDisabled(protocol.slice(0, -1)) ? null : plugins[protocol];
};

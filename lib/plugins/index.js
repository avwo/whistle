...
var REMOTE_RULES_RE = /^\s*@(https?:\/\/[^\s#]+)\s*(?:#.*)?$/im;
...
pluginMgr.on('updateRules', function() {
  ...
  Object.keys(allPlugins).sort(function(a, b) {
    ...
    if (rules) {
      rules = rules.replace(REMOTE_RULES_RE, function (_, rulesUrl) {
        hasRulesUrl = true;
  ...
  util.getRuleValue(resScript, function(fileRules) {
    ...
    if (fileRules) {
      if (!util.isRulesContent(fileRules)) {
        ...
        if (util.isOperatorContent(fileRules)) {
          fileRules = util.opToRules(resScript.rawPattern, fileRules);
        }

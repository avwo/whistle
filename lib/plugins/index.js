...
pluginMgr.CUR_HOST_HEADER = CUR_HOST_HEADER;
pluginMgr.CUR_PROXY_HEADER = CUR_PROXY_HEADER;
...
function getPluginByPath(req) {
  var options = url.parse(req.fullUrl);
  if (!config.isLocalUIUrl(options.hostname)) {
    return;
  }
  ...
  if (isWhistlePlugin || /^\/plugin\.([^\/\?]+)(.*)$/.test(path)) {
    path = RegExp.$2;
    var pluginHomePage = isWhistlePlugin ? _getPlugin(RegExp.$1 + ':') : getPluginByName(RegExp.$1);
    if (pluginHomePage) {
      req.url = path || '/';
      req.fullUrl = options.protocol + '//' + options.host + req.url;
    }
    return pluginHomePage;
  }
}

pluginMgr.getPluginByPath = getPluginByPath;

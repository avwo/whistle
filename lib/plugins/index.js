...
var CUSTOM_INSPECT_HEADER = 'x-whistle-inspect';
...
function showVerbose(oldData, newData) {
  ...
  if (uninstallData || installData || updateData) {
    console.log('\n***********[%s] %s has changed***********', util.formatDate(), 'plugins');
  }
...
function addRuleHeaders(req, rules, headers, customInspect) {
  ...
  if (customInspect) {
    headers[CUSTOM_INSPECT_HEADER] = customInspect;
  } else {
    delete headers[CUSTOM_INSPECT_HEADER];
  }
...
function loadPlugin(plugin, callback) {
  ...
  p.fork({
    ...
    CUSTOM_INSPECT_HEADER: CUSTOM_INSPECT_HEADER,
...
function getRulesFromPlugins(type, req, res, callback) {
  loadPlugins(req.whistlePlugins, function(ports) {
...
function postStats(req, res) {
  var plugins = req.whistlePlugins;
  if (!plugins) {
    plugins = req.whistleTunnelPlugins;
    req.whistleTunnelPlugins = null;
  }
  if (!plugins) {
    return;
  }

  ...
  loadPlugins(plugins, function(ports) {
    ...
    authReq(!isResRules, ports, req, options, function(forbidden) {
      if (forbidden) {
        ...
        if (req._redirectUrl) {
          mgr.parse('* resHeaders://x-server=whistle redirect://' + req._redirectUrl);
        } else {
          var status = forbidden === 'Forbidden' ? (req._showLoginBox ? 401 : 403) : 502;
          mgr.parse('* resHeaders://x-server=whistle status://' + status + ' resBody://{msg} resType://html resCharset://utf8');
        }
        return callback(mgr);
      }

    ...
    if (!pkg || pkg.path != newPkg.path || pkg.mtime != newPkg.mtime) {
      ...
      fse.readJson(newPkg.pkgPath, function(err, result) {
        if (result && result.version) {
          ...
          newPkg.homepage = util.getHomePageFromPackage(result);
          newPkg.description = result.description;
          newPkg.moduleName = result.name;
          newPkg.registry = comUtil.getRegistry(result);
...
function addRuleHeaders(req, rules, headers) {
  ...
  var proxyRule = getHeaderValueFromRule(rules.proxy);
  if (proxyRule) {
    headers[CUR_PROXY_HEADER] = proxyRule.raw;
    if (proxyRule.value) {
      headers[PROXY_VALUE_HEADER] = proxyRule.value;
    }
  }

...
function showUpdateInfo(action, name) {
  return '[' + (new Date().toLocaleTimeString()) + '] ' + action + ' ' + name.slice(0, -1);
}
...
function showVerbose(oldData, newData) {
  ...
  uninstallData &&
    Object.keys(uninstallData).forEach(function (name) {
      console.log(colors.red(showUpdateInfo('UNINSTALL', name)));
    });
  installData &&
    Object.keys(installData).forEach(function (name) {
      console.log(colors.green(showUpdateInfo('INSTALL', name)));
    });
  updateData &&
    Object.keys(updateData).forEach(function (name) {
      console.log(colors.yellow(showUpdateInfo('UPDATE', name)));
    });
}
...
function isClosed(req) {
  return req._hasClosed || req._hasError;
}
...
function resolvePipe(req, m1, m2) {
  if (req.isPluginReq) {
    return;
  }
  var filter = m1 && m1.resolveFilter(req);
  if (m2) {
    var filter2 = m2.resolveFilter(req);
    filter = filter ? extend(filter, filter2) : filter2;
  }
  if (util.isIgnored(filter, 'pipe')) {
    return;
  }
  return (m1 && m1.resolvePipe(req)) || (m2 && m2.resolvePipe(req));
}
...
function resolvePipePlugin(req, callback) {
  if (req._pipePlugin == null) {
    ...
    if (config.multiEnv) {
      pipe = resolvePipe(req, hRules, rulesMgr);
    } else {
      pipe = resolvePipe(req, rulesMgr, hRules);
    }
    var plugin;
    req._pipeRule = pipe;
    ...
    resolvePipePlugin(req, function (ports) {
      ...
      var handleConnect = function (socket, _res) {
        if (!done) {
          ...
          if (isClosed(req)) {
            return util.endSocket(socket);
          }
          req.once('_closed', function () {
            util.endSocket(socket);
          });
          callback(socket);
        } else if (socket) {
          util.endSocket(socket);
      ...
      client.once('error', function () {
        if (!done && !req._hasError) {
          client = config.connect(options, handleConnect);
          client.once('error', function (err) {
            if (!done && !isClosed(req)) {
              debugMode && console.log(type, err);

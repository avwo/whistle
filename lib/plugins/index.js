...
function getRulesFromPlugins(type, req, res, callback) {
  loadPlugins(req.whistlePlugins, function(ports) {
    ...
    var hookName = isResRules ? 'RES_RULES' : 'REQ_RULES';
    if (!req.fullUrl.indexOf('tunnel://')) {
      hookName = 'TUNNEL_RULES';
    }
...
function postStats(req, res) {
  ...
  loadPlugins(plugins, function(ports) {
    ...
    options.headers[PLUGIN_HOOK_NAME_HEADER] = PLUGIN_HOOKS[res ? 'REQ_STATS' : 'RES_STATS'];

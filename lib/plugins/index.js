...
function requestRules(options, callback) {
  comUtil.getResponseBody(options, function(err, body) {
    ...
    callback(err, typeof data.rules == 'string' ? data.rules : '', data.values)
  });
}
...
pluginMgr.getRules = function(req, callback) {
	...
	(function getRulesMgr() {
		loadPlugin(plugin, function(err, ports) {
			plugin.ports = ports;
			getRules(req, ports && ports.rulesPort, function(err, body, values) {
				if (retryCount++ < 5 && err) {
...
pluginMgr.getResRules = function(req, res, callback) {
	...
	(function getRulesMgr() {
		loadPlugin(plugin, function(err, ports) {
			getResRules(req, res, ports && ports.resRulesPort, function(err, body, values) {
				if (retryCount++ < 5 && err) {
					setTimeout(getRulesMgr, 60);
					return;
				}
...
pluginMgr.getTunnelRules = function(req, callback) {
	...
	(function getTunnelRulesMgr() {
		loadPlugin(plugin, function(err, ports) {
			plugin.ports = ports;
			getTunnelRules(req, ports && ports.tunnelRulesPort, function(err, body, values) {
				if (retryCount++ < 5 && err) {

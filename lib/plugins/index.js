...
var JSON_RE = /^\{[\s\S]+\}$/;
var RULE_VALUE_HEADER = common.RULE_VALUE_HEADER;
...
pluginMgr.loadCert = function (req, plugin, callback) {
  loadPlugin(plugin, function (err, ports) {
    ...
    options.headers[common.SERVER_NAME_HEADER] = encodeURIComponent(req.serverName);
    options.headers[common.COMMON_NAME_HEADER] = encodeURIComponent(req.commonName);
    if (!req.useSNI || req.isHttpsServer) {
      options.headers[config.SNI_TYPE_HEADER] = req.isHttpsServer ? '1' : '0';
    }
    if (req.sniRuleValue) {
      options.headers[common.SNI_VALUE_HEADER] = encodeURIComponent(req.sniRuleValue);
    }
    if (req.hasCertCache) {
      options.headers[common.CERT_CACHE_INFO] = req.hasCertCache;
    }
...
function addRealUrl(req, newHeaders) {
  ...
  if (realUrl && realUrl != req.fullUrl) {
    newHeaders[common.REAL_URL_HEADER] = encodeURIComponent(realUrl);
  }
  var rule = req.rules && req.rules.rule;
  if (rule) {
    if (rule.url !== rule.matcher) {
      var relPath = rule.url.substring(rule.matcher.length);
      newHeaders[common.RELATIVE_URL_HEADER] = encodeURIComponent(relPath);
    }
    if (rule.url) {
      newHeaders[common.RULE_URL_HEADER] = encodeURIComponent(rule.url);
    }
  }
}
...
function addPluginVars(req, headers, rule) {
  ...
  if (rule.rawPattern) {
    headers[common.PATTERN_HEADER] = encodeURIComponent((rule.isRegExp ? 1 : 0) + ',' + rule.rawPattern);
  }
  if (rule.url) {
    ...
    if (extraUrl) {
      headers[common.EXTRA_URL_HEADER] = encodeURIComponent(extraUrl);
    }
  }
...
function addPluginHeaders(req, headers, isKey) {
  if (req.reqId) {
    headers[common.REQ_ID_HEADER] = req.reqId;
  }
  if (req.fullUrl) {
    headers[common.FULL_URL_HEADER] = encodeURIComponent(req.fullUrl);
  }
  ...
  util.setFlagHeader(headers, req.fromTunnel, FROM_TUNNEL_HEADER);
  util.setFlagHeader(headers, req.fromComposer, config.FROM_COM_HEADER);
...
function addRuleHeaders(req, rules, headers, isPipe) {
  ...
  if (localHost) {
    headers[common.HOST_VALUE_HEADER] = localHost;
  }
  var rule = rules.rule;
  if (isPipe) {
    if (req._pipePlugin) {
  ...
  if (proxyRule) {
    headers[common.PROXY_VALUE_HEADER] = proxyRule;
  }
  var pac = getValue(rules.pac);
  if (pac) {
    headers[common.PAC_VALUE_HEADER] = pac;
  }
  if (req._pipeValue) {
    headers[common.PIPE_VALUE_HEADER] = encodeURIComponent(req._pipeValue);
  }
  if (req.customParser) {
    headers[common.CUSTOM_PARSER_HEADER] = req.customParser;
  } else {
    delete headers[common.CUSTOM_PARSER_HEADER];
  }
  if (req.globalValue) {
    headers[common.GLOBAL_VALUE_HEADER] = encodeURIComponent(req.globalValue);
  }
  return headers;
}
...
function getOptions(req, res, type, isPipe) {
  ...
  headers[common.METHOD_HEADER] = encodeURIComponent(req.method || 'GET');

  if (isResRules) {
    headers.host = req.headers.host;
    headers[common.HOST_IP_HEADER] = req.hostIp || LOCALHOST;
    headers[common.STATUS_CODE_HEADER] = encodeURIComponent(
  ...
  util.setFlagHeader(headers, req.isPluginReq, common.PLUGIN_REQUEST_HEADER);
  util.setFlagHeader(headers, req._existsCustomCert, common.CUSTOM_CERT_HEADER);
  util.setFlagHeader(headers, req._enableCapture, common.ENABLE_CAPTURE_HEADER);
  ...
  return function (req, res, callback) {
    ...
    resolvePipePlugin(req, function (ports) {
      ...
      if (req._noDecompress) {
        options.headers[common.WS_DECOMPRESS_HEADER] = 1;
      }
      if (req._isUpgrade) {
      ...
      delete options.headers[common.CUSTOM_PARSER_HEADER];
...
pluginMgr.resolveKey = function(url, rule, req) {
  ...
  if (req) {
    ...
    headers[common.METHOD_HEADER] = req.method ? encodeURIComponent(req.method) : 'GET';
    delete headers.upgrade;
    delete headers.connection;
    delete headers['content-length'];
  }
  if (ruleName) {
    headers[common.RULE_PROTO_HEADER] = ruleName;
  }
  return {
    originalKey: url,
    pluginName: name,

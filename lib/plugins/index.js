...
function addRuleHeaders(req, rules, headers, isPipe) {
  ...
  if (req.customParser) {
    ...
    delete headers[CUSTOM_PARSER_HEADER];
  }
  if (req.globalValue) {
...
function getOptions(req, res, type, isPipe) {
  ...
  util.setFlagHeader(headers, req.isPluginReq, PLUGIN_REQUEST_HEADER);
  util.setFlagHeader(headers, req._existsCustomCert, CUSTOM_CERT_HEADER);
  util.setFlagHeader(headers, req._enableCapture, ENABLE_CAPTURE_HEADER);

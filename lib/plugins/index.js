...
function resolvePipePlugin(req) {
  if (req._pipePlugin == null) {
    ...
    req._pipeValue = value;
    req._pipePlugin = plugin;
  }
  return req._pipePlugin;
}
...
function getPipe(type, hookName) {
  ...
  return function(req, res, callback) {
    ...
    loadPlugin(resolvePipePlugin(req), function(err, ports) {
...
pluginMgr.getWsReqReadPipe = getPipe('wsReqRead', 'WS_REQ_READ');
pluginMgr.getWsReqWritePipe = getPipe('wsReqWrite', 'WS_REQ_WRITE');
pluginMgr.getWsResReadPipe = getPipe('wsResRead', 'WS_RES_READ');
pluginMgr.getWsResWritePipe = getPipe('wsResWrite', 'WS_RES_WRITE');
pluginMgr.getTunnelReqReadPipe = getPipe('tunnelReqRead', 'TUNNEL_REQ_READ');
pluginMgr.getTunnelReqWritePipe = getPipe('tunnelReqWrite', 'TUNNEL_REQ_WRITE');
pluginMgr.getTunnelResReadPipe = getPipe('tunnelResRead', 'TUNNEL_RES_READ');
pluginMgr.getTunnelResWritePipe = getPipe('tunnelResWrite', 'TUNNEL_RES_WRITE');

...
pluginMgr.loadCert = function(req, plugin, callback) {
  loadPlugin(plugin, function(err, ports) {
    ...
    if (req.useSNI) {
      delete options.headers['x-whistle-non-sni-req'];
    } else {
      options.headers['x-whistle-non-sni-req'] = '1';
    }
    if (req.sniRuleValue) {
...
function getRulesFromPlugins(type, req, res, callback) {
  ...
  loadPlugins(plugins, function(ports) {
    ...
    var enableAuth = !isResRules && req.justAuth !== false && (!req.fromTunnel || !util.isAuthCapture(req));

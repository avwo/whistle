...
function getRules(req, port, callback) {
    ...
    var localHost = rulesMgr.resolveLocalHost(req.fullUrl);
    if (nextRule) {
    	options.headers[NEXT_RULE_HEADER] = encodeURIComponent(nextRule.matcher);
    }
    if (localHost) {
    	options.headers[LOCAL_HOST_HEADER] = encodeURIComponent(localHost);
    }
...
pluginMgr.getRules = function(req, callback) {
	var plugin = req.pluginMgr = getPluginByRuleUrl(comUtil.rule.getUrl(req.rules.rule));
	if (!plugin) {
		return callback();
	}

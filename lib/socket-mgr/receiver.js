      ...
      this.messageHandlers.push(function(callback) {
        self.applyExtensions(packet, state.lastFragment, state.compressed, function(err, buffer) {
          ...
          if (state.lastFragment) {
            ...
            self.currentMessage = [];
            self.currentMessageLength = 0;
            self.onData(messageBuffer, {length: length, compressed: state.compressed, opcode: 1});
          }
          callback();
        });
      });
      ...
      this.messageHandlers.push(function(callback) {
        self.applyExtensions(packet, state.lastFragment, state.compressed, function(err, buffer) {
          ...
          if (state.lastFragment) {
            ...
            self.currentMessage = [];
            self.currentMessageLength = 0;
            self.onData(messageBuffer, {length: length, compressed: state.compressed, opcode: 2});
          }
          callback();
        });
      });

...
function createCertificate(hostname) {
  ...
  cert = createCert(pki.setRsaPublicKey(ROOT_KEY.n, ROOT_KEY.e), serialNumber, 2);
...
function createCert(publicKey, serialNumber, expiryPeriod) {
  ...
  cert.validity.notAfter.setFullYear(curYear + (expiryPeriod || 10));
  return cert;
}

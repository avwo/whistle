...
function getCertificate(hostname) {
  return customPairs[hostname] || wildcardPairs[hostname] || cachePairs[hostname];
}
...
function createCertificate(hostname) {
  ...
  // http://openssl.6102.n7.nabble.com/openssl-org-4301-BUG-OpenSSL-1-1-0-pre2-fails-to-parse-x509-certificate-in-DER-format-td63467.html
  var serialNumber = crypto.createHash('sha1')
    .update(hostname + RANDOM_SERIAL, 'binary').digest('hex');
  var leading = serialNumber.substring(0, 2).toLowerCase();
  if (leading === '00' || leading === 'ff') {
    serialNumber = '10' + serialNumber.substring(4);
  }

  ...
  keys.forEach(function(filename) {
    ...
    altNames.forEach(function(item) {
      if ((item.type === 2 || item.type === 7) && !pairs[item.value]) {
        ...
        certsInfo[item.value] = extend({ filename: filename, mtime: mtime, domain: item.value }, validity);
      }
    });
    if (dnsName.length) {
      certFiles[filename] = extend({ mtime: mtime, dir: cert.dir, dnsName: dnsName.join(', ') }, validity);
    }
  });
  customPairs = pairs;
...
function createRootCA() {
  ...
  if (ROOT_KEY && ROOT_CRT && ROOT_KEY.length && ROOT_CRT.length) {
    ...
    try {
      ...
      var dnsName = [];
      altNames.forEach(function(item) {
        if ((item.type === 2 || item.type === 7) && dnsName.indexOf(item.value) === -1) {
          dnsName.push(item.value);
        }
      });
...
function writeFile(filename, ctn) {
  fs.writeFile(filename, ctn, function(err) {
    err && fs.writeFile(filename, ctn, util.noop);
  });
}
// 异步重试，出错重试即可
...
// 异步写入，出错重试即可
function writeCertFile(filename, cert) {
  writeFile(path.join(CUSTOM_CERTS_DIR, filename + '.key'), cert.key);
  writeFile(path.join(CUSTOM_CERTS_DIR, filename + '.crt'), cert.cert);
}

var ILLEGAL_PATH_RE = /[/\\]/;

function checkFilename(name) {
  return name && !ILLEGAL_PATH_RE.test(name) && name.length <= 128;
}
  ...
  Object.keys(certs).forEach(function(filename) {
    ...
    if (cert && util.isString(cert.key) && util.isString(cert.cert)) {
      try {
        ...
        if (cert) {
          writeCertFile(filename, cert.cert);

...
var USE_NEW_RSA = parseInt(CUR_VERSION.slice(1), 10) >= 6;
var enableATS = USE_NEW_RSA && config.ATS;
...
var useNewKey = enableATS || (fs.existsSync(ROOT_NEW_KEY_FILE) && fs.existsSync(ROOT_NEW_CRT_FILE));
...
if (config.ATS) {
  assert(USE_NEW_RSA, 'Enable ATS requires Node >= 6 (current: '
...
function createCACert() {
  var keys = pki.rsa.generateKeyPair(USE_NEW_RSA ? 2048 : 1024);
  var cert = createCert(keys.publicKey);
  var now = Date.now() + '' + Math.floor(Math.random() * 1000);
  var attrs = [ {
    name : 'commonName',
    value : 'whistle.' + now
  }, {

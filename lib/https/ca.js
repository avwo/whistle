...
var HTTPS_DIR = util.mkdir(path.join(util.LOCAL_DATA_PATH, 'certs'));
...
function createCertificateContext(hostname) {
	if (!/^[.\-\w%]+$/.test(hostname)) {
		return DEFAULT_CERT_CTX;
	}
	
	var cert = cache[hostname];
	if (!cert) {
		cert =  cache[hostname] = createSecureContext({
			key: ROOT_KEY,
			cert: _createCertificate(hostname)
		});
	}
...
exports.createCertificateContext = createCertificateContext;

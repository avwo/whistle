...
var wildcardPairs = {};
var cachePairs = {};
var SPECIAL_HOSTNAME = 'com.org.gov.net.mil.edu'.split('.');
...
exports.getDomain = function(hostname) {
  ...
  if (len > 3 || ILEGAL_CHAR_RE.test(prefix)) {
    return wildDomain;
  }
  if (list[2] === 'com' || SPECIAL_HOSTNAME.indexOf(list[1]) === -1) {
    return wildDomain;
  }
  return hostname;
};
...
function createCertificate(hostname) {
  ...
  var serialNumber = crypto.createHash('sha1')
    .update(hostname, 'binary').digest('hex');

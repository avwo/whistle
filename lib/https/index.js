...
function toHttp1(req, res) {
  ...
  if (isH2) {
    var s = req.socket || '';
    s._h2SessionId = s._h2SessionId || util.getReqId();
    headers[config.H2_SESSION_ID_HEADER] = s._h2SessionId;
  ...
  function request(chunk) {
    ...
    if (HTTP_RE.test(headersStr)) {
      ...
      var useNoSNIServer = function() {
        checkTimeout();
        serverAgent.createServer(domain, handlers, handleConnect, config.timeout);
      };

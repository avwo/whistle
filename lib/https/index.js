                ...
                var handleProxy = function(proxySocket) {
                  if (wss) {
                    ...
                    proxySocket.on('error', function(err) {
                      if (util.checkTlsError(err) && util.checkAuto2Http(socket, ip, proxyUrl)) {
                        wss = false;
                        handleProxy(proxySocket);
                      } else {
                        execCallback(err);
                      }
                    });
            ...
            if (retryXHost < 2 && (
              ...
              if (retryXHost > 1) {
                if (isHttpsInternal) {
                  ...
                  options.protocol = origProto === 'wss:';
                }
                socket.curUrl = fullUrl;
        ...
        pluginMgr.getResRules(socket, res, function() {
          util.parseRuleJson([_rules.resHeaders, _rules.resCors, _rules.resCookies],
            function(resHeaders, cors, cookies) {
              reqSocket.headers = headers = res.headers;
              if (resHeaders) {
                resHeaders = util.lowerCaseify(resHeaders, res.rawHeaderNames);
                extend(res.headers, resHeaders);
              }
              var origin = headers['access-control-allow-origin'];
              if (socket._origin && origin && origin !== '*') {
                headers['access-control-allow-origin'] = socket._origin;
              }
              ...
              util.setResponseFor(_rules, headers, socket, socket.hostIp);
...
function toHttp1(req, res) {
  var clientErr, client;
  var handleError = function(e) {
    if (e) {
      client && client.abort();
      client = null;
      clientErr = clientErr || e;
    }
  };
  addReqInfo(req);
  req.stream && req.stream.on('error', handleError);
  res.stream && res.stream.on('error', handleError);
  ...
  client.on('response', function(svrRes) {
    try {
      ...
      res.writeHead(code, getStatusMessage(svrRes), formatRawHeaders(svrRes, req.httpVersion == 2));
      var write = res.write;
...
module.exports = function(socket, hostname, next, isWebPort) {
  ...
  function request(chunk) {
    ...
    if (HTTP_RE.test(headersStr)) {
      ...
      var domain = hostname && getDomain(hostname);
      var recieveData, authTimer;
      var useSNI = (!domain || !serverAgent.existsServer(domain)) && checkSNI(chunk);
      socket.pause();
      ...
      if(useSNI) {
        ...
        getSNIServer(h2Handlers, handleConnect, (disable.h2 || disable.http2) && !enable.h2 && !enable.http2);
      } else {
        useNoSNIServer();
      }
    }

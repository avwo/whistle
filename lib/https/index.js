...
function handleWebsocket(socket, clientIp, callback) {
  ...
  rules.resolveFileRules(socket, function() {
    ...
    var reqSocket, options, pluginRules, matchedUrl, isHttps2HttpProxy, done;
    ...
    pluginMgr.getRules(socket, function(rulesMgr) {
      ...
      rules.getProxy(fullUrl, plugin ? null : socket, function(err, hostIp, hostPort) {
        ...
        if (proxyUrl) {
          ...
          isHttps2HttpProxy = /^https2http-proxy:\/\//.test(proxyUrl);
    ...
    function pipeData() {
      ...
      reqSocket.write(socket.getBuffer((isHttps2HttpProxy || plugin || matchedUrl || _rules.hostname) ? headers : null,

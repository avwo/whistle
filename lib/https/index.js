...
function handleWebsocket(socket, clientIp, callback) {
  ...
  plugin = pluginMgr.resolvePluginRule(socket);
  pluginMgr.getRules(socket, function(rulesMgr) {
    if (pluginRules = rulesMgr) {
      ...
      if (filter.rule) {
        ...
        if (newRules.host) {
          _rules.host = newRules.host;
        }
      }
      if (!filter.pac && newRules.pac) {
        _rules.pac = newRules.pac;
      }
    }


    var proxyUrl = plugin ? null : util.rule.getProxy(_rules.rule);
    var ruleUrlValue = plugin || proxyUrl ? null : util.rule.getUrl(_rules.rule);
    if (ruleUrlValue && /^wss?:\/\//.test(ruleUrlValue) && fullUrl != ruleUrlValue) {
      data.realUrl = fullUrl = matchedUrl = ruleUrlValue;
    }
    rules.findProxyFromPac(fullUrl, plugin || proxyUrl ? null : _rules.pac, function(err, proxyRule) {
      if (proxyRule) {
        proxyUrl = proxyRule;
      }

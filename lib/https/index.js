    ...
    function _pipeData() {
      ...
      util.parseRuleJson(_rules.params, function(params) {
        if (params) {
          ...
          if (fullUrl !== _url) {
            socket.urlChanged = paramsChanged = true;
            data.realUrl = fullUrl = _url;
            options = url.parse(fullUrl);
          }
        }
        reqSocket.write(socket.getBuffer((socket.urlChanged || isInternalProxy || reqHeaders
          || plugin || matchedUrl || _rules.hostname || util.hasHeaderRules(headers)) ? headers : null,
            (paramsChanged || matchedUrl) ? options.path : null));

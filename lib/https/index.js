...
function handleWebsocket(socket, wss, callback) {
	...
	var proxyUrl = util.rule.getMatcher(_rules.rule);
	if (proxyUrl && !util.isProxyProtocol(url.parse(proxyUrl).protocol)) {
		proxyUrl = null;
	}
	...
	function resolveHost(url, callback) {
		rules.resolveHost(url, function(err, ip, customHost) {
			if (err) {

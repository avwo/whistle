...
function getRequestEmitter(socket, wss) {
	...
	reqEmitter.ip = socket.remoteAddress; //对tls的请求，为兼容node v0.10.x只能通过转发，导致获取的请求ip都是本地ip
	...
	if (proxyUrl) {
		...
		rules.resolveHost(proxyUrl, function(err, ip, customHost) {
			...
			connect(socket, function(err, proxySocket) {
				...
				if (wss) {
					d.run(function() {//TODO 看下具体原因，为什么会抛出异常
...
exports.dispatch = function dispatch(socket, hostname) {
	...
	function request(chunk) {
		...
		if (/upgrade\s*:\s*websocket/i.test(buffer && buffer.toString())) { //ws
			...
			cert.createCert(hostname, function(err) {
				d.run(function() {//TODO 看下具体原因，为什么会抛出异常
					reqSocket = net.connect(config.tlsport, LOCALHOST, function() {
...
exports.createServer = function createServer(_app) {
	app = _app;
	config = app.config;
	d.run(function() {
		var server = tls.createServer({
			SNICallback: function(hostname, callback) {
				if (typeof callback != 'function') {
					return cert.getCert(hostname);
				}
				
				callback(null, cert.getCert(hostname));
			},
			key: DEFAULT_CERT[0],
			cert: DEFAULT_CERT[1]
		}, handleTlsSocket)
			.listen(config.tlsport, 'localhost')
				.on('error', util.noop)
					.on('clientError', util.noop);
	});

...
function resolveWebsocket(socket, wss) {
  ...
  rules.resolveRulesFile(socket, function() {
    ...
    var reqSocket, options, pluginRules, isInternalProxy, origProto, done, proxyUrl;
        ...
        pluginMgr.loadPlugin(plugin, function(err, ports) {
          ...
          rules.getProxy(fullUrl, socket, function(err, hostIp, hostPort) {
            ...
            if (proxyUrl) {
              ...
              var isHttpsProxy = proxyRule.isHttps || proxyRule.isHttpsInternal;
    ...
    function pipeData() {
      ...
      clearTimeout(timeout);
      proxyUrl ? util.setClientId(headers, socket.enable, socket.disable, clientIp, isInternalProxy) : util.removeClientId(headers);
...
module.exports = function(socket, next, isWebPort) {
  ...
  function request(chunk) {
    ...
    if (HTTP_RE.test(headersStr)) {
      ...
      if(useSNI) {
        checkTimeout();
        getSNIServer(h2Handlers, handleConnect, !properties.isEnableHttp2() || util.isDisableH2(socket, true));
      } else {
        useNoSNIServer();
      }
    }

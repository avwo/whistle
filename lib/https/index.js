...
var createHttpsServer = ca.createHttpsServer;
...
function resolveWebsocket(socket, wss) {
  ...
  rules.resolveRulesFile(socket, function() {
    ...
    pluginMgr.getRules(socket, function(rulesMgr) {
      ...
      if (!filter.hide || socket.disable.hide) {
        data.abort = destroy;
        ...
        rules.getProxy(fullUrl, plugin ? null : socket, function(err, hostIp, hostPort) {
          ...
          if (proxyUrl) {
            ...
            getServerIp(proxyUrl, function(ip) {
              ...
              var isProxyPort = proxyOptions.port == config.port || proxyOptions.port == config.sslport;
              if (isProxyPort && util.isLocalAddress(ip)) {
                return execCallback(new Error('Self loop (' + ip + ':' + proxyOptions.port + ')'));
              }
    ...
    function connectServer(hostIp, hostPort) {
      getServerIp(fullUrl, function(ip, port) {
        ...
        if (port == config.sslport && util.isLocalAddress(ip)) {
...
module.exports.createHttpsServer = function(port, host, callback) {
  return createHttpsServer(port, host, handlers, callback);
};

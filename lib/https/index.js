...
function handleWebsocket(socket, clientIp, callback) {
  ...
  rules.resolveFileRules(socket, function() {
    ...
    var pluginHomePage = plugin = pluginMgr.getPluginByPath(socket);
    var timeout = util.setTimeout(function() {
      destroy(new Error('Timeout'));
    });

    if (pluginHomePage) {
      matchedUrl = fullUrl = socket.fullUrl;
    } else {
      pluginHomePage = plugin = pluginMgr.getPluginByHomePage(fullUrl);
    }
    ...
    function connect() {
      ...
      if (plugin) {
        pluginMgr.loadPlugin(plugin, function(err, ports) {
          ...
          if (localHost) {
            headers[pluginMgr.LOCAL_HOST_HEADER] = encodeURIComponent(util.removeProtocol(localHost.matcher, true) + (localHost.port ? ':' + localHost.port : ''));
          }
    ...
    function resolveHost(url, callback) {
      ...
      rules.resolveHost(url, function(err, ip, port, host) {
        ...
        callback(ip, port);
      }, pluginRules, socket.rulesFileMgr);
    }

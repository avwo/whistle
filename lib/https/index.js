...
var existsSNIServer = httpsUtil.existsSNIServer;
...
var TIMEOUT = 3600;
var FIRST_LOCAL_TIMEOUT = 1200;
var LOCAL_TIMEOUT = 600;
var proxy;
...
var HTTP_RE = /^\w+\s+[^\s]+\s+HTTP\/1.1(?:\r?\n|\r)/;
var CRLF = util.toBuffer('\r\n');

module.exports = function(socket, hostname, callback, next) {
  ...
  timer = setTimeout(request, util.isLocalAddress(clientIp) ? LOCAL_TIMEOUT : TIMEOUT);
  function request(chunk) {
    ...
    if (/upgrade\s*:\s*websocket/i.test(headersStr)) { //ws
      ...
      var existsAgentServer = serverAgent.existsServer(domain);
      var useSNI = existsAgentServer || checkSNI(chunk);
      var checkTimeout = function() {
        var timeout = LOCAL_TIMEOUT;
        if (useSNI ? !existsSNIServer() : !existsAgentServer) {
          timeout = FIRST_LOCAL_TIMEOUT;
        }
      ...
      var handleConnect = function(port) {
        reqSocket = net.connect(port, LOCALHOST, function() {
          ...
          recieveData = function(data) {
            reqSocket.removeListener('data', recieveData);
            clearTimeout(authTimer);
            authTimer = null;
            socket.write(data);
            reqSocket.pipe(socket).pipe(reqSocket);
          };
          reqSocket.on('data', recieveData);
          reqSocket.write(chunk);
        });
        abortIfUnavailable(reqSocket);
        callback(reqSocket);

                    ...
                    if (isInternalProxy && !socket._phost) {
                      ...
                      if (isSocks) {
                        ...
                        if (!util.isLocalAddress(clientIp)) {
                          proxyHeaders[config.CLIENT_IP_HEADER] = clientIp;
                        }
                        if (isProxyPort) {
  ...
  function pipeData() {
    ...
    if (disable.clientIp || disable.clientIP) {
      ...
      if (net.isIP(forwardedFor)) {
        headers[config.CLIENT_IP_HEADER] = forwardedFor;
      } else if (net.isIP(socket._customXFF)) {
        headers[config.CLIENT_IP_HEADER] = socket._customXFF;
      } else if (
        (!isInternalProxy &&
          !plugin &&
          !socket.enableXFF &&
    ...
    util.parseRuleJson(
      ...
      function (
        ...
        if (reqHeaders) {
          ...
          socket._customXFF = reqHeaders[config.CLIENT_IP_HEADER];
          socket._customClientId = reqHeaders[config.CLIENT_ID_HEADER];
          delete reqHeaders[config.CLIENT_IP_HEADER];
...
function addReqInfo(req) {
  ...
  if (remoteData) {
    ...
    headers[util.CLIENT_INFO_HEADER] =

  ...
  function setupSocket(cb) {
    ...
    util.parseRuleJson(
      ...
      function (
        ...
        newUrl = util.deleteQuery(newUrl, util.parseDelQuery(socket));
        if (newUrl !== fullUrl) {
          fullUrl = newUrl;
...
module.exports = function (socket, next, isWebPort) {
  ...
  util.readOneChunk(
    ...
    function (chunk) {
      ...
      if (isHttp) {
        if ((enable.forHttps && !disable.forHttps) || (disable.captureHttp && !enable.captureHttp)) {
          next(chunk);
        } else {
          socket.resume();
      ...
      } else if ((enable.forHttp && !disable.forHttp) || (disable.captureHttps && !enable.captureHttps)) {
        return next(chunk);
      } else {

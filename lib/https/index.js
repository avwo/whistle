...
function resolveWebsocket(socket, wss) {
  ...
  var reqSocket, options, pluginRules, isXProxy, isInternalProxy, isHInternal, isHttpsInternal;
  ...
  pluginMgr.getRules(socket, function(rulesMgr) {
    ...
    if (!socket.isPluginReq && !socket.isInternalUrl) {
...
function addReqInfo(req) {
  ...
  if (remoteData) {
    ...
    if (remoteData.certSource) {
      headers[config.CERT_SOURCE_HEADER] = remoteData.certSource;
    }
  ...
  util.readOneChunk(socket, function(chunk) {
    ...
    if (HTTP_RE.test(headersStr)) {
      ...
      if (useSNI && !socket.isLocalUIUrl) {
        socket.curUrl = 'https://' + useSNI;
        plugin = rules.resolveSNICallback(socket);
        if (plugin) {
          if (socket.rules) {
            socket.rules.sniCallback = plugin;
          }
          plugin = util.getMatcherValue(plugin);
          plugin = plugin && pluginMgr.getPlugin(plugin + ':');
        }
      }
      if (plugin) {
        var curCert = remoteCerts.get(useSNI);
        socket.serverName = socket.servername = useSNI;
        socket.commonName = domain;
        if (curCert && curCert.name) {
          socket.hasCertCache = curCert.name;
        }

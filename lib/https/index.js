...
function handleConnect(socket, reqSocket) {
...
function resolveWebsocket(socket, wss) {
  socket.isWebSocket = util.isWebSocket(socket.headers) || socket.enable.websocket;
  ...
  var handleResponse = function () {
    ...
    getResRules(socket, resData, function () {
      ...
      if (isSuccess) {
        ...
        var upgrade = getTransProto(headers.upgrade);
        if (key) {
        ...
        resHeaders.Upgrade = upgrade || 'websocket';

...
function parseReqHeaders(reqHeaders) {
  ...
  try {
    reqHeaders = util.removeProtocol(reqHeaders, true);
    return reqHeaders && util.parseJSON(reqHeaders);
  } catch(e) {}
}

function handleWebsocket(socket, clientIp, clientPort, callback) {
  ...
  rules.resolveFileRules(socket, function() {
    ...
    function pipeData() {
      ...
      //TODO: whistle v2.0将全面支持对websocket的操作
      var ruleKey;
      var ruleValue;
      var reqHeaders = parseReqHeaders(_rules.reqHeaders);
      if (reqHeaders) {
        reqHeaders = util.lowerCaseify(reqHeaders);
        ruleKey = reqHeaders['x-whistle-rule-key'];
        if (ruleKey) {
          headers['x-whistle-rule-key'] = ruleKey;
        }
        ruleValue = reqHeaders['x-whistle-rule-value'];
        if (ruleValue) {
          headers['x-whistle-rule-value'] = ruleValue;
        }
      }

      reqSocket.write(socket.getBuffer((isInternalProxy || ruleKey || ruleValue
        || plugin || matchedUrl || _rules.hostname) ? headers : null,
  ...
  function request(chunk) {
    ...
    if (/upgrade\s*:\s*websocket/i.test(chunk.toString())) { //ws
      util.parseReq(socket, function(err, socket) {
        ...
        handleWebsocket(socket, false, false, function(err, req) {
          if (err) {
            return destroy(err);
          }

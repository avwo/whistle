...
function resolveWebsocket(socket, wss) {
  ...
  rules.resolveRulesFile(socket, function() {
    ...
    var reqSocket, options, pluginRules, isInternalProxy, done, proxyUrl;
    ...
    pluginMgr.getRules(socket, function(rulesMgr) {
      ...
      if (!socket.isPluginReq && (!filter.hide || socket.disable.hide)) {
        data.abort = destroy;
          ...
          rules.getProxy(fullUrl, socket, function(err, hostIp, hostPort) {
            ...
            if (proxyUrl) {
              ...
              getServerIp(proxyUrl, function(ip) {
                ...
                var proxyPort = proxyOptions.port = parseInt(proxyOptions.port, 10) || (isSocks ? 1080 : (isHttpsProxy ? 443 : 80));
    ...
    function setupSocket(cb) {
      ...
      var list = [_rules.reqHeaders, cors, _rules.reqCookies, _rules.params, _rules.urlReplace];
      util.parseRuleJson(list, function(reqHeaders, _reqCors, reqCookies, params, urlReplace) {
...
module.exports = function(socket, hostname, next, isWebPort) {
  ...
  function request(chunk) {
    ...
    if (chunk && !isConnect && CONNECT_RE.test(headersStr)) {
      isConnect = true;
      tunnelHost = RegExp.$1;
      hostname = util.parseHost(tunnelHost)[0];
      timer = timeout && setTimeout(request, timeout);

...
function resolveWebsocket(socket, clientIp, clientPort, callback, wss) {
  ...
  util.parseRuleJson(socket.rules.urlParams, function(urlParams) {
    ...
    handleWebsocket(socket, clientIp, clientPort, callback, wss);
  });
}
...
function handleWebsocket(socket, clientIp, clientPort, callback, wss) {
  ...
  rules.resolveRulesFile(socket, function() {
        ...
        if (retryConnect) {
          ...
          retryConnect = function(e) {
            if (retryXHost < 2 && _rules.host && X_RE.test(_rules.host.matcher)) {
              ...
              retryConnect = false;
              if (retryXHost > 1) {
                rules.lookupHost(fullUrl, function(err, _ip) {
    ...
    function _pipeData() {
      ...
      util.parseRuleJson([_rules.reqHeaders, _rules.reqCors, _rules.reqCookies, _rules.params, _rules.urlReplace],
      function(reqHeaders, reqCors, reqCookies, params, urlReplace) {
...
function handleTlsSocket(socket) {
  ...
  parseReq(socket, function(err, socket) {
    ...
    var headers = socket.headers;
    setRuleHeaders(headers, remoteData.rulesHeaders, remoteData.tunnelHost);
    if (headers.upgrade && headers.upgrade.toLowerCase() == 'websocket') {
      resolveWebsocket(socket, clientIp, clientPort, function(err, req) {
        if (err) {
          return destroy(err);
        }
        reqSocket = req;
...
function setRuleHeaders(headers, rulesHeaders, tunnelHost) {
  try {
    headers[TUNNEL_KEY] = encodeURIComponent(JSON.stringify({
  ...
  function request(chunk) {
    ...
    if (/upgrade\s*:\s*websocket/i.test(headersStr)) { //ws
      parseReq(socket, function(err, socket) {
        ...
        setRuleHeaders(socket.headers, rulesHeaders, tunnelHost);
        resolveWebsocket(socket, clientIp, socket.remotePort, function(err, req) {
          if (err) {
            return destroy(err);
          }
...
module.exports.handleWebsocket = resolveWebsocket;

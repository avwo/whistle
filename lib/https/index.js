    ...
    pluginMgr.getResRules(socket, res, function() {
      util.parseRuleJson([_rules.resHeaders, _rules.resCors, _rules.resCookies],
                function(resHeaders, cors, cookies) {
                  ...
                  curStatus = util.getMatcherValue(_rules.replaceStatus) || util.getMatcherValue(_rules.statusCode);
                  callback();
                });
    });
  }
  ...
  function _pipeData() {
    parseReq(reqSocket, function(err, res) {
      ...
      getResRules(socket, res, function() {
        ...
        if (code == 101) {
          socket.write(res.getHeaders(curResHeaders, curStatus));
          ...
          socket.write(res.getBuffer(curResHeaders, curStatus));
          reqSocket.resume();

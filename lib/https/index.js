...
function handleWebsocket(socket, clientIp, dataId, callback) {
  ...
  rules.resolveFileRules(socket, function() {
    ...
    var pluginHomePage = plugin = pluginMgr.getPluginByHomePage(fullUrl);
    ...
    plugin = pluginMgr.resolveWhistlePlugins(socket);
    pluginMgr.getRules(socket, function(rulesMgr) {
      ...
      rules.getProxy(fullUrl, plugin ? null : socket, function() {
      ...
      if (plugin) {
        pluginMgr.loadPlugin(plugin, function(err, ports) {
          ...
          if (pluginHomePage && !ports.uiPort) {
            return execCallback(new Error('Not implemented'));
          }
          ...
          if (!options.port) {
            return execCallback(new Error(pluginHomePage ? 'Not implemented' : 'No plugin.server'));
          }
...
function handleTlsSocket(socket, hostname) {
  ...
  util.parseReq(socket, function(err, socket) {
    ...
    //wss
    var tunnelData = tunnelTmplData[socket.remotePort] || {};
    var clientIp = tunnelData.clientIp || LOCALHOST;
    var data = tunnelData.data || {};
    var headers = socket.headers;
    if (headers.upgrade && headers.upgrade.toLowerCase() == 'websocket') {
      handleWebsocket(socket, clientIp, data.id, function(err, req) {
        if (err) {
          return destroy(err);
        }
        reqSocket = req;
      ...
      reqSocket = net.connect(config.port, LOCALHOST, function() {
        ...
        if (data.id) {
          headers[config.DATA_ID] = data.id;
        }
...
module.exports = function dispatch(socket, hostname, _proxy, data, callback) {
  proxy = _proxy;
  ...
  function request(chunk) {
    ...
    if (/upgrade\s*:\s*websocket/i.test(chunk.toString())) { //ws
      util.parseReq(socket, function(err, socket) {
        ...
        handleWebsocket(socket, false, data.id, function(err, req) {
          if (err) {
            return destroy(err);
          }

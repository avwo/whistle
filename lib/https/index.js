...
function handleWebsocket(socket, clientIp, callback) {
	...
	function resolveHost(url, callback) {
		rules.resolveHost(plugin ? '127.0.0.1' : url, function(err, ip) {
			if (err) {
	...
	if (!pluginHomePage && proxyUrl) {
		...
		if (!matchedUrl) {
			pluginHomePage = pluginMgr.getPluginByHomePage(fullUrl);
			plugin = pluginHomePage || pluginMgr.getPlugin(options.protocol);
		}
		...
		if (plugin) {
			pluginMgr.loadPlugin(plugin, function(err, ports) {
				...
				if (options.protocol == 'wss:') {
					options.protocol == 'ws:';
					headers[pluginMg.HTTPS_FLAG_HEADER] = 'true';
				}
		...
		} else {console.log(_rules.rule && _rules.rule.url);
			connect();
		}
	}

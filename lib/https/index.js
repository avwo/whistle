...
function _handleWebsocket(socket, clientIp, clientPort, callback, wss) {
  ...
  rules.resolveRulesFile(socket, function() {
    ...
    function _pipeData() {
      ...
      util.parseRuleJson([_rules.reqHeaders, _rules.reqCors, _rules.reqCookies,
        _rules.resHeaders, cors, _rules.resCookies, _rules.params],
      function(reqHeaders, reqCors, reqCookies, resHeaders, cors, cookies, params) {
  ...
  function request(chunk) {
    ...
    if (/upgrade\s*:\s*websocket/i.test(chunk.toString())) { //ws
      parseReq(socket, function(err, socket) {
        ...
        var clientIp = util.getClientIp(socket.headers, true) || socket.clientIp;

...
function _handleWebsocket(socket, clientIp, clientPort, callback, wss) {
  ...
  rules.resolveRulesFile(socket, function() {
    ...
    function _pipeData() {
      ...
      util.parseRuleJson([_rules.reqHeaders, _rules.reqCors, _rules.resHeaders, cors, _rules.params],
      function(reqHeaders, reqCors, resHeaders, cors, params) {
        extend(headers, reqHeaders);
        var hasCorsChanged = util.setReqCors({ headers: headers }, reqCors);
        if (params) {
        ...
        var deleteProps = util.parseDeleteProperties(socket);
        var delReqHeaders = deleteProps.reqHeaders;
        var delResHeaders = deleteProps.resHeaders;
        var hasDelReq, hasDelRes;
        ...
        reqSocket.write(socket.getBuffer((_rules.referer || hasCorsChanged || forwardedFor || hasDelReq || socket.urlChanged || isInternalProxy || reqHeaders
          || plugin || matchedUrl || _rules.hostname || util.hasHeaderRules(headers)) ? headers : null,
            (socket.urlChanged || matchedUrl) ? options.path : null));
        parseReq(reqSocket, function(err, res) {
          ...
          hasCorsChanged = util.setResCors({headers: headers}, resCors, cors, socket.headers.origin);
          ...
          socket.write(res.getBuffer((hasCorsChanged || hasDelRes || resHeaders || matchedUrl) ? headers : null));

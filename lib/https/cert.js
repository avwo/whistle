...
var SUBJ = '/C=CN/ST=ZJ/L=HZ/O=webDebuger/OU=proxyTools/CN=whistleProxy/emailAddress=avenwu@vip.qq.com';
...
function _createCert(hostname, callback) {
	...
	execAll(['genrsa -passout pass:whistle -out ' + key + ' 2048',
	         ...
	         'req -new -key ' + key + ' -out ' + csr + ' -passin pass:whistle -subj ' + SUBJ.replace('whistleProxy', hostname),
...
function exec(args, callback) {
	  ...
	  var err;
	  var openssl = spawn('openssl', args, CWD); //TODO 加个超时

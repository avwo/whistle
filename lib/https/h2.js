...
function closeClose(client, name) {
  try {
    client.close();
  } catch (e) {}
  delete clients[name];
...
setInterval(function () {
  ...
  Object.keys(clients).forEach(function (name) {
    ...
    if (isClosed(client) || now - client._updateTime > CACHE_TIMEOUT) {
      closeClose(client, name);
    }
  });
}, INTERVAL);

function getOrigin(req) {
  return (req.useHttpH2 ? 'http' : 'https') + '://' + req.headers.host;
}

function getKey(options, req) {
  var proxyOpts = options._proxyOptions;
  var proxyType = '';
  if (proxyOpts) {
    var headers = proxyOpts.headers || {};
    proxyType = [
      proxyOpts.proxyType,
      proxyOpts.proxyHost,
      proxyOpts.proxyPort,
...
function getClient(req, socket, name, callback) {
  ...
  var client = http2.connect(getOrigin(req), util.setRejectUnauthorized(req, {
...
function isClosed(client) {
  return client.destroyed || client.closed;
}

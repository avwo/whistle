...
function getKey(options) {
  ...
  var proxyType = '';
  if (proxyOpts) {
    proxyType = proxyOpts._proxyType + '\n' + proxyOpts.headers.host;
  }
  return [options.servername, proxyType, options.host, options.port].join('\n');
}
...
function getProxySocket(options, callback) {
  var proxyOpts = options._proxyOptions;
  proxyOpts._proxyType === 'socks' ?
    getSocksSocket(options, callback) :
      getTunnelSocket(options, callback);
}
...
function getSocket(options, callback) {
  ...
  options._proxyOptions ?
    getProxySocket(options, callback) :
      util.connect({

  ...
  try {
    ...
    h2Session.on('response', function (h2Headers) {
      ...
      if (!util.hasBody(svrRes, req)) {
        var wrapperStream = util.createTransform();
        wrapperStream.statusCode = statusCode;

        var hasReceivedData = false;
        h2Session.on('data', function(chunk) {
          hasReceivedData = true;
          wrapperStream.push(chunk);
        });

        h2Session.on('end', function() {
          if (!hasReceivedData) {
            // 确实没有数据，正常结束

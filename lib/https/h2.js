...
function getProxySocket(options, callback, sendAlpn) {
  var handleConnect = function(err, socket) {
    ...
    if (sendAlpn) {
      opts.ALPNProtocols = SUPPORTED_PROTOS;
      opts.NPNProtocols = SUPPORTED_PROTOS;
    }
    socket = tls.connect(opts, handleCallback);
...
exports.getTlsProxySocket = getProxySocket;
...
function getSocket(options, callback) {
  ...
  options._proxyOptions ?
    getProxySocket(options, handleCallback, true) :
      util.connect({
        servername: options.servername,
        host: options.host,
        port: options.port || 443,

...
proto.createServer = function createServer(hostname, secureConnectionListener, callback) {
	...
	if (!self._removeServer) {
		self._removeServer = self.removeServer.bind(self);
	}
	
	self.freeServer();
	getServer(ca.createCertificate(hostname), secureConnectionListener, 
			function(port, server) {
		server.on('error', self._removeServer);
		setTimeout(function() {
			promise.server = server;
			var timeout = setTimeout(self._removeServer, TIMEOUT);

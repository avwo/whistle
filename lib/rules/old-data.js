...
var ROOT = getDataDir();
var HOSTS = ROOT && path.join(ROOT, 'hosts');
var LIST_PATH = ROOT && path.join(HOSTS, 'list');
var values = ROOT && util.parseJSON(readFileSync(path.join(ROOT, 'values'))) || {};
var register = ROOT && util.parseJSON(readFileSync(path.join(ROOT, 'register'))) || {};
var metaData = ROOT && util.parseJSON(readFileSync(path.join(HOSTS, 'meta'))) || {};
...
function getRootCA() {
	return ROOT && readFileSync(path.join(ROOT, 'certs/root.crt'));
}

function getRootCAKey() {
	return ROOT && readFileSync(path.join(ROOT, 'certs/root.key'));
}

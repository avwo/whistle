...
var TPL_VAR_RE =
  /(\$)?\$\{(\{)?(id|reqId|whistle|env|now|random|randomUUID|host|port|realPort|realHost|version|url|hostname|query|search|queryString|searchString|path|pathname|clientId|localClientId|ip|clientIp|clientPort|remoteAddress|remotePort|serverIp|serverPort|method|status(?:Code)|reqCookies?|resCookies?|re[qs]H(?:eaders?)?)(?:\.([^{}]+))?\}(\})?/gi;
...
function getValue(url, start, end) {
...
function resolveValue(rule, vals, req) {
  ...
  if (value !== false || (value = getValue(matcher)) !== false) {
    rule.value = setProtocol(protocol + value, req.curUrl);
  ...
  } else if ((value = getValue(matcher, '<', '>')) !== false) {
...
function parse(rulesMgr, text, root, append) {
  ...
  if (Array.isArray(text)) {
    text.forEach(function (item) {
      item && parseText(rulesMgr, item.text, item.root, item.file);
    });
  } else {
...
function parseFirst(rulesMgr, item) {
  var firstItem = Array.isArray(item.text) && item.text[0];
  parse(rulesMgr, firstItem && firstItem.file ? item.text : trimInlineValues(item.text), item.root);
}
...
function getPluginName(root) {
  return root && ROOT_PLUGIN_RE.test(root) ? 'Plugin: ' + RegExp.$1 : undefined;
}

function parseText(rulesMgr, text, root, file) {
  var pluginVars = rulesMgr._globalPluginVars;
  var pluginName = getPluginName(root);
  ...
  getLines(text, root).forEach(function (line) {
    ...
    if (len === 1 && PLUGIN_VAR_RE.test(line[0])) {
      ...
      enabledList.push([rawLine, file || pluginName]);
      return;
    }
    if (!len || len < 2) {
      return;
...
function resolveInlineValuesFn(item) {
  item.text = util.resolveInlineValues(item.text, inlineValues);
  return item;
}
...
function trimInlineValues(text) {
  return Array.isArray(text)
    ...
    : util.resolveInlineValues(text, inlineValues);
}
...
proto.clearAppend = function () {
  ...
  if (this._rawText && this._rawText[0].first) {
    ...
    !this.disabled && parseFirst(this, item);
    this._rawText = [item];
    this._values = extend({}, this._orgValues, inlineValues);
    inlineValues = {};
  } else {
...
function getHostFromList(list, req, vals) {
  ...
  for (var i = 0, len = list.length; i < len; i++) {
    var mgr = list[i];
    var _host = mgr && mgr.getRule(req, mgr._rules.host, vals[i] || mgr._values);
    if (_host) {

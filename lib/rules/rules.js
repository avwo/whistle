...
var PROPS_FILTER_RE = /^(?:filter|excludeFilter|includeFilter|ignore):\/\/(m(?:ethod)?|i(?:p)?|h(?:eader)?|s(?:tatusCode)?|from|b(?:ody)?|clientIp|clientIP|clientPort|serverIp|serverIP|serverPort|re[qs](?:H(?:eaders?)?)?):(.+)$/;
var PURE_FILTER_RE = /^(?:excludeFilter|includeFilter):\/\/(statusCode|from|clientIp|clientIP|clientPort|serverIp|serverIP|serverPort|re[qs](?:H(?:eaders?)?)?)[.=](.+)$/;
  ...
  for (var i = 0; rule = list[i]; i++) {
    ...
    if (rule.isRegExp) {
      ...
      if ((not ? !matchedRes : matchedRes) && checkFilter() && --index < 0) {
        ...
        (rule.isDomain && domainUrl.indexOf(pattern) === 0)) && checkFilter()) {
...
function matchFilter(url, filter, req) {
  ...
  if (filter.from) {
    ...
    if (filter.from === 'composer' && req.fromComposer) {
...
proto.getHost = function(req, pluginRulesMgr, rulesFileMgr, headerRulesMgr) {
  ...
  var host;
  if (config.multiEnv) {
    host = (pluginRulesMgr && getRule(req, pluginRulesMgr._rules.host, pluginRulesMgr._values))

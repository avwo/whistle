...
var PROPS_FILTER_RE = /^(?:filter|excludeFilter|includeFilter|ignore):\/\/(m(?:ethod)?|i(?:p)?|h(?:eader)?|s(?:tatusCode)?|b(?:ody)|reqBody|resBody|clientIp|clientIP|serverIp|serverIP|req(?:H(?:eaders?)?)?|res(?:H(?:eaders?)?)?):(.+)$/;
...
var TPL_VAR_RE = /(\$)?\$\{(id|reqId|whistle|now|port|version|url|query|search|path|pathname|ip|clientIp|serverIp|method|status(?:Code)|reqCookies?|resCookies?|reqHeaders?|resHeaders?)(?:\.([^{}]+))?\}/ig;
...
function resolveRuleValue(req, key) {
  ...
  for (var i = 0, len = list.length; i < len; i++) {
    plugin = list[i];
    if (!plugin.matcher.indexOf(key)) {
      return plugin.matcher.substring(key.length);
    }
  }
  return '';
}
...
function resolveRuleList(req, list, vals, index) {
  ...
  for (var i = 0; rule = list[i]; i++) {
    ...
    if (rule.isRegExp) {
      ...
      if (matchedRes && !matchFilters(curUrl, rule, req) && --index < 0) {
        regExp['0'] = curUrl;
      ...
      if (hostname != null && !matchFilters(curUrl, rule, req)) {
...
function parseRule(rulesMgr, pattern, matcher, raw, root, options) {
  ...
  if (options.hasReqBodyFilter) {
    rules._reqBodyFilters.push(rule);
  }
  if (options.hasResBodyFilter) {
    rules._resBodyFilters.push(rule);
  }
...
function resolveFilterPattern(matcher) {
  var not, filter, caseIns, wildcard;
...
function resolveMatchFilter(list) {
  var matchers = [];
  var filters, filter, not, hasReqBodyFilter, hasResBodyFilter;
  list.forEach(function(matcher) {
    if (PROPS_FILTER_RE.test(matcher)) {
      ...
      if (isIp || isClientIp || isServerIp) {
        ...
        || propName === 'reqBody' || propName === 'resBody') {
        if (propName[0] === 'b') {
          hasReqBodyFilter = true;
          hasResBodyFilter = true;
          propName = 'body';
        } else if (propName === 'reqBody') {
          hasReqBodyFilter = true;
        } else {
          hasResBodyFilter = true;
        }
        propName = pattern ? propName + 'Pattern' : propName;
        value = pattern || value.toLowerCase();
      } else {
  ...
  return {
    hasReqBodyFilter: hasReqBodyFilter,
    hasResBodyFilter: hasResBodyFilter,
    matchers: matchers,
    filters: filters
  };
}
  ...
  getLines(text, root).forEach(function(line) {
    ...
    if (patternIndex > 0) {
      ...
      matchers.forEach(function(matcher) {
        parseRule(rulesMgr, matcher, pattern, raw, root, result);
      });
    } else {
...
function matchFilters(url, rule, options) {
  ...
  if (filters) {
    for (var i = 0, len = filters.length; i < len; i++) {
      if (matchFilter(url, filters[i], options)) {
        return true;
      }
    }
  }
...
proto.resolveReqBodyFilter = function(req) {
  return getRule(req, this._rules._reqBodyFilters);
};
proto.resolveResBodyFilter = function(req) {
  return getRule(req, this._rules._resBodyFilters);
};

...
var METHOD_FILTER_RE = /^(?:filter|ignore):\/\/(!)?([A-Z]+[A-Z\d_-]*)$/;
...
function resolveRuleList(url, list, vals, index) {
  ...
  for (var i = 0; rule = list[i]; i++) {
    ...
    if (rule.isRegExp) {
      ...
      if ((not ? !matchedRes : matchedRes) && !matchFilters(url, rule, method) && --index < 0) {
        var regExp = {};
        if (!not) {
          for (var j = 1; j < 10; j++) {
            regExp['$' + j] = RegExp['$' + j];
      ...
      if (wildcard.preMatch.test(url) && !matchFilters(url, rule, method)) {
      ...
      if ((not ? !matchedRes : matchedRes) && !matchFilters(url, rule, method) && --index < 0) {
        ...
        (rule.isDomain && domainUrl.indexOf(pattern) === 0)) && !matchFilters(url, rule, method)) {
...
function resolveMatchFilter(list) {
  ...
  list.forEach(function(matcher) {
    if (METHOD_FILTER_RE.test(matcher)) {
      filters = filters || [];
      return filters.push({
        not: RegExp.$1,
        method: RegExp.$2
...
function matchFilter(url, filter, method) {
  var result;
  if (filter.pattern) {
...
function matchFilters(url, rule, method) {
  ...
  // refine perfermence
  if (matchFilter(url, filter, method)) {
    return true;
  }
  filter = filters[1];
  if (!filter) {

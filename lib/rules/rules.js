...
function isRegUrl(url) {
  ...
  var caseInsensitive;
  if (hasStartSymbol) {
    caseInsensitive = RegExp.$1.length;
    url = url.substring(caseInsensitive);
    hasEndSymbol = /\$$/.test(url);
    if (hasEndSymbol) {
      url = url.slice(0, -1);
    }
  }
  if (REG_URL_RE.test(url)) {
    return false;
  }
  ...
  var query = pathname.indexOf('?');
  query = query === -1 ? '' : pathname.substring(query);
  ...
  if (protocol && webProtocols.indexOf(protocol) === -1) {
    ...
    if (protocol === '//') {
      ...
      result = new RegExp('(?:^|/)' + protocol).test(webProtocolString);
    }
  }
  if (result) {
    if (!protocol) {
    ...
    if (domain) {
      domain = domain.replace(/\*+/g, domainToRegExp);
    } else {
      domain = '[^/.]*';
    }
    pathname = escapeRegExp(pathname).replace(/\*+/g, pathToRegExp);
    query = escapeRegExp(query).replace(/\*+/g, '(.*)');
    try {
      var reg = '^' + protocol + domain + pathname + query + (hasEndSymbol ? '$' : '');
      regUrlCache[oriUrl] = {
        not: not,
        reg: new RegExp(reg, caseInsensitive ? 'i' : '')
      };
    } catch (e) {}
  }
  return result;
...
function indexOfPattern(list) {
  ...
  for (var i = 0, len = list.length; i < len; i++) {
    var item = list[i];
    if (isExactPattern(item) || isRegUrl(item) || isNegativePattern(item) ||

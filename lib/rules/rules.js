...
proto.resolveHost = function(_url, callback, pluginRules) {
	...
	if (!filterHost) {
		var host = getRule(_url, this._rules.host);
		if (host) {
			return callback(null, util.removeProtocol(host.matcher, true));
		}
		if (pluginRules) {
			return pluginRules.resolveHost(_url, callback);
		}
	}
	lookup(options.hostname, callback, allowDnsCache && !this.resolveDisable(_url).dnsCache);
};

proto.resolveLocalHost = function(_url) {
	_url = formatUrl(util.setProtocol(_url));
	var options = url.parse(_url);
	if (!options.hostname || (!protoMgr.isWebsocketProtocol(options.protocol) 
			&& !protoMgr.isWebProtocol(options.protocol)) || this.resolveFilter(_url).host) {
		return;
	}
	
	var host = getRule(_url, this._rules.host);
	return host && util.removeProtocol(host.matcher, true);
};

proto.resolveFilter = function(url) {
	return resolveProperties(url, this._rules.filter);
};

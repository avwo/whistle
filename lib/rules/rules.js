...
function getRuleList(url, list, vals) {
  return _getRuleList(url, list).map(function(rule) {
...
function _getRule(url, list, index) {
  return _getRuleList(url, list, index || 0);
}

function _getRuleList(url, list, index) {
  url = formatUrl(url);
  //支持域名匹配
  ...
  var matchedUrl, files, matcher, result;
...
proto.resolveFilter = function(url) {
  ...
  extend(filter, resolveProperties(url, this._rules.enable, this._values));
  if (filter.socks) {
    filter.proxy = true;
  }
...
proto.resolveProxy = function resolveProxy(url) {
  var filter = this.resolveFilter(url);
  return filter.proxy ? null : getRule(url, this._rules.proxy, this._values);
};

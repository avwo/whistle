...
function parse(rulesMgr, text, root, append) {
  ...
  if (Array.isArray(text)) {
    text.forEach(function(item) {
      item && parseText(rulesMgr, item.text, item.root, append);
    });
  } else {
    parseText(rulesMgr, text, root, append);
  }
}
...
function parseText(rulesMgr, text, root, append) {
  text = text.replace(COMMENT_RE, '').replace(CONTROL_RE, '');

...
function resolveMatchFilter(list) {
  var matchers = [];
  var filters, filter, not, hasBodyFilter;
  list.forEach(function(matcher) {
    if (PROPS_FILTER_RE.test(matcher)) {
      ...
      if (isIp || isClientIp || isServerIp) {
        ...
        hasBodyFilter = true;
        pattern = util.toRegExp(value);
        propName = pattern ? 'bodyPattern' : 'body';
        value = pattern || value.toLowerCase();
      } else {
...
function matchFilter(url, filter, req) {
  ...
  if (typeof req._reqBody === 'string' && (filter.bodyPattern || filter.body)) {
    result = filter.bodyPattern ? filter.bodyPattern.test(req._reqBody)
    : req._reqBody.toLowerCase().indexOf(filter.body) !== -1;

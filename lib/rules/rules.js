...
function isRegUrl(url) {
  ...
  query = escapeRegExp(query).replace(/\*+/g, queryToRegExp);
...
function parseWildcard(pattern, not) {
  ...
  preMatch = escapeRegExp(preMatch).replace(/[*~]+/g, domainToRegExp);
...
proto.resolveRules = function(url) {
  ...
  protocols.forEach(function(name) {
    if ((name === 'proxy' || name === 'rule' || !filter[name]) && (rule = getRule(url, rules[name], vals))) {
      _rules[name] = rule;
    }
  });

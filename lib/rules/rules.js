...
var PLUGIN_NAME_RE = /^[a-z\d_\-]+(?:\.g?var(?:\$|\d*))?(?:\.replace\(.+\))?$/;
var VAR_INDEX_RE = /^([a-z\d_\-]+)\.(g)?var(\$|\d*)/;
...
function getPluginVar(vars, name, index) {
  vars = vars && vars[name];
  if (!vars) {
    return '';
  }
...
function resolveRuleValue(req, key) {
  ...
  if (curRules) {
    if (VAR_INDEX_RE.test(key)) {
      return getPluginVar(RegExp.$2 ? req._globalPluginVars : req._pluginVars, RegExp.$1, RegExp.$3 || 0);
    }

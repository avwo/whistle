...
function getRule(url, list, vals, index) {
  var rule = resolveRuleList(url, list, vals, index || 0);
  resolveValue(rule, vals);
  return rule;
}

function getRuleList(url, list, vals) {
  return resolveRuleList(url, list, vals).map(function(rule) {
...
var SEP_RE = /^[?/]/;
function resolveRuleList(url, list, vals, index) {
  var options;
  ...
  for (var i = 0; rule = list[i]; i++) {
    ...
    if (rule.isRegExp) {
      ...
      if ((not ? !matchedRes : matchedRes) && !matchFilters(url, rule, options) && --index < 0) {
        var regExp = {};
        if (!not) {
          for (var j = 1; j < 10; j++) {
            regExp['$' + j] = RegExp['$' + j];
      ...
      if (wildcard.preMatch.test(url) && !matchFilters(url, rule, options)) {
      ...
      if ((not ? !matchedRes : matchedRes) && !matchFilters(url, rule, options) && --index < 0) {
        ...
        (rule.isDomain && domainUrl.indexOf(pattern) === 0)) && !matchFilters(url, rule, options)) {
...
function resolveProperties(url, rules, vals) {
  return util.resolveProperties(getRuleList(url, rules, vals));
}
...
function matchFilter(url, filter, options) {
  var result;
  if (filter.pattern) {
    result = filter.pattern.test(url);
  } else if (options) {
    if (filter.method) {
      result = options.method === filter.method;
    } else if (filter.mPattern) {
      result = filter.mPattern.test(options.method);
    }
    if (filter.ip) {
      result = options.ip === filter.ip;
    } else if (filter.iPattern) {
      result = filter.iPattern.test(options.ip);
    }
    var header = filter.header;
    if (header && options.headers) {
      var value = String(options.headers[header.key] || '');
      if (header.hPattern) {
...
proto.lookupHost = function(req, callback) {
  ...
  req.curUrl = curUrl;
  var options = url.parse(curUrl);
  if (options.hostname === '::1') {
    return callback(null, '127.0.0.1');
  }
  lookup(options.hostname, callback, allowDnsCache && !this.resolveDisable(curUrl).dnsCache);
};
...
proto.getHost = function(req, pluginRulesMgr, rulesFileMgr, headerRulesMgr) {
  ...
  var host = (pluginRulesMgr && getRule(curUrl, pluginRulesMgr._rules.host, pluginRulesMgr._values))
  || getRule(curUrl, this._rules.host, vals)
  || (rulesFileMgr && getRule(curUrl, rulesFileMgr._rules.host, vals))
  || (headerRulesMgr && getRule(curUrl, headerRulesMgr._rules.host, vals));
  return host;
};

proto.resolveFilter = function(req) {
  var curUrl = req.curUrl;
  var filter = resolveProperties(curUrl, this._rules.filter, this._values);
  var ignore = resolveProperties(curUrl, this._rules.ignore, this._values);
...
proto.resolveRules = function(req) {
  ...
  protocols.forEach(function(name) {
    if ((name === 'proxy' || name === 'rule' || name === 'plugin'
      || !filter[name]) && (rule = getRule(curUrl, rules[name], vals))) {
      _rules[name] = rule;
    }
  });
  multiMatchs.forEach(function(name) {
    rule = _rules[name];
    if (rule) {
      rule.list = getRuleList(curUrl, rules[name], vals);
    }
  });
...
proto.resolveProxy = function resolveProxy(req) {
  ...
  var proxy = getRule(curUrl, this._rules.proxy, this._values);
  if (proxy) {
...
proto.resolveLocalRule = function(req) {
  return getRule(req.curUrl, this._rules._localRule);
};

...
var VALUE_KEY_RE = /^\{(\S+)\}$/;
var LINE_END_RE = /\n|\r\n|\r/g;
var LOCAL_RULE_RE = /^https?:\/\/local\.(?:whistlejs\.com|wproxy\.org)(:realPort)?(?:\/|\?|$)/;
var PATH_RE = /^<.*>$/;
var VALUE_RE = /^\(.*\)$/;
var REG_URL_RE = /^((?:[\w.*-]+:|\*+)?\/\/)?([^/?]*)/;
...
var TPL_VAR_RE = /(\$)?\$\{(\{)?(id|reqId|whistle|now|port|realPort|version|url|query|search|queryString|searchString|path|pathname|ip|clientIp|serverIp|method|status(?:Code)|reqCookies?|resCookies?|reqHeaders?|resHeaders?)(?:\.([^{}]+))?\}(\})?/ig;
...
function isRegUrl(url, isCheck) {
  ...
  query = pathname + (query ? util.escapeRegExp(query).replace(STAR_RE, queryToRegExp) : '');
...
function resolveTplVar(value, req) {
  return value.replace(TPL_VAR_RE, function(all, escape, lb, name, key, rb) {
    if ((lb && !rb) || (name === 'whistle' && (!key || !PLUGIN_NAME_RE.test(key)))) {
      return all;
    }
...
function resolveRuleList(req, list, vals, index, isFilter) {
  ...
  var checkFilter = function(curUrl, rule, req) {
  ...
  for (var i = 0; rule = list[i]; i++) {
    ...
    if (rule.isRegExp) {
      ...
      if (matchedRes && checkFilter(curUrl, rule, req) && --index < 0) {
        regExp['0'] = curUrl;
      ...
      if (hostname != null && checkFilter(curUrl, rule, req)) {
      ...
      if ((not ? !matchedRes : matchedRes) && checkFilter(curUrl, rule, req) && --index < 0) {
        if (result = getExactRule(getRelativePath(pattern, curUrl, rule.matcher))) {
          return result;
        }
      }
    } else if (((matchedUrl = (curUrl.indexOf(pattern) === 0)) ||
        (rule.isDomain && domainUrl.indexOf(pattern) === 0)) && checkFilter(curUrl, rule, req)) {
...
function parseWildcard(pattern, not) {
  ...
  if (!startWithDot && domain.indexOf('*') === -1 && domain.indexOf('~') === -1) {
    return;
  }
  if (not) {
    return false;
  ...
  if (allowMatchPath) {
    ...
    if (!startWithDot && (domain === '*' || domain === '~') && path.charAt(0) === '/') {
      preMatch += '*';
    }
...
function isPattern(item) {
  return PORT_PATTERN_RE.test(item) || NO_SCHEMA_RE.test(item) || isExactPattern(item) || isRegUrl(item, true) ||
...
function matchExcludeFilters(url, rule, options) {
  ...
  for (var i = 0, len = filters.length; i < len; i++) {
    ...
    if ((filter.isInclude ? !include : !exclude) && matchFilter(url, filter, options)) {
      if (filter.isInclude) {
        include = true;
      } else {
        exclude = true;
...
proto.getHost = function(req, pluginRulesMgr, rulesFileMgr, headerRulesMgr) {
  ...
  || (rulesFileMgr && getRule(req, rulesFileMgr._rules.host, req._scriptValues))
...
proto.resolveBodyFilter = function(req) {
  return req.isPluginReq ? null : getRule(req, req._bodyFilters || this._rules._bodyFilters, null, null, true);
};

...
var PROPS_FILTER_RE = /^(?:filter|ignore):\/\/(m(?:ethod)?|i(?:p)?|h(?:eader)?|s(?:tatusCode)?|clientIp|clientIP|serverIp|serverIP|req(?:H(?:eaders?)?)?|res(?:H(?:eaders?)?)?):(.+)$/;
...
var TPL_VAR_RE = /\$\{(id|reqId|now|url|clientIp|serverIp|method|status(?:Code)|reqCookies?|resCookies?|reqHeaders?|resHeaders?)(?:\.([^{}]+))?\}/ig;
var SEP_RE = /^[?/]/;
var CONTROL_RE = /[\u001e\u001f\u200e\u200f\u200d\u200c\u202a\u202d\u202e\u202c\u206e\u206f\u206b\u206a\u206d\u206c]+/g;
...
function resolveUrlVar(req, key) {
  var url = req.fullUrl;
  if (!key) {
    return url;
  }
  ...
  if (!query) {
    query = req.__query = util.parseQuery(options.query);
  }
  return util.getQueryValue(query[key.substring(6)]);
}
function resolveReqCookiesVar(req, key) {
  ...
  if (!cookies || req.__rawCookies !== cookie) {
    ...
    cookies = req.__cookies = util.parseQuery(cookie, '; ');
  }
  ...
  if (!cookies || req.__rawResCookies !== rawCookie) {
    ...
    cookie.forEach(function(c) {
      ...
      Object.keys(c).forEach(function(key) {
        ...
        case 'domain':
          item.domain = cookie[key];
          break;
        case 'path':
          item.path = cookie[key];
          break;
        case 'expires':
          item.expires = cookie[key];
          break;
        case 'max-age':
          item.maxAge = item['max-age'] = item['Max-Age'] = cookie[key];
          break;
        case 'httponly':
          item.httpOnly = true;
          break;
        case 'secure':
          ...
          break;
        case 'samesite':
          item.samesite = item.sameSite = item.SameSite = cookie[key];
          break;
        default:
          if (!cookies[key]) {
            item.value = cookie[key];
            cookies[key] = item;
          }
        }
      });
...
function resolveTplVar(value, req) {
  return value.replace(TPL_VAR_RE, function(_, name, key) {
    ...
    case 'reqid':
      return req.reqId || '';
    case 'url':
      return resolveUrlVar(req, key);
    case 'reqcookie':
    case 'reqcookies':
      return resolveReqCookiesVar(req, key);
    case 'rescookie':
    case 'rescookies':
...
function resolveVar(rule, vals, req) {
  ...
  return matcher.replace(VAR_RE, function(all, key) {
    ...
    if (rule.isTpl && typeof key === 'string') {
      return key && resolveTplVar(key, req);
    }
    return all;
  });
}

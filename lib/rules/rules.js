...
proto.resolveHost = function(_url, callback, pluginRules, fileRules) {
  ...
  var filterHost = this.resolveFilter(_url).host
    || (pluginRules && pluginRules.resolveFilter(_url).host)
    || (fileRules && fileRules.resolveFilter(_url).host);
  var vals = this._values;
  var host;
  if (!filterHost) {
    host = (pluginRules && getRule(_url, pluginRules._rules.host, vals))
      || getRule(_url, this._rules.host, vals)
      || (fileRules && getRule(_url, fileRules._rules.host, vals));
  }
  if (host) {

...
var TPL_VAR_RE = /(\$)?\$\{(id|reqId|now|port|url|query|ip|clientIp|serverIp|method|status(?:Code)|reqCookies?|resCookies?|reqHeaders?|resHeaders?)(?:\.([^{}]+))?\}/ig;
var SEP_RE = /^[?/]/;
var inlineValues;
var CONTROL_RE = /[\u001e\u001f\u200e\u200f\u200d\u200c\u202a\u202d\u202e\u202c\u206e\u206f\u206b\u206a\u206d\u206c]+/g;
...
function resolveInlineValues(str) {
  str = str && str.trim();
  if (!str || !CHECK_MULTI_LINE_VALUE_RE.test(str)) {
    return str;
  }
...
proto.parse = function(text, root, backRulesFirst) {
  var item = {
    first: true,
    text: text,
    root: root
  ...
  if (backRulesFirst && CHECK_MULTI_LINE_VALUE_RE.test(text)) {
    text = trimInlineValues(util.reverseText(text));
    parse(this, util.reverseText(text), root);
  } else {

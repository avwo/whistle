...
var TPL_VAR_RE = /(\$)?\$\{(\{)?(id|reqId|whistle|now|port|realPort|version|url|hostname|query|search|queryString|searchString|path|pathname|ip|clientIp|serverIp|method|status(?:Code)|reqCookies?|resCookies?|re[qs]H(?:eaders?)?)(?:\.([^{}]+))?\}(\})?/ig;
...
function parseRule(rulesMgr, pattern, matcher, raw, root, options) {
  ...
  if (isHost(matcher)) {
    ...
    if (protocol === 'statusCode') {
      statusCode = matcher.substring(index + 3) || 204;
    }
  }

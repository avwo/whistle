...
var PROPS_FILTER_RE = /^(?:filter|excludeFilter|includeFilter|ignore):\/\/(m(?:ethod)?|i(?:p)?|h(?:eader)?|s(?:tatusCode)?|clientIp|clientIP|serverIp|serverIP|req(?:H(?:eaders?)?)?|res(?:H(?:eaders?)?)?):(.+)$/;
...
function resolveRuleList(req, list, vals, index) {
  ...
  for (var i = 0; rule = list[i]; i++) {
    ...
    if (rule.isRegExp) {
      ...
      if (regExp && !matchFilters(curUrl, rule, req) && --index < 0) {
        regExp['0'] = curUrl;

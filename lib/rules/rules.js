...
function parsePluginTpl(lines) {
  ...
  lines.forEach(function(line) {
    line = line.replace(COMMENT_RE, '').trim()
      .replace(PLUGIN_TPL_RE, function(all, sep, key, value) {
        var tpl = ruleTpls[key.slice(0, key.indexOf('=') === -1 ? -3 : -1)];
        if (tpl == null) {
          return all;
        }
        return (sep || '') + tpl.replace(TPL_KEY_RE, value);
      });
  ...
  lines.forEach(function (line) {
    ...
    if (isRuleKey || VALUE_KEY_RE.test(line)) {
      if (root) {
        ...
        ruleList = line && line.split(LINE_END_RE);
      }
    } else {
      ruleList = [line];
    }

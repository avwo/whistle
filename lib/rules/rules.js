...
function _getRule(url, list, index) {
	url = formatUrl(url);
	//支持域名匹配
	domainUrl = formatUrl(url).replace(/^((?:https?|wss?|tunnel):\/\/[^\/]+):\d*(\/.*)/i, '$1$2');
	index = index || 0;
	...
	for (var i = 0, rule; rule = list[i]; i++) {
		...
		} else if ((matchedUrl = (url.indexOf(pattern = setProtocol(pattern, url)) === 0)) || 
...
proto.resolveHost = function(_url, callback, pluginRules) {
	...
	var host = !filterHost && ((pluginRules && getRule(_url, pluginRules._rules.host)) ||  getRule(_url, this._rules.host));
	if (host) {
...
Rules.parseRule = function(pattern, matcher, root) {
  var res = {rule: []};
  parseRule(res, pattern, matcher, pattern + ' ' + matcher, root);
  return res.rule[0];
};

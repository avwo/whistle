...
proto.getHost = function(req, pluginRulesMgr, rulesFileMgr, headerRulesMgr) {
  var curUrl = formatUrl(util.setProtocol(req.curUrl));
  req.curUrl = curUrl;
  var filterHost = ignoreHost(curUrl, this) || ignoreHost(curUrl, pluginRulesMgr)
    || ignoreHost(curUrl, rulesFileMgr) || ignoreHost(curUrl, headerRulesMgr);
...
proto.resolveRules = function(url) {
  ...
  multiMatchs.forEach(function(name) {
    rule = _rules[name];
    if (rule) {
      rule.list = getRuleList(url, rules[name], vals);
    }
  });

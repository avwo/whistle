...
function detactShorthand(url) {
  ...
  if (VALUE_KEY_RE.test(url) || PATH_RE.test(url) || VALUE_RE.test(url)) {
    return 'file://' + url;
  }
...
proto.resolveHost = function(_url, callback, pluginRules, fileRules, headerRulesMgr) {
  if (!_url || typeof _url !== 'string') {
    return callback();
  }
  var host = this.getHost(_url, pluginRules, fileRules, headerRulesMgr);
  if (host) {
...
proto.getHost = function(url, pluginRules, fileRules, headerRulesMgr) {
  url = formatUrl(util.setProtocol(url));
  var filterHost = ignoreHost(url, this) || ignoreHost(url, pluginRules)
    || ignoreHost(url, fileRules) || ignoreHost(url, headerRulesMgr);

...
var FILE_RE = /^(?:[a-z]:(?:\\|\/[^/])|\/[^/])/i;
var UPLOAD_FILE_PATH = /^\$(?:whistle|w2)\/[^/]+$/i;
...
function detactShorthand(url) {
  ...
  if (url === '/' || UPLOAD_FILE_PATH.test(url) || (FILE_RE.test(url) && !util.isRegExp(url))) {
    return 'file://' + url;
  }
  // compact Chrome
...
proto.resolveRules = function(req) {
  ...
  protocols.forEach(function(name) {
    if (name !== 'codec' && (name === 'proxy' || name === 'rule' || name === 'plugin'
...
proto.resolveCodec = function(req) {
  req.curUrl = req.curUrl || req.fullUrl;
  var filter = this.resolveFilter(req);
  if (util.isIgnored(filter, 'codec')) {
    return;
  }
  return getRule(req, this._rules.codec, this._values);
};

...
var protocols = ['host', 'req', 'rule', 'res', 'weinre', 'filter', 'disable', 'log', 'referer', 'auth', 'ua', 'urlParams', 'params', 'statusCode', 'redirect', 'method', 'cache', 
	...
	for (var i = 0, rule; rule = list[i]; i++) {
		...
		if (rule.isRegExp) {
			if (pattern.test(url)) {
				...
				var matcher = setProtocol(replaceSubMatcher(rule.matcher), url);

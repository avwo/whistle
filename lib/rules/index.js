...
function append(text, root) {
	_parse(text, root, true);
}

function _parse(text, root, append) {
	...
	getLines(text).forEach(function(line) {
		...
		if ((net.isIP(pattern) && !isRule(line[1])) || isRule(pattern)
			...
			line.slice(1).forEach(function(matcher) {
				parseRule(matcher, pattern, raw, root);
			});
		} else {
			//supports: pattern operator-uri1 operator-uri2 ... operator-uriN
			line.slice(1).forEach(function(matcher) {
				parseRule(pattern, matcher, raw, root);
			});
		}
	});
}

function parseRule(pattern, matcher, raw, root) {
	var isRegExp;

	...
	for (var i = 0, rule; rule = list[i]; i++) {
		...
		if (rule.isRegExp) {
			...
			if (pattern.indexOf('?') != -1) {
				...
				return extend({
					url: matcher + (matcher.indexOf('?') == -1 && filePath.indexOf('?') == -1 ? '?' : '') + filePath,
					files: files && files.map(function(file) {
						return file + (file.indexOf('?') == -1 && filePath.indexOf('?') == -1 ? '?' : '') + filePath;
					})
				}, rule);
			}
...
function join(first, last) {
	...
	if (isPathSeparator(first[first.length - 1])) {
		return isPathSeparator(last[0]) ? first + last.substring(1) : first + last;
	}

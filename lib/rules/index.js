...
proto.resolveHost = function(_url, callback, pluginRules) {
	...
	var host = !this.resolveFilter(_url).host && getRule(_url, this._rules.host);
	if (host) {
		return callback(null, util.removeProtocol(host.matcher, true));
	}
	if (pluginRules) {//TODO: 改成内置的getPlugin
		pluginRules.resolveHost(_url, callback);
	} else {
...
function resolveRules(fullUrl, callback) {

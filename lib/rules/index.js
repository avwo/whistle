...
function isProxyHost(req, proxy) {
  var props = proxy.lineProps;
  req._proxyTunnel = props.proxyTunnel || util.isEnable(req, 'proxyTunnel');
  return props.proxyHost;
}
...
function getProxy(url, req, callback, isHttpProxy) {
  ...
  var ignoreProxy;
  var proxy = (pRules && pRules.resolveProxy(req, isHttpProxy)) || rules.resolveProxy(req, isHttpProxy) ||

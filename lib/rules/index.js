...
var rules, hosts, heads, req, res, dump;
rules = hosts = heads = req = res = dump = [];
...
function _resolveRule(url, _rules) {
	_rules = _rules || rules;
	var _url = url.replace(/(?:\?|#).*$/, '');
	for (var i = 0, rule; rule = _rules[i]; i++) {
		var pattern = rule.pattern;
		if (rule.isRegExp) {
			if (pattern.test(url)) {
				...
				var result = extend({}, rule);
				result.url = setProtocol(rule.matcher.replace(/(^|.)?(\$[1-9])/g, 
						function(matched, $1, $2) {
					return $1 == '\\' ? $2 : ($1 || '') + regExp[$2];
				}), url);
				return result;
			...
			if (_url.indexOf(pattern) === 0) {
				...
				if (pattern == _url || isPathSeparator(_url[len]) || isPathSeparator(pattern[len - 1])) {
					var result = extend({}, rule);
					result.url = join(setProtocol(rule.matcher, url), url.substring(len));
					return result;
				}
			}
		}
...
function resolveReq(url, data) {
	return resolveRule(url, data, req);
}

exports.resolveReq = resolveReq;

function resolveRes(url, data) {
	return resolveRule(url, data, res);
}

exports.resolveRes = resolveRes;

function resolveDump(url, data) {
	return resolveRule(url, data, dump);
}

exports.resolveDump = resolveDump;

...
var protocols = ['host', 'req', 'reqRaw', 'rule', 'res', 'resRaw', 'weinre', 'filter', 'disable', 'log', 'referer', 'auth', 'ua', 'urlParams', 'params', 'statusCode', 'redirect', 'method', 'cache', 
	...
	for (var i = 0, rule; rule = list[i]; i++) {
		...
		if (rule.isRegExp) {
			if (pattern.test(url)) {
				...
				matcher = replaceSubMatcher(matcher);
				var result = extend({
					url: matcher,

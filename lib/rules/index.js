...
function initHeaderRules(req, needBodyFilters) {
  ...
  if (ruleValue) {
    ...
    var headerVars = rulesMgr._pluginVars;
    var curVars = rules._pluginVars;
    var pluginVars = extend({}, curVars, headerVars);
    req._pluginVars = pluginVars;
    Object.keys(pluginVars).forEach(function(key) {
      var headerVal = headerVars[key];
      var curVal = curVars[key];
      if (curVal && headerVal) {
        pluginVars[key] = headerVal.concat(curVal);
      }
      try {
        var value = JSON.stringify(pluginVars[key]);
        pluginVars[key] = Buffer.from(value).toString('base64');
      } catch (e) {}
    });
  }
}
...
exports.getClientCert = function(req, cb) {
  ...
  if (config.multiEnv) {
    ...
    rule = (pRules && pRules.resolveClientCert(req)) || rules.resolveClientCert(req) ||

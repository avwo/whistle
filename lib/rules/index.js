...
function initRules(req) {
  var ruleValue = util.trimStr(req.headers['x-whistle-rule-value']);
  if (ruleValue) {
    try {
      ruleValue = decodeURIComponent(ruleValue).trim();
    } catch(e) {}
  }
  var ruleKey = util.trimStr(req.headers['x-whistle-rule-key']);
  if (ruleKey) {
  ...
  delete req.headers['x-whistle-rule-value'];
  delete req.headers['x-whistle-rule-key'];
  if (rulesFromHeader) {

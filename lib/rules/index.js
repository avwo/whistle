...
function findProxyFromPac(url, pacRule, callback) {
  ...
  return pac.findWhistleProxyForURL(url.replace('tunnel:', 'https:'), function(err, proxyRule) {
    if (proxyRule) {
      pacRule.proxyRule = proxyRule;
    }
    callback(err, proxyRule);
  });
}

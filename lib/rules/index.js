...
var TUNNEL_HEADER = 'x-whistle-tunnel-host';
...
function getHost(req) {
  ...
  var tunnelHost = headers[TUNNEL_HEADER];
  if (tunnelHost) {
    var host = headers.host;
    if (host !== tunnelHost && tunnelHost !== getFullHost(req)) {
      tunnelHost = host.split(':')[0] + ' host://' + tunnelHost + ' # from tunnel';
      return hostHeader ? hostHeader + '\n' + tunnelHost : tunnelHost;
    }
    delete headers[TUNNEL_HEADER];
  }
  return hostHeader;
}

...
if (config.networkMode && !config.shadowRulesMode) {
  ...
  exports.parse = function (text, root, _values) {
    if (config.pluginsMode || config.shadowRulesMode) {
      text = util.resolveInlineValues(config.shadowRules || '', _values);
      text = util.getRemoteRulesResolver(_values)(text);
    }
    rules.parse(text, root, extend({}, config.shadowValues, _values));
  };
}

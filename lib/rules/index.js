...
var rule, hosts, head, req, res, weinre, filter;
rule = hosts = head = req = res = weinre = filter = [];
...
function _getRule(url, _rules) {
	...
	for (var i = 0, _rule; _rule = _rules[i]; i++) {
		var pattern = _rule.pattern;
		var matcher = setProtocol(_rule.matcher, url);

...
function getProxy(url, req, callback) {
  ...
  if (proxy) {
    if (!req._phost && P_HOST_RE.test(proxy.matcher)) {
      pattern = proxy.rawPattern;
      req._phost = parseUrl(util.setProtocol(RegExp.$1));
    }
    if (req._phost && proxy.isInternal) {
      var hostValue = pattern && req._phost.host;
      if (hostValue) {
        try {
          req.headers[PROXY_HOST_HEADER] = encodeURIComponent(pattern + ' host://' + hostValue);
        } catch (e) {}
      }
      req._phost = null;
    }
...
function resolveRulesFile(req, callback) {
  req.globalValue = util.getMatcherValue(req.rules.G);
...
function initHeaderRules(req, needBodyFilters) {
  ...
  var hostHeader = getValue(req, HOST_HEADER);
  var keyHeader = getValue(req, KEY_HEADER);
  var kvHeader = getValue(req, KV_HEADER);
  var phHeader = util.trimStr(getValue(req, PROXY_HOST_HEADER));
  ...
  if (ruleValue) {
    try {
      kvHeader = decodeURIComponent(kvHeader);
    } catch (e) {}
...
exports.getClientCert = function(req, cb) {
  ...
  var fRules = req.rulesFileMgr;
  var hRules = req.headerRulesMgr;
  var rule = (pRules && pRules.resolveClientCert(req)) || rules.resolveClientCert(req) ||
    (fRules && fRules.resolveClientCert(req)) || (hRules && hRules.resolveClientCert(req));
  if (!rule) {
    return cb();
  }

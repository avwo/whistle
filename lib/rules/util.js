...
function syncData() {
	if (!oldData.exists || propertiesStorage.getProperty('syncedDataSuccess')) {
		return;
	}
	var syncedDataSuccess;
	var _syncedData = propertiesStorage.getProperty('_syncedData');
	oldData.getRulesList().forEach(function(data) {
		if (!rulesStorage.existsFile(data.name) && (data.value || !_syncedData)) {
			addRulesFile(data.name, data.value);
			data.selected && selectRulesFile(data.name);
		}
		if (data.value) {
			syncedDataSuccess = true;
		}
	});
	
	oldData.getValuesList().forEach(function(data) {
		if (!valuesStorage.existsFile(data.name) && (data.value || !_syncedData)) {
	...
	if (!_syncedData) {
		...
		propertiesStorage.setProperty('_syncedData', true);
	}
	propertiesStorage.setProperty('syncedDataSuccess', syncedDataSuccess);
}

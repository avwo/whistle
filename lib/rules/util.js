...
try {
  var _files = fs.readdirSync(LOCAL_FILES)
...
function parseRules() {
  var disableRules = !config.notAllowedDisableRules && propertiesStorage.getProperty('disabledAllRules');
  var shadowRules = disableRules && config.allowDisableShadowRules ? null : config.shadowRules;
  var value = [];
  ...
  var backRulesFirst =  !config.disabledBackOption && propertiesStorage.getProperty('backRulesFirst') === true;
...
function allowMultipleChoice() {
  return !config.disabledMultipleOption && propertiesStorage.getProperty('allowMultipleChoice');
}
...
exports.properties = {
  ...
  set: function(name, value) {
    typeof name == 'string' ? propertiesStorage.setProperty(name, value) : propertiesStorage.setProperties(name);
  },
  ...
  addHistory: function(data) {
    ...
    for (var i = 0, len = history.length; i < len; i++) {
      ...
      if (item.url === result.url && item.method === result.method
        && item.headers === result.headers && item.body === result.body && !item.useH2 !== result.useH2) {
        history.splice(i, 1);
        break;
      }
    }

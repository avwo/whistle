...
var interceptRuleList = [];
var interceptRuleMap = {};
...
if (metaData._interceptRuleList && metaData._interceptRuleList.length) {
	interceptRuleList = metaData._interceptRuleList;
	interceptRuleList.forEach(function(domain) {
		interceptRuleMap[domain] = util.toRegExp(domain) || (domain + '');
	});
}
...
module.exports = {
		dataPath: ROOT,
		intercepts: function(domain) {
			if (!domain) {
				return false;
			}
			for (var i = 0, len = interceptRuleList.length; i < len; i++) {
				var d = interceptRuleList[i];
				d = interceptRuleMap[d];
				if (d == domain || (d.test && d.test(domain))) {
					return true;
				}
			}
		...
		getInterceptRuleList: function() {
			
			return interceptRuleList;
		},
		addInterceptRule: function(domain) {
			if (!domain || typeof domain != 'string' || domain.length > 256 || domain in interceptRuleMap) {
				return;
			}
			interceptRuleMap[domain] = util.toRegExp(domain) || domain;
			interceptRuleList.push(domain);
			setProperty('_interceptRuleList', interceptRuleList);
		},
		removeInterceptRule: function(domain) {
			var index = interceptRuleList.indexOf(domain);
			if (index == -1) {
				return;
			}
			
			delete interceptRuleMap[domain];
			interceptRuleList.splice(index, 1);
			setProperty('_interceptRuleList', interceptRuleList);
		},
		clearInterceptRules: function() {
			interceptRuleList = [];
			interceptRuleMap = {};
			setProperty('_interceptRuleList');
		},

...
var MAX_REMOTE_RULES_COUNT = 10;
var REMOTE_RULES_RE = /^\s*@(https?:\/\/[^\s#]+)\s*(?:#.*)?$/img;
...
function parseRules() {
  ...
  if (propertiesStorage.getProperty('disabledAllRules')) {
    ...
    if (backRulesFirst) {
      value = util.reverseText(value.join('\n'), true);
    }
    value = value.join('\r\n');
    if (shadowRules) {
      if (backRulesFirst) {
        value = util.reverseText(shadowRules) + '\n' + value;
      } else {
        value += '\n' + shadowRules;
      }
    }
  ...
  var index = 0;
  if (rulesText) {
    rulesText = rulesText.replace(REMOTE_RULES_RE, function(_, rulesUrl) {
      if (index >= MAX_REMOTE_RULES_COUNT) {
        return '';
      }
      ++index;
      var remoteRules = httpMgr.add(rulesUrl);
      return backRulesFirst ? util.reverseText(remoteRules) : remoteRules;
    });
  }
  if (!index) {
    httpMgr.clean();
  }
  rules.parse(rulesText, null, backRulesFirst);
}

...
function parseRules() {
  if (propertiesStorage.getProperty('disabledAllRules')) {
    return rules.parse(config.allowDisableShadowRules ? null : shadowRules);
  }
  var value = [];
  ...
  if (backRulesFirst) {
    value = value.join('\n').split(LINE_END_RE).reverse();
  }
  value = value.join('\r\n');
  if (shadowRules) {
    if (backRulesFirst) {
      shadowRules = shadowRules.split(LINE_END_RE).reverse().join('\n');
      value = shadowRules + '\n' + value;
    } else {
      value += '\n' + shadowRules;
    }
  }
  rules.parse(value, null, backRulesFirst);
  if (hosts) {
    hosts = hosts.join('\r\n\r\n');

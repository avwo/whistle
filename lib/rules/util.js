...
try {
  uploadFiles = fs.readdirSync(LOCAL_FILES)
...
exports.values = {
  ...
  addUploadFile: function(options, callback) {
    ...
    fs.writeFile(path.join(LOCAL_FILES, name), function(err) {
      if (!err) {
        var index = uploadFiles.indexOf(name);
        if (index !== -1) {
  ...
  removeUploadFile: function(name, callback) {
    if (!checkFilename(name) || uploadFiles.indexOf(name) === -1) {
      return callback();
    }
    
    fs.unlink(path.join(LOCAL_FILES, name), function() {
      var index = uploadFiles.indexOf(name);
      if (index !== -1) {
        uploadFiles.splice(index, 1);
      }

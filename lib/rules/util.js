...
var CLEAR_INTERVAL = 1000 * 60 * 3;
var TEMP_VALUE_RE = /\$WHISTLE_TEMP_VALUES\/\d+(?:\.\w+)?\b/g;
var inlineValues;
var CONTROL_RE = /[\u001e\u001f\u200e\u200f\u200d\u200c\u202a\u202d\u202e\u202c\u206e\u206f\u206b\u206a\u206d\u206c]+/g;
...
function clearTempValues() {
  var curTempFiles = rulesStorage.getFileList()
    .map(function(file) {
      return file.data;
    });
  curTempFiles.unshift(rulesStorage.getProperty('defalutRules'));
  curTempFiles = curTempFiles.join('\n').match(TEMP_VALUE_RE);
  if (curTempFiles) {
    curTempFiles = curTempFiles.map(function(file) {
      return file.substring(file.indexOf('/') + 1);
    });
  }
  fs.readdir(TEMP_VALUES_DIR, function(err, list) {
    if (err) {
      return setTimeout(clearTempValues, CLEAR_INTERVAL);
    }
    if (curTempFiles) {
      list = list.filter(function(name) {
        return curTempFiles.indexOf(name) === -1;
      });
    }
    var len = list.length;
    var callback = function() {
      if (--len < 0) {
        setTimeout(clearTempValues, CLEAR_INTERVAL);
      }
    };
    callback();
    list.forEach(function(name) {
      fs.unlink(path.join(TEMP_VALUES_DIR, name), callback);
    });
  });
}

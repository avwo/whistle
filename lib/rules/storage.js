...
function Storage(dir) {
  ...
  fs.readdirSync(self._files).forEach(function(file) {
    ...
    files[filename] = {
      index: index,
      name: filename,
      data: readFileSafe(path.join(self._files, file)) || readFileSafe(path.join(backupDir, file))
    };
  });

  var properties = readJsonSafe(self._properties) || readJsonSafe(self._backupProps) || {};
  self._cache = {
    maxIndex: maxIndex,
    files: files,
...
proto.renameFile = function(file, newFile) {
  ...
  var backupPath = this._getFilePath(file, true);
  delete cache.files[file.name];
  file.name = newFile;
  cache.files[newFile] = file;
  fs.rename(path, this._getFilePath(file), function(err) {
    if (!err) {
      fs.unlink(backupPath, noop);
    }
  }); //不考虑并发
  this._writeProperties();
  return true;

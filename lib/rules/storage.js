...
proto.getFileList = function getFileList(origObj) {
  ...
  var list = [];
  Object.keys(cache.files).forEach(function(file) {
    list.push(origObj ? cache.files[file] : copyFileObj(cache.files[file]));
  });
...
proto.moveUp = function moveUp(file) {
  var cache = this._cache;
  if (!(file = cache.files[file])) {
    return;
  }
  var list = this.getFileList(true);
  var prev = list[list.indexOf(file) - 1];
  if (prev) {
    this._swap(file, prev);
    return prev.name;
  }
};

proto.moveDown = function moveDown(file) {
  var cache = this._cache;
  if (!(file = cache.files[file])) {
    return;
  }
  var list = this.getFileList(true);
  var next = list[list.indexOf(file) + 1];
  if (next) {
    this._swap(file, next);
    return next.name;
  }
};

proto._swap = function swap(file1, file2) {
  var path1 = this._getFilePath(file1);
  var path2 = this._getFilePath(file2);
  var index = file1.index;
  file1.index = file2.index;
  file2.index = index;
  fs.rename(path1, this._getFilePath(file1), noop); //不考虑并发
  fs.rename(path2, this._getFilePath(file2), noop); //不考虑并发
};

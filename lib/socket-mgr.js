...
function handleClose(req, res, justTunnel) {
  handleSocketEnd(req, res, function(err) {
    ...
    req._hasClosed = true;
    if (closed || req.customParser || justTunnel) {
      return;
    }
    proxy.emit('frame', {
      reqId:  req.reqId,
...
function isHide(req) {
  return config.rulesMode || (req.filter.hide && !req.disable.hide);
}

function emitDataToProxy(req, chunk, fromClient, ignore) {
  if (isHide(req) || req._hasClosed) {
    return;
  }
  proxy.emit('frame', {
    reqId: req.reqId,
...
function drainData(status, socket, receiver, toServer) {
  ...
  status.sender && status.data.forEach(function(item) {
...
exports.handleUpgrade = function(req, res) {
  ...
  var hide = isHide(req);
  if (req.isPluginReq || (!hasEvent && hide)) {

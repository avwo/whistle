...
function pipeDecoder(req, decoder, emitReqError) {
  if (decoder) {
    req.pipe(decoder);
    decoder.on('error', emitReqError);
  }
  return req;
}

function pipeEncoder(encoder, res, emitReqError) {
  if (encoder) {
    encoder.on('error', emitReqError);
    encoder.pipe(res);
    return encoder;
  }
  return res;
...
exports.handleConnect = function(req, res) {
  ...
  pluginMgr.getReqDecoder(req, function(reqDecoder) {
    if (hasEvent || reqDecoder || isConn) {
      var url = req.fullUrl;
      var reqId = req.reqId;
      ...
      var emitReqError = function(err) {
        req.emit('error', err);
      };
      reqTrans.on('error', emitReqError);
      ...
      resTrans.on('error', emitReqError);
      ...
      pluginMgr.getReqEncoder(req, function(reqEncoder) {
        pipeDecoder(req, reqDecoder, emitReqError).pipe(reqTrans);
        reqTrans.pipe(pipeEncoder(reqEncoder, res, emitReqError));
        pluginMgr.getResDecoder(req, res, function(resDecoder) {
          pluginMgr.getResEncoder(req, res, function(resEncoder) {
            pipeDecoder(res, resDecoder, emitReqError).pipe(resTrans);
            resTrans.pipe(pipeEncoder(resEncoder, req, emitReqError));
          });
        });
      });
    } else {

...
function handleConnSend(ctx, reqTrans, sendStatus) {
  ...
  sendStatus.emitData = function() {
    sendStatus.chunk && emitDataToProxy(req, sendStatus.chunk, true, sendStatus.ignore);
...
function handleConnReceive(ctx, resTrans, receiveStatus) {
  ...
  receiveStatus.emitData = function() {
    receiveStatus.chunk && emitDataToProxy(req, receiveStatus.chunk, undefined, receiveStatus.ignore);
...
function handleWsSend(ctx, reqTrans, sendStatus) {
  var req = ctx.req;
  var res = ctx.res;
  var hasEvent = ctx.hasEvent;
  var url = ctx.url;
  ...
  handleSocketEnd(req, res, reqReceiver.cleanup.bind(reqReceiver));
  ctx.sendToServer = function(data) {
...
function handleWsReceive(ctx, resTrans, receiveStatus) {
  var req = ctx.req;
  var res = ctx.res;
  var hasEvent = ctx.hasEvent;
  var url = ctx.url;
  ...
  handleSocketEnd(req, res, resReceiver.cleanup.bind(resReceiver));
  ctx.sendToClient = function (data) {
...
exports.handleUpgrade = function(req, res) {
  ...
  pluginMgr.getWsPipe(req, res, function(reqRead, reqWrite, resRead, resWrite) {
    var hasPipeStream = reqRead || reqWrite || resRead || resWrite;
    if (hasPipeStream) {
      req.customParser = customParser = false;
    }
    var ctx = conns[reqId] = getContext(req, res, hasEvent, sendStatus, receiveStatus);
    if (customParser) {
      removePending(reqId);
    ...
    pipeStream(req, reqRead).pipe(reqTrans).pipe(pipeStream(reqWrite, res, true));
    pipeStream(res, resRead).pipe(resTrans).pipe(pipeStream(resWrite, req, true));
    handleClose(req, res);
  });
};

  ...
  var ctx = conns[reqId] = {
    ...
    sendToServer: function(data) {
      ...
      if (sendStatus.ignoring || sendStatus.callback || !reqReceiver.existsCacheData) {
        return drainData(sendStatus, res, reqReceiver);
      }
      return sendStatus.data.length > MAX_COMPOSE_FRAME_COUNT;
    },
    sendToClient: function(data) {
      receiveStatus.data.push(data);
      if (receiveStatus.ignoring || receiveStatus.callback || !resReceiver.existsCacheData) {
        return drainData(receiveStatus, req, resReceiver);
      }
      return receiveStatus.data.length > MAX_COMPOSE_FRAME_COUNT;
    }
  };
  initStatus(ctx, req.enable);
  var reqTrans = new PassThrough();
...
exports.sendData = function(data) {
  ...
  if (ctx.customParser) {
    ...
    data.base64 = data.data.toString('base64');
    delete data.data;
    dataList.push(data);
    return;
  }

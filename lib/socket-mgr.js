...
function emitDataToProxy(req, chunk, fromClient, ignore) {
  if (util.isHide(req) || req._emittedClosed) {
    return;
  }
  proxy.emit('frame', {
    reqId: req.reqId,
...
exports.sendData = function (data) {
  ...
  if (isServer) {
    data.mask = true;
    return ctx.sendToServer && ctx.sendToServer(data);
  } else {
    return ctx.sendToClient && ctx.sendToClient(data);
  }
};

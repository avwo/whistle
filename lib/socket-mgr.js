...
function emitErrorFrame(req, res, err) {
  if (req._emittedErrFrame) {
    return;
  }
  req._emittedErrFrame = true;
  proxy.emit('frame', {
    reqId: req.reqId,
    frameId: getFrameId(),
    closed: true,
...
function handleClose(req, res, justTunnel) {
  handleSocketEnd(req, res, function (err) {
    ...
    if (closed || justTunnel || (ctx && ctx._justTunnel)) {
      return;
    }
    emitErrorFrame(req, res, err);
  });
}
...
function handleWsSend(ctx, reqTrans, sendStatus, isTunnel) {
  ...
  var reqId = req.reqId;
  ctx._justTunnel = true;
  req.on('_closed', function(err) {
  ...
  reqReceiver.onerror = function (err) {
    if (req._hasClosed || req._emittedErrFrame) {
      return;
    }
    req._hasClosed = true;
    proxy.emit('frame', {

...
function setConnStatus(ctx, status, statusObj, name, emitData) {
  ...
  if (status === 2) {
    ...
    emitData && statusObj.chunk && emitData(statusObj.chunk, true, statusObj.ignore);
    ...
    if (isConn) {
      var ctx = conns[reqId] = {
        ...
        setSendStatus: function(status) {
          setConnStatus(ctx, status, sendStatus, 'sendStatus', emitData);
        },
...
exports.handleWebsocket = function(req, res) {
  ...
  var ctx = conns[reqId] = {
    ...
    sendToServer: function(data) {
      ...
      if (sendStatus.ignoring || sendStatus.callback) {
        drainData(sendStatus, res, reqReceiver);
      }
    },
    sendToClient: function(data) {
      receiveStatus.data.push(data);
      if (receiveStatus.ignoring || receiveStatus.callback) {

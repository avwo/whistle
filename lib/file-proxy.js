var fs = require('fs');
var util = require('util');
var commonUtil = require('../util/util');
var url = require('url');
var mime = require('mime');
var config = require('../package.json');
var hosts = require('../data/hosts');
var dust = require('dustjs-helpers');
var Readable = require('stream').Readable;
var querystring = require('querystring');


module.exports = function(req, res, next) {
	var options = req.options;
	var protocol = options && options.protocol;
	if (!/^(?:|x|xs)(?:file|dust):$/.test(protocol)) {
		next();
		return;
	}
	
	var path = options.url.substring(protocol.length + 2).replace(/\/?(?:\?|#).*$/, '');
	fs.stat(path, function(err, stat){
	    if(err || !stat.isFile()) { 
	    	if (/^x/.test(protocol)) {
				  util._extend(options, url.parse(req.fullUrl));
				  if (/^xs/.test(protocol)) {
					  options.protocol = 'https:';
				  }
				 
				  hosts.resolveHost(options.hostname, function(err, ip) {
					  options.hosts[1] = ip;
					  next(err);
				  });
			  } else {
				  next(err || new Error('Not found file ' + path));
			  }
	    	return;
	    }

	    commonUtil.drain(req, function() {
	    	var reader = fs.createReadStream(path);
		    reader.statusCode = 200;
		    reader.headers = {
			    	'Content-Type': mime.lookup(path) + '; charset=utf-8',
			    	'Server': config.name
			    };
		    if (/dust:$/.test(protocol)) {
		    	var content = '';
		    	reader.setEncoding('utf8');
		    	reader.on('data', function(data) {
		    		content += data;
		    	});
		    	reader.on('end', function() {
		    		var index = req.url.indexOf('?') + 1;
		    		dust.compileFn(content)(index > 0 ? querystring.parse(req.url.substring(index)) : {}, 
		    				function(err, data) {
				    			if (err) {
				    				next(err);
				    				return;
				    			}
				    			var _reader = new Readable();
				    			_reader.statusCode = reader.statusCode;
						    	_reader.headers = reader.headers;
				    			_reader._read = function() {
				    				_reader.push(data);
				    				_reader.push(null);
				    			};
				    			
				    			res.response(_reader);
		    		});
		    	});
		    	
		    	reader.on('error', next);
		    } else {
		    	res.response(reader);
		    }
	    });
	    
	});
};
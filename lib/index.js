...
function proxy(conf) {
	if (conf) {
		['port', 'sockets', 'days', 'timeout',
		 'username', 'password', 'uipath', 'rules']
	...
	if (!conf || !Array.isArray(conf.middlewares)) {
		config.middlewares = [];
	} else if (typeof conf.middlewares == 'string') {
	...
	var agentConfig = {maxSockets: config.sockets};
	config.httpAgent = new (require('http').Agent)(agentConfig);
	config.httpsAgent = new (require('https').Agent)(agentConfig);
...
process.on('uncaughtException', function(err){
	...
	fs.writeFileSync(path.join(process.cwd(), config.name + '-log.log'), '\r\n' + util.formatDate()  + '\r\n' +  err.stack + '\r\n', {flag: 'a'});

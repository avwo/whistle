...
//see: https://github.com/joyent/node/issues/9272
  ...
  server.on('connect', function(req, reqSocket, head) {//ws, wss, https proxy
    var tunnelUrl = req.fullUrl = util.setProtocol(PROXY_HOST_RE.test(req.url) ? req.url : req.headers.host, true);
    var options;
    ...
    rules.resolveFileRules(req, function() {
      ...
      util.mergeRules(req, rulesMgr && rulesMgr.resolveRules(tunnelUrl));
      var useTunnelPolicy = req.headers[WHISTLE_POLICY_HEADER] == 'tunnel';
      if (!useTunnelPolicy && (config.isLocalUIUrl(hostname) || config.isPluginUrl(hostname) || ((filter.https || filter.tunnel

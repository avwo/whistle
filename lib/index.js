  ...
  server.on('connect', function(req, reqSocket, head) {//ws, wss, https proxy
    ...
    pluginMgr.getTunnelRules(req, function(_rulesMgr) {
      ...
      if (/^tunnel:\/\//.test(realUrl) && realUrl != tunnelUrl) {
        tunnelUrl = data.realUrl = _rules.rule.url;
        parseUrl();
      }
      
      rules.findProxyFromPac(tunnelUrl, tunnelPort || proxyUrl ? null : _rules, function() {
        if (!proxyUrl) {
    ...
    function tunnel() {
      resolveHost(tunnelUrl, function(ip, port) {
        resData.ip = !data.realUrl && port ? ip + ':' + port : ip;
        resSocket = net.connect(proxyUrl ? options.port : (port || options.port), ip, function() {

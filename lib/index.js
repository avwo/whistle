	...
	server.on('connect', function(req, reqSocket, head) {//ws, wss, https proxy
		...
		pluginMgr.getTunnelRules(req, function(_rulesMgr) {
			...
			if (filter.rule) {
			  ...
			  tunnelPort = _plugin && _plugin.ports && _plugin.ports.tunnelPort;
			}
          ...
          if (isSocks) {
            ...
            dstOptions.proxyHost = ip;
		        dstOptions.proxyPort = parseInt(options.port, 10) || 1080;
            ...
            socks.connect(dstOptions, function(_proxySocket) {

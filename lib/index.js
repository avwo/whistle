...
function proxy(conf) {
	...
	config.httpsProxyAgent = config.debug ? false : createAgent(agentConfig, true);
...
function createAgent(agentConfig, https) {
	return new (require(https ? 'https' : 'http').Agent)(agentConfig).on('free', avoidThrowOutError);
}

...
function tunnelProxy(server, proxy) {
	...
	server.on('connect', function(req, reqSocket, head) {//ws, wss, https proxy
				...
				if (/^https:/i.test(_url)) {
					...
					if (port) {
						function next (err, data) {
							...
							options.port = port;
							_url = 'https://' + req.headers.host.replace(/:\d*$/, '') + ':' + port;
							tunnel(_url, options.hostname);
						}
		...
		function tunnel(url, ip) {
			...
			url = url || tunnelUrl;
			ip ? rules.resolveHost(url, callback, filter.host)
					: callback(null, ip);
			
			function callback(err, ip, customHost) {
				if (err) {
					emitError(err);
					return;
				}

...
function proxy(callback) {
  ...
  if (config.values) {
    Object.keys(config.values).forEach(function(name) {
      var value = name ? config.values[name] : null;
      if (typeof value === 'string') {
        rulesUtil.values.add(name, value);
      }
    });
  }
  if (config.rules) {
    if (Array.isArray(config.rules) || typeof config.rules == 'string') {
      rulesUtil.rules.setDefault(getRules(config.rules));
    } else {
      Object.keys(config.rules).forEach(function(name) {
        var item = name ? config.rules[name] : null;
        ...
        if (item) {
          ...
          if (name === 'Default') {
            if (typeof item.rules === 'string') {
              rulesUtil.rules.setDefault(item.rules);
            }
            if (item.enable) {
              rulesUtil.rules.enableDefault();
            } else if (item.enable === false) {
              rulesUtil.rules.disableDefault();
            }
          } else {
            if (typeof item.rules === 'string') {
              rulesUtil.rules.add(name, item.rules);
            }
            if (item.enable) {
              rulesUtil.rules.select(name);
            } else if (item.enable === false) {
              rulesUtil.rules.unselect(name);
            }
          }
        }
      });

...
function tunnelProxy(server, proxy) {
	...
	server.on('connect', function(req, reqSocket, head) {//ws, wss, https proxy
		var tunnelUrl = req.fullUrl = util.setProtocol(req.headers.host, true);
		...
		if (!plugin && _rules.plugin) {
			...
			if (pluginRule) {
				plugin = pluginMgr.getPlugin(pluginRule.name + ':');
				req.ruleValue = pluginRule.value;
			}
		}
		req.pluginMgr = plugin;
		req.reqId = ++index;
		pluginMgr.getTunnelRules(req, function(_rulesMgr) {
			...
			var tunnelPort = _plugin && _plugin.ports && _plugin.ports.tunnelPort;
			var proxyUrl = tunnelPort ? 'proxy://127.0.0.1:' + tunnelPort : util.rule.getProxy(_rules.rule);
			if (proxyUrl) {
				...
				resolveHost(_url, function(ip) {
					...
					util.connect({
						...
						headers: tunnelPort ? extend({'x-whistle-req-id': req.reqId}, headers) : headers
		...
		function tunnel(url, ip) {
			resolveHost(url || tunnelUrl, function(ip, port) {
		...
		function resolveHost(url, callback, ip) {
			rules.resolveHost(ip || url, function(err, ip, port, host) {
				if (host) {
					_rules.host = host;
				}

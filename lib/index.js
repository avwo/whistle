...
function proxy(callback) {
  ...
  server.timeout = 1000 * 60 * (config.strict ? 3 : 60);
  ...
  createNormalServer(config.httpsPort, https, ca.SNI_OPTIONS);
  if (config.socksPort) {
    ++count;
    var boundHost;
    var socksServer = socks.createServer(function(info, accept, deny) {
      var dstPort = info.dstPort;
      var connPath = info.dstAddr + ':' + dstPort;

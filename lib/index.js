...
function tunnelProxy(server, proxy) {
	...
	server.on('connect', function(req, reqSocket, head) {//ws, wss, https proxy
		...
		if (rulesUtil.intercepts(options.hostname + (!port || port == 80 || port == 443 ? '' : ':' + port))) {
			cert.createCert(options.hostname, function(err) {
				if (err) {
					//TODO 通知到抓包页面
				}
				sendEstablished();
				https.dispatch(reqSocket, proxy);
			});
			return;
		}
...
function proxy(conf) {
	...
	https.createServer(proxyEvents);

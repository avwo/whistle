...
function tunnelProxy(server, proxy) {
	...
	server.on('connect', function(req, reqSocket, head) {//ws, wss, https proxy
		...
		if (!useTunnelPolicy && (/local\.whistlejs\.com$/.test(hostname) || filter.https || filter.tunnel
		...
		if (!plugin && _rules.plugin) {
			plugin = pluginMgr.getPlugin(util.getMatcherValue(_rules.plugin) + ':');
		}
		...
		pluginMgr.getTunnelRules(req, function(_rulesMgr) {
			if (_rulesMgr) {
				...
				if (!disable.tunnel && _disable.tunnel) {
					disable.tunnel = _disable.tunnel;
					_rules.disable = pluginsRules.disable;
					return reqSocket.destroy();
				}
			}

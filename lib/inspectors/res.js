...
module.exports = function(req, res, next) {
	...
	req.request = function(options) {
		...
		var proxyUrl = proxy && util.rule.getMatcher(proxy);
		...
		rules.resolveHost(net.isIP(options.host) ? options.host : (proxyUrl
			...
			if (proxyUrl) {
				...
				options.port = proxyOptions.port = proxyOptions.port || config.port;
				if (proxyOptions.protocol == 'socks:') {
					console.log(util.getAuths(proxyOptions));
					return;
				}

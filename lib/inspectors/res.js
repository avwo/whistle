...
function checkH2(req, isHttps) {
  ...
  if (isHttps) {
    if (req.disable.h2) {
      req.useH2 = false;
    } else if (req.enable.h2) {
      req.useH2 = true;
    }
  } else {
    if (req.disable.httpH2) {
      req.useH2 = false;
    } else if (req.enable.httpH2) {
      req.useH2 = true;
    }
          ...
          rules.resolveHost(req, function(err, ip, port, host) {
            ...
            function request(options, serverPort, direct) {
              ...
              var send = function(sock) {
                ...
                req.customParser = options.isPlugin && util.getParserStatus(req);
                if (req.customParser && req.initCustomParser) {
                ...
                util.getFileWriters([bodyFile, rawFile], function(writer, rawWriter) {
                  ...
                  util.delay(util.getMatcherValue(resRules.resDelay), function() {
                    ...
                    if (req.enableCustomParser) {
                      if (_res.isCustomRes || headers['x-whistle-disable-custom-frames'] || !util.isCustomParser(req)) {
                        delete headers['x-whistle-disable-custom-frames'];
                        req.disableCustomParser();
                      } else {
                        req.enableCustomParser();
                      }
                    }
                    try {

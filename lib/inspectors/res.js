...
var maxSockets = parseInt(config.sockets,10) || 0;
var keepAlive = config.keepAlive;
var sockets = {};


function connectProxy(options, callback, _keepAlive) {
  if (maxSockets < 1 || !keepAlive || !_keepAlive) {
    return util.connect(options, function(err, socket) {
      !err && socket.once('free', destroy);
      callback(err, socket);
    });
  }
  var key = (options.isSocks ? 'socks_' : 'http_') + options.host + ':' + options.port;
  var list = sockets[key];
  var socket;
  if (!list) {
    sockets[key] = list = [];
  } else {
    socket = getAvailableSocket(list);
  }
  if (socket) {
    return callback(null, socket);
  }
  
  util.connect(options, function(err, socket) {
    if (err) {
      return callback(err);
    }
    var removeSocket = function() {
      var index = list.indexOf(socket);
      if (index != -1) {
        list.splice(index, 1);
      }
    };
...
function getAvailableSocket(list) {
  var socket;
  while((socket = list.shift()) && socket.destroyed) {}
  return socket;
}

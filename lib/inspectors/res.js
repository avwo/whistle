			...
			util.parseRuleJson([resRules.res, resRules.resHeaders, resRules.resCookies, cors, resRules.resReplace], 
				...
				util.readInjectFiles(data, function(data) {
					util.getRuleValue([resRules.resBody, resRules.resPrepend, resRules.resAppend, resRules.html, resRules.js, resRules.css,
					                   resRules.resBodyBin, resRules.resPrependBin, resRules.resAppendBin], 
							function(resBody, resPrepend, resAppend, html, js, css, resBodyBin, resPrependBin, resAppendBin) {
								if (resBody) {
									data.body = resBody;
								}
								...
								if (resBodyBin || resPrependBin || resAppendBin) {
									res.addZipTransform(new BufferTransform({
										top: resPrependBin,
										body: resBodyBin,
										bottom: resAppendBin
									}));
								...
								util.drain(req, function() {
									util.getFileWriters([util.hasBody(_res) ? getWriterFile(util.getRuleFile(resRules.resWrite), _res.statusCode) : null, 
										...
										var done;
										function postStatus(err, status) {
											if (done) {
												return;
											}
											done = true;
											pluginMgr.postStatus(req, res, status || 'error', err);
										}
										try {
											res.writeHead(_res.statusCode, _res.headers);
											_res.trailers && res.addTrailers(_res.trailers);
											pluginMgr.postStatus(req, res, 'response');
											req.once('error', postStatus);
											res.once('error', postStatus);
											res.once('finish', function() {
												postStatus(null, 'response');
											});
										} catch(e) {
											e._resError = true;

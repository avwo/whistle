...
function getWriterFiles(resRules, statusCode) {
	var file = getWriterFile(util.getRuleFile(resRules.resWrite), statusCode);
	var rawFile = getWriterFile(util.getRuleFile(resRules.resWriteRaw), statusCode);
...
module.exports = function(req, res, next) {
	...
	res.response = function(_res) {
		...
		util.getFileWriters(util.hasBody(_res) ? getWriterFiles(resRules, _res.statusCode) : null, function(writer, rawWriter) {
				res.on('src', function(_res) {
					if (writer) {
						res.addZipTransform(new FileWriterTransform(writer, res));
					}
					
					if (rawWriter) {
						res.addZipTransform(new FileWriterTransform(rawWriter, res, true));
					}
				});

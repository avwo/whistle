    ...
    rules.getProxy(options.href, options.isPlugin ? null : req, function(err, hostIp, hostPort) {
      ...
      rules.resolveHost(curUrl, function(err, ip, port, host) {
        ...
        if (proxyUrl) {
          ...
          isSocks = SOCKS_PROXY_RE.test(proxyOptions.protocol);
          if (isHttps || isSocks) {
        ...
        function request(options, serverPort, direct) {
          ...
          var retryReq = function(err) {
            ...
            } else if (isHttps && !req.disable.auto2http && (req.enable.auto2http
              || resRules.host || util.isLocalAddress(ip)) && util.checkSslError(err)) {
              isHttps = false;
              ++maxRetryCount;
              setAgent();
            }

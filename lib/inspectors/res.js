...
function readResRules(rules, callback) {
	...
	['resBody', 'prependRes', 'appendRes'].forEach(function(key) {
		util.getRuleValue(rules[key], function(value) {
			if (value != null) {
				if (key == 'resBody') {
					result.body = value;
				} else if (key == 'prependRes') {
					result.top = value;
				} else {
					result.bottom = value;
				}
	...
	util.parseRuleToJson(req.rules.res, 
		...
		util.readInjectFiles(data, function(data) {
			readResRules(req.rules, function(result) {
				...
				var cache = util.removeProtocol(util.rule.getMatcher(req.rules.cache), true);
				var maxAge = parseInt(cache, 10);
				var noCache = cache == 'no' || maxAge < 0;
				if (maxAge > 0 || noCache) {
					var headers = data.headers = data.headers || {};
					headers['cache-control'] = noCache ? 'no-cache' : 'max-age=' + maxAge;
					headers['expires'] = new Date(Date.now() + (noCache ? -60000000 : maxAge)).toGMTString();
					headers['pragma'] = noCache ? 'no-cache' : '';
				}
				...
				var statusCode = util.removeProtocol(util.rule.getMatcher(req.rules.statusCode), true);
				if (statusCode) {
					data.statusCode = statusCode;
					data.body = data.body || STATUS_CODE[statusCode];
				} else {
					var redirect = util.removeProtocol(util.rule.getMatcher(req.rules.redirect), true);
					if (redirect) {
						data.statusCode = 302;
						data.headers = data.headers || {};
						data.headers.location = redirect;
					}
				}

		...
		util.readInjectFiles(data, function(data) {
			readResRules(req.rules, function(result) {
				...
				if (resCors == '*') {
					...
					cors = null;
					var origin = req.fullUrl || '';
					origin = origin.substring(0, origin.indexOf('/', origin.indexOf('//') + 3));
					util.setHeaders(data, {
				...
				util.parseRuleToJson([req.rules.resCookies, cors], 

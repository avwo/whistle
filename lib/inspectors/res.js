...
var TunnelAgent = require('tunnel-agent').httpsOverHttp;
...
var maxSockets = Math.max(parseInt(require('../config').sockets), 12);
var agents = {};


function getTunnelAgent(options) {
  var key = [options.host, options.port, options.auth || ''].join(':');
  ...
  if (!agent) {
    ...
    agent = agents[key] = new TunnelAgent(options);
  }
		...
		rules.resolveHost(options.localDNS && net.isIP(options.host) ? options.host : 
            ...
            headers: {
              ...
              'user-agent': headers['user-agent']
				...
				if (isHttps) {
				  options.agent = getTunnelAgent(opts);
				  request(options);
				  return;
				}
				
				if (isSocks) {
					util.connect(opts, function(err, socket) {
						if (err) {

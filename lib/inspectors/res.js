              ...
              var send = function(sock) {
                ...
                try {
                  ...
                  client.once('socket', function(socket) {
                    util.perf.measure('once_socket');
                    if (socket.connecting || socket._connecting) {
                      util.perf.measure('socket_connecting');
                      ...
                      socket.once((isHttpsProxy || isHttps) ? 'secureConnect' : 'connect', function() {
                        util.perf.measure(`once_${ (isHttpsProxy || isHttps) ? 'secureConnect' : 'connect' }`);
                      ...
                      util.perf.measure('socket_disconnected');
                      retryCount = maxRetryCount;
                      piped = true;
                      pipeClient(req, client);
                      socket.resume();

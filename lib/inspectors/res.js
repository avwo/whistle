...
module.exports = function(req, res, next) {
  ...
  res.response = function(_res) {
    ...
    pluginMgr.getResRules(req, _res, function() {
      ...
      util.parseRuleJson([resRules.res, resRules.resHeaders, resRules.resCookies, cors, resRules.resReplace, resRules.resParams],
...
//nodejs的url只支持ascii，对非ascii的字符要encodeURIComponent，否则传到浏览器是乱码
  ...
  if ((type === 'JSON' || type === 'JS') && params && Object.keys(params).length) {
    ...
    transform._transform = function(text, encoding, callback) {
      if (text) {
        if (!interrupt) {
          ctn += text;
          text = null;
          if (Buffer.byteLength(ctn) > MAX_RES_SIZE) {
            interrupt = true;
            text = ctn;
            ctn = null;
          }

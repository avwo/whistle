        ...
        function request(options, serverPort, direct) {
          ...
          var retryReq = function(err) {
            if (retryCount >= maxRetryCount) {
              ...
              if (getMethod(req) !== 'GET' || resetCount > 1 || code !== 'ECONNRESET') {
            ...
            if (proxyUrl) {
              if (X_RE.test(proxyUrl)) {
                proxyUrl = '';
                setAgent();
                rules.resolveHost(req.realUrl, function(_err, _ip, _port, _host) {
              ...
              ++retryXHost;
              if (retryXHost > 1) {
                rules.lookupHost(req.realUrl, function(_err, _ip) {
                  setHostsInfo(_ip);
                  if (_err) {
                    showDnsError(res, _err);
                    return;
  ...
  res.response = function(_res) {
    ...
    util.drain(req, function() {
      pluginMgr.getResRules(req, _res, function() {
        ...
        var ruleList = [resRules.res, resRules.resHeaders, resRules.resCookies, cors, resRules.resReplace, resRules.resMerge];
        util.parseRuleJson(ruleList, function(data, headers, cookies, cors, replacement, params) {

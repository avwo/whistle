					...
					if (list && list.length) {
						...
						list.forEach(function(name) {
							...
							if (cookie == null) {
								...
								if (value != null) {
									attrs.push(value ? util.encodeURIComponent(value) : value);
								}
								...
								cookie.domain && attrs.push(cookie.domain);
								cookie.httpOnly && attrs.push('httpOnly');
								result[name] = attrs.join('; ');
							} else {
								result[name] = util.encodeURIComponent(cookie);
							}
						});
						...
						curCookies.forEach(function(cookie) {
							...
							var name = index == -1 ? name : cookie.substring(0, index);
							if (!(name in result)) {
								result[name] = index == -1 ? null : cookie.substring(index + 1);
							}
						});

...
function setRedirect(req, headers) {
  var redirect = util.getMatcherValue(req.rules.redirect);
  if (redirect) {
    headers.location = redirect;
    return util.isEnable(req, 'movedPermanently') ? 301 : 302;
  }
}
...
module.exports = function (req, res, next) {
              ...
              function (err, ip, port, hostRule) {
                ...
                function request(options, serverPort, direct) {
                  ...
                  var send = function (sock) {
                    ...
                    try {
                      var client = (isHttps ? https : http).request(
                        options,
                        res.response
                      );
                      curClient = client;
    ...
    util.drain(req, function () {
      readFirstChunk(req, res, _res, function (firstChunk) {
        pluginMgr.getResRules(req, _res, function () {
          var status = setRedirect(req, res.headers);
          if (status) {
  ...
  var resHeaders = {};
  var statusCode = util.getStatusCodeFromRule(resRules) || setRedirect(req, resHeaders);
  if (statusCode) {

...
module.exports = function(req, res, next) {
  ...
  req.request = function(options) {
    ...
    var now = Date.now();
    var proxyUrl;
    if (!options.isPlugin && protoMgr.isProxyProtocol(options.protocol)) {
      proxyUrl = util.rule.getMatcher(resRules.rule);
      options = url.parse(req.fullUrl);
    }

    rules.findProxyFromPac(options.href, options.isPlugin || proxyUrl ? null : resRules.pac, function(err, proxyRule) {
      if (proxyRule) {
        proxyUrl = proxyRule;
      }
        ...
        (proxyUrl && util.replaceProtocol(proxyUrl, util.getProtocol(req.fullUrl)) || options.href),
            function(err, ip, port, host) {
              ...
              if (proxyUrl) {
                ...
                if (isHttps || isSocks) {
                  if ((proxyOptions.port || (isHttps ? 1080 : 80)) == config.port && util.isLocalAddress(ip)) {

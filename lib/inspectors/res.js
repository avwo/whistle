...
module.exports = function(req, res, next) {
	...
	req.request = function(options) {
		...
		var proxyUrl;
		if (options.proxy) {
			proxyUrl = util.rule.getMatcher(options.proxy);
			proxyUrl = util.replaceProtocol(proxyUrl, util.getProtocol(req.fullUrl));
		}
		
		rules.resolveHost(net.isIP(options.host) ? options.host : (proxyUrl || options.href),

...
function readResRules(rules, callback) {
	...
	util.parseRuleJson(rules.resHeaders, function(headers) {
		if (headers) {
	...
	util.getRuleValue([rules.resBody, rules.resPrepend, rules.resAppend, rules.insertHTML, rules.insertJS, rules.insertCSS], 
	function(resBody, resPrepend, resAppend, insertHTML, insertJS, insertCSS) {
		if (resBody) {
			result.body = resBody;
		}
		
		if (resPrepend) {
			result.top = resPrepend;
		}
		
		if (resAppend) {
			result.append = resAppend;
		}
		
		if (insertHTML) {
			result.insertHTML = insertHTML;
		}
		
		if (insertJS) {
			result.insertJS = insertJS;
		}
		
		if (insertCSS) {
			result.body = insertCSS;
		}
	});
	
	['resBody', 'resPrepend', 'resAppend', 'insertHTML', 'insertJS', 'insertCSS']
...
module.exports = function(req, res, next) {
	...
	util.parseRuleJson(resRules.res, 
			function(data) {
		...
		util.readInjectFiles(data, function(data) {
			readResRules(resRules, function(result) {
				...
				util.parseRuleJson([resRules.resCookies, cors], 
						function(cookies, cors) {
					...
					if (!util.isEmptyObject(data)) {
						res.on('src', function(_res) {
							var type = util.getContentType(_res.headers);
							var bottom = data.bottom || '';
							switch(type) {
								case 'HTML':
									bottom += data.insertHTML || '';
									if (data.insertJS) {
										bottom += '<script>' + data.insertJS + '</script>';
									}
									
									if (data.insertCSS) {
										bottom += '<style>' + data.insertCSS + '</style>';
									}
									
									break;
								case 'JS':
									bottom +=  data.insertJS || '';
									break;
								case 'CSS':
									bottom +=  data.insertCSS || '';
									break;
							}
							delete data.insertHTML;
							delete data.insertJS;
							delete data.insertCSS;

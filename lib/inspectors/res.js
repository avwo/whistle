		...
		util.readInjectFiles(data, function(data) {
			readResRules(resRules, function(result) {
				...
				util.parseRuleToJson([resRules.resCookies, cors], 
					...
					if (util.hasBody(res)) {
						util.getRuleValue(resRules.resTransform || resRules.resT, function(script) {
							if (script && (script = script.trim())) {
								res.on('src', function(_res) {
									...
									var whistle = {
											url: req.realUrl,
											originalUrl: req.fullUrl,
											statusCode: res.statusCode, 
								            headers: util.clone(res.headers),
									...
									try {
										...
										if (typeof whistle.transform == 'function') {
											...
											transform._transform = function(chunk, encoding, callback) {
												...
												try {
													var end = !chunk;
													newChunk = util.toBuffer(whistle.transform(chunk, end));
												} catch(e) {}
												callback(null, newChunk === undefined ? chunk : newChunk);
											};

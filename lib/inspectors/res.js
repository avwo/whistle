        ...
        if (options.localDNS && net.isIP(options.host)) {
          ...
          if (isInternalProxy || proxyRule.isHttps2http) {
            if (isInternalProxy && options.protocol === 'https:') {
              ...
              if (req.isH2) {
                headers[config.SESSION_ID_HEADER] = req.sessionId;
      ...
      rules.resolveHost(req, function(err, ip, port, host) {
        ...
        function request(options, serverPort, direct) {
          ...
          var send = function(sock) {
            ...
            req.useH2 = false;
            req.setServerPort && req.setServerPort();
            try {
              ...
              client.once('socket', function(socket) {
                if (!sock && (socket.connecting || socket._connecting)) {
          ...
          if (headers[config.HTTPS_FIELD]) {
            ...
            delete headers[config.SESSION_ID_HEADER];
          ...
          req.useH2 = isHttps && config.enableH2 && ((req.isH2 && !util.isDisableH2(req)) || (req.sessionId && util.isEnableH2(req)));
          req.setServerPort && req.setServerPort(options._proxyPort || options.port || (useHttps ? 443 : 80));
          req.options = options;

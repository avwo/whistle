...
var SCRIPT_START = new Buffer('<script>');
var SCRIPT_END = new Buffer('</script>');
var STYLE_START = new Buffer('<style>');
var STYLE_END = new Buffer('</style>');
var REQ_HEADER_RE = /^req\.headers\.(.+)$/;
var RES_HEADER_RE = /^res\.headers\.(.+)$/;
var HEADER_RE = /^headers\.(.+)$/;

function getDeleteProperties(deleteRule) {
  deleteRule = util.getMatcherValue(deleteRule);
  if (!deleteRule) {
    return;
  }
  var reqHeaders = {};
  var resHeaders = {};
  deleteRule.split('|').forEach(function(prop) {
...
module.exports = function(req, res, next) {
  ...
  var deleteHeaders = getDeleteProperties(resRules['delete']);
  ...
  req.request = function(options) {
    ...
    var now = Date.now();
    rules.findProxyFromPac(options.href, options.isPlugin ? null : resRules, function() {
      ...
      rules.resolveHost(options.localDNS && net.isIP(options.host) ? options.host :
        (proxyUrl && 'http:' + util.removeProtocol(proxyUrl) || options.href),
            function(err, ip, port, host) {
              ...
              if (proxyUrl) {
                ...
                if (isHttps || isSocks) {
                  if ((proxyOptions.port || (isSocks ? 1080 : 80)) == config.port && util.isLocalAddress(ip)) {
                ...
                if (proxyOptions.auth) {
                  headers['proxy-authorization'] = 'Basic ' + new Buffer(proxyOptions.auth + '').toString('base64');
                }
              }
              ...
              if ((port || (isHttps ? 443 : 80)) == config.port && util.isLocalAddress(options.host)) {
                res.response(util.wrapResponse({
                  statusCode: 302,
                  headers: {
                    location: 'http://' + config.localUIHost + (options.path || '')
              ...
              function request(options, port, isHttp) {
                ...
                if (isHttp) {
                  options.port = proxyOptions ? (proxyOptions.port || 80) : (port || options.port);
                }

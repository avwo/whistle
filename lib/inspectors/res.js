      ...
      rules.resolveHost(curUrl, function(err, ip, port, host) {
        ...
        function request(options, port, isDirectReq) {
          ...
          var retryReq = function(err) {
            if (retry) {
              res.response(util.wrapGatewayError(util.getErrorStack(err || new Error('Timeout'))));
              return;
            }
            var stack = err && err.stack || '';
            if (isHttps && !req.disable.autoAdaptHttp && (resRules.host || util.isLocalAddress(ip))
              && (stack.indexOf('at TLSSocket.onHangUp') !== -1 || err.code === 'EPROTO')) {
              isHttps = false;
              if (!req.disable.keepAlive) {

  ...
  req.request = function(options) {
    readFirstChunk(req, function() {
      ...
      rules.getProxy(options.href, options.isPlugin ? null : req, function(err, hostIp, hostPort) {
        ...
        var curUrl, auth, isInternalProxy, isHttpsInternal, isHttpsProxy, origPath, origProxy;
        if (!hostIp) {
          if (options.localDNS && net.isIP(options.host)) {
            curUrl = options.host;
          } else if (proxyUrl) {
            isHttpsInternal = proxyRule.isHttpsInternal;
            isHttpsProxy = proxyRule.isHttps || isHttpsInternal;
        ...
        rules.resolveHost(req, function(err, ip, port, host) {
          ...
          if (proxyUrl) {
            ...
            if (isHttps || isHttpsInternal || isSocks || isHttpsProxy || req._phost) {
          ...
          function request(options, serverPort, direct) {
            ...
            var retry = function(err) {
              ...
              if (retryCount >= maxRetryCount) {
                var toHttp;
                if (isHttps && req.noReqBody && util.checkTlsError(err) && util.checkAuto2Http(req, ip, proxyUrl)) {
                  isHttps = false;
                  toHttp = true;

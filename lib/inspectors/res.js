...
//nodejs的url只支持ascii，对非ascii的字符要encodeURIComponent，否则传到浏览器是乱码
  ...
  case 'HTML':
    if (cssAppend) {
      bottom = Buffer.concat([STYLE_START, util.toBuffer(cssAppend, charset), STYLE_END]);
    }
    ...
    if (jsAppend) {
      jsAppend = Buffer.concat([SCRIPT_START, util.toBuffer(jsAppend, charset), SCRIPT_END]);

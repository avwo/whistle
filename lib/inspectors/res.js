...
module.exports = function (req, res, next) {
  ...
  var deleteHeaders = util.parseDelProps(req);
        ...
        pluginMgr.getResRules(req, _res, function () {
          ...
          util.parseRuleJson(
            ...
            function (
              ...
              var resType, customHeaders;
              if (data.headers) {
                setCookies(_resHeaders, data);
                resType = data.headers['content-type'];
                if (typeof resType == 'string') {
                  if (resType.indexOf(';') == -1) {
                    var origType = _resHeaders['content-type'];
                    if (
                ...
                extend(_resHeaders, customHeaders);
              }
              ...
              resType = util.getContentType(_resHeaders);
              ...
              util.getRuleValue(injectRules, function (
                ...
                if (
                    (isJs || resType === 'JSON' || !_resHeaders['content-type']) &&
                    params &&

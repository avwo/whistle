...
function addTimeout(req, res, timeout) {
	...
	function emitTimeoutError() {
		util.emitErrorIfExist(responsed ? res : req, new Error('timeout'));
	}
	setTimeout(emitTimeoutError, 1000);
...
module.exports = function(req, res, next) {
	...
	addTimeout(req, res, timeout || TIMEOUT);
	next();
};

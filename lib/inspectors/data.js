...
function emitDataEvents(req, res, proxy) {
  ...
  res.once('src', function(r) {
    _res = r;
    var MAX_RES_BODY = isUnzipJs(_res) ? MAX_RES_BODY_SIZE : MAX_SIZE;
    ...
    resData.rawHeaderNames = req.useH2 ? '' : _res.rawHeaderNames;
    if (!data.endTime) {
      setResStatus();
    ...
    res.end = function(chunk) {
      ...
      unzipResBody = function() {
        unzipResBody = null;
        unzipBody(_res, resBody, function(err, body) {

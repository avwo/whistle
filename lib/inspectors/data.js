...
function emitDataEvents(req, res, proxy) {
	...
	var statusPort = plugin && plugin.ports && plugin.ports.statusPort;
	var showHttpData = !req.filter.hide && EventEmitter.listenerCount(proxy, 'request') || statusPort;
	var clientIp = util.getClientIp(req) || LOCALHOST;
	...
	function receiveResBody() {
		...
		if (!contentType || (contentType && contentType != 'IMG' && util.hasBody(_res))) {

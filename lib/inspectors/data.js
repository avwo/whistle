...
function emitDataEvents(req, res, proxy) {
  ...
  if (!exportsFile && !showHttpData && !req.enable.needNetworkData) {
    return;
  }
  var _res = {};
  ...
  if (!req.filter.hide || req.enable.needNetworkData) {
  ...
  request._transform = function(chunk, encoding, callback) {
    ...
    zlib.unzip(req.headers['content-encoding'], reqBody, function(err, body) {
  ...
  function receiveResBody() {
    ...
    response._transform = function(chunk, encoding, callback) {
      if (chunk) {
        ...
        zlib.unzip(_res.headers['content-encoding'], resBody, function(err, body) {

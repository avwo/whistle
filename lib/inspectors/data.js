...
function emitDataEvents(req, res, proxy) {
  ...
  var reqData = {
    ...
    rawHeaderNames: req.isH2 ? '' : req.rawHeaderNames,
    headers: req.headers,
  ...
  var handleReqBody = function(stream) {
    if (reqDone) {
      return;
    }
    reqDone = true;
  ...
  var handleResBody = function(stream) {
    if (resDone) {
      return;
    }
    resDone = true;
    var MAX_RES_BODY = isUnzipJs(res) ? MAX_RES_BODY_SIZE : MAX_SIZE;
    if (!cleared && util.hasBody(res, req) && checkType(res)) {
      if (res.headers['content-length'] > MAX_RES_BODY) {
        resBody = false;
      } else {
  ...
  res.once('src', function(r) {
    ...
    resData.rawHeaderNames = req.useH2 ? '' : res.rawHeaderNames;
    if (!data.endTime) {
      setResStatus();

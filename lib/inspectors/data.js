...
function emitDataEvents(req, res, proxy) {
  ...
  var handleReqBody = function (stream, info) {
    ...
    reqData.size = 0;
    var captureStream = !disable.captureStream && (resDone || enable.captureStream) && !getZipType(info);
  ...
  var handleResBody = function (stream, info) {
    ...
    var captureStream = !disable.captureStream && (!reqDone || enable.captureStream ||
    ...
    if (!cleared && util.hasBody(info, req) && checkType(info)) {
      if (info.headers['content-length'] > MAX_RES_BODY && !captureStream) {
        resBody = false;
      } else {
        resBody = null;
      }

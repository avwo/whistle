...
function handleReq(req, data) {
  ...
  util.handleReqHeaderReplace(req.headers, req.rules.headerReplace);
}
...
function handleParams(req, params, urlParams) {
  ...
  if ((_params || urlParams) && /^https?:/.test(req.options.href)) {
    urlParams = urlParams && _params ? extend(_params, urlParams) : (_params || urlParams);
    var newUrl = util.replaceUrlQueryString(req.options.href, urlParams);

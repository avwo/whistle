...
function handleReplace(req, replacement) {
	...
	type = /x-www-form-urlencoded/i.test(type) ? 'FORM' : util.getContentType(type);
	if (!type || type == 'IMG') {
		return;
	}
		...
		util.readInjectFiles(data, function(data) {
			util.getRuleValue([reqRules.reqBody, reqRules.reqPrepend, reqRules.reqAppend], 
				function(reqBody, reqPrepend, reqAppend) {
					...
					util.getFileWriter(util.hasRequestBody(req) ? util.getRuleFile(reqRules.reqWrite) : null, function(writer) {
						if (writer) {
							req.add(new FileWriterTransform(writer));
						}
						next();
					});
				});

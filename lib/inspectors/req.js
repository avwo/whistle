...
function handleReq(req, data) {
	...
	if (typeof data.charset == 'string') {
		...
		if (typeof type == 'string') {
			req.headers['content-type'] = type.split(';')[0] + '; charset=' + charset;
		} else {
			req.headers['content-type'] = data.charset;
		}
	} else {
		delete data.charset;
	}
...
module.exports = function(req, res, next) {
	...
	util.parseRuleJson([reqRules.req, reqRules.reqHeaders, reqRules.reqCookies, 
		...
		if (reqRules.reqType) {
			...
			type = (!type || type.indexOf('/') != -1) ? type : (REQ_TYPE[type] || REQ_TYPE.defaultType);

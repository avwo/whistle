...
function handleReq(req, data, reqRules, delType) {
  ...
  if (opList) {
    ...
    var xff = req.headers[config.CLIENT_IP_HEADER];
    ...
    if (xff !== req.headers[config.CLIENT_IP_HEADER]) {
      req._customXFF = req.headers[config.CLIENT_IP_HEADER];
    }
  ...
  util.parseRuleJson(
    ...
    function (
      ...
      if (data.headers) {
        ...
        req._customXFF = data.headers[config.CLIENT_IP_HEADER];

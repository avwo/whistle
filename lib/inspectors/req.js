...
var BufferTransform = util.BufferTransform;
...
function handleParams(req, params) {
	...
	if (util.isUrlEncoded(req)) {
		...
		req.options =url.parse(util.replaceUrlQueryString(req.options.href, params));
	}
}

function handleUrlReplace(req, params) {
	if (!params || !/^https?:/.test(req.options.href)) {
		return;
	}
	
	var path = req.options.href;
	var index = path.indexOf('://');
	if (index == -1) {
		return;
	}
	
	index = path.indexOf('/', index + 3) + 1;
	if (index <= 0) {
		return;
	}
	...
	Object.keys(params).forEach(function(pattern) {
		...
		if (util.isOriginalRegExp(pattern) && (pattern = util.toOriginalRegExp(pattern))) {
			...
			path = path.split(pattern).join(value);
		}
	});
	
	req.options = url.parse(root + path);
}
...
module.exports = function(req, res, next) {
	...
	util.parseRuleJson([reqRules.req, reqRules.reqHeaders, reqRules.reqCookies, 
			...
			function(data, headers, cookies, auth, params, cors, replacement, urlReplace) {
		...
		util.readInjectFiles(data, function(data) {
			util.getRuleValue([reqRules.reqBody, reqRules.reqPrepend, reqRules.reqAppend, reqRules.reqBodyBin, reqRules.reqPrependBin, reqRules.reqAppendBin], 
				function(reqBody, reqPrepend, reqAppend, reqBodyBin, reqPrependBin, reqAppendBin) {
					if (reqBody) {
						data.body = reqBody;
					}
					...
					if (reqBodyBin || reqPrependBin || reqAppendBin) {
						req.add(new BufferTransform({
							top: reqPrependBin,
							body: reqBodyBin,
							bottom: reqAppendBin
						}));
					...
					util.getFileWriters([util.hasRequestBody(req) ? util.getRuleFile(reqRules.reqWrite) : null, util.getRuleFile(reqRules.reqWriteRaw)], function(writer, rawWriter) {
						if (writer) {
							req.add(new FileWriterTransform(writer, req, false, false, true));
						}
						
						if (rawWriter) {
							req.add(new FileWriterTransform(rawWriter, req, true, false, true));
						}
						
						next();
					});
				
				}, !util.hasRequestBody(method));
		});
	});
};
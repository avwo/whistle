...
function handleReq(req, data) {
  ...
  !util.isEmptyObject(data) && req.addZipTransform(new WhistleTransform(data));
}
...
function handleParams(req, params) {
  ...
  if (util.isUrlEncoded(req)) {
    ...
    req.options = req.realUrl && req.realUrl !== req.options.href ? url.parse(newUrl) : util.parseUrl(newUrl);
  }
}

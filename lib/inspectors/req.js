...
function handleParams(req, params) {
	...
	if (!req.method || /^GET$/i.test(req.method)) {
		...
		req.add(transform);
			...
			/;\s*boundary=([^;]+)/i.test(req.headers['content-type'])) {
		var boundary = new Buffer('--' + RegExp.$1 + '\r\n');
		var endBoundary = new Buffer('--' + RegExp.$1 + '--');
		var length = boundary.length;
		var transform = new Transform();
		delete req.headers['content-length'];
		params = _params;
		transform._transform = function(chunk, encoding, callback) {
			if (this._parsed || !chunk) {
				return callback(null, chunk || this._restBuffer);
			}
			
			chunk = this._restBuffer ? this._restBuffer.concat(chunk) : chunk;
			var index = 0;
			if (this._startHeadersIndex) {
				this._restBuffer = null;
				index = util.indexOfBuffer(chunk, D_CRLF);
				//TODO: 解析头部
				return callback(null, chunk);
			}
			
			index = util.indexOfBuffer(chunk, boundary);
			if (index != -1) {
				this._startHeadersIndex = index = index + length;
				this._restBuffer = chunk.slice(index);
				chunk = chunk.slice(0, index);
				return callback(null, chunk);
			}
			
			index = util.indexOfBuffer(chunk, endBoundary);
			if (index == -1) {
...
function parseUploadHeaders(buffer) {
	var headers = buffer + '';
}

function toUploadHeadersBuffer(str) {
...
module.exports = function(req, res, next) {
	...
	util.parseRuleJson([reqRules.req, reqRules.reqHeaders, reqRules.reqCookies, 
	                    reqRules.auth, reqRules.params, reqRules.reqCors, reqRules.reqReplace], 
			...
			util.getRuleValue([reqRules.reqBody, reqRules.reqPrepend, reqRules.reqAppend], 
				function(reqBody, reqPrepend, reqAppend) {
					...
					util.getFileWriters([util.hasRequestBody(req) ? util.getRuleFile(reqRules.reqWrite) : null, util.getRuleFile(reqRules.reqWriteRaw)], function(writer, rawWriter) {
						...
						if (rawWriter) {
							req.add(new FileWriterTransform(rawWriter, req, true, true));
						}

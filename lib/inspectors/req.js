		...
		util.readInjectFiles(data, function(data) {
			util.getRuleValue([reqRules.reqBody, reqRules.reqPrepend, reqRules.reqAppend], 
				function(reqBody, reqPrepend, reqAppend) {
					...
					var reqHeader = util.getMatcherValue(reqRules.reqHeader);
					if (reqHeader) {
						data.headers = data.headers || {};
						reqHeader.split('|').forEach(function(header) {
							var speratorIndex = header.indexOf(':');
							var name = (speratorIndex == -1 ? header : header.substring(0, speratorIndex)).toLowerCase();
							if (speratorIndex == -1) {
								delete data.headers[name];
								delete req.headers[name];
							} else {
								data.headers[name] = header.substring(speratorIndex + 1);
							}
						});
					}

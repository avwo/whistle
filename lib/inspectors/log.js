...
function getScript(host, isHtml, isHttps) {
  var logCgiPath = config.WEBUI_PATH + 'log.' + config.port + '/cgi-bin/log/set';
  var result = logHtmlScript;
  if (!isHtml) {
    result = logScript;
    logCgiPath = (isHttps ? 'https://' : 'http://') + host + logCgiPath;
  }
  ...
  if (log) {
    ...
    res.on('src', function(_res) {
      ...
      if (util.supportHtmlTransform(_res, req)) {
        isHtml = true;
        topScript = getScript(host, isHtml, req.isHttps);
      } else if (util.getContentType(_res.headers) == 'JS') {
        topScript = getScript(host, isHtml, req.isHttps);
      }
      
      if (topScript) {
        topScript = topScript.replace(/\$INTERCEPT_CONSOLE/g, !req.disable.interceptConsole || !!req.enable.interceptConsole);

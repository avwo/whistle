    ...
    res.on('src', function (_res) {
      ...
      if (topScript) {
        ...
        transform._transform = function (chunk, _, callback) {
          if (!added) {
            added = true;
            var logId = util.removeProtocol(log.matcher, true);
            util.getRuleValue(!logId || /^[^/\\{}()<>]+$/.test(logId) ? null : log, function (script) {
              topScript = topScript.replace('$LOG_ID', logId && util.encodeURIComponent(logId));

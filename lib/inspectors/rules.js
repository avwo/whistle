...
var getEncodeTransform = require('../util/transproto').getEncodeTransform;
...
function pipeStream(src, target, isEncoder) {
  ...
  var trans = getEncodeTransform();
  if (isEncoder) {
    src.pipe = function(stream) {
      return target.pipe(stream);
    };
    target.on('error', function(err) {
      src.emit('error', err);
    });
    trans.pipe(src).pipe(target);
    return trans;
  }
  src.pipe(trans).pipe(target);
  return src;
}

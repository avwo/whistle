...
function pipeStream(src, target, isEncoder) {
  if (!src || !target) {
    return src || target;
  }
  var pipe = src.pipe;
...
function hasBody(obj, req) {
  return req ? util.hasBody(obj, req) : util.hasRequestBody(obj);
}

function getDecoder(obj, req) {
  return function(socket, callback) {
    var encoding = obj._originEncoding;
    if (socket || encoding !== obj.headers['content-encoding'] || isEnable(req)) {
      obj._hasZipBody = true;
    }
    var decoder = obj._hasZipBody && encoding && hasBody(obj, req) && util.getUnzipStream(encoding);
    if (decoder) {
...
function getEncoder(obj, req) {
  return function(socket, callback) {
    ...
    callback(pipeStream(socket, encoder, true));
  };
}

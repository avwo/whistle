...
function getEncoder(obj, req) {
  return function(callback) {
    var type;
    if (req ? util.hasBody(obj) : util.hasRequestBody(obj)) {
      var enable = (req || obj).enable || '';
      type = req ? (enable.gzip || enable.resGzip) : enable.reqGzip;
      type = type ? 'gzip' : (obj._hasZipBody && obj.headers);
    }

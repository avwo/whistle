...
var HTTPS_FIELD = config.HTTPS_FIELD;
var HTTPS_FLAG = config.HTTPS_FLAG;
...
module.exports = function(req, res, next) {
	if (req.headers[HTTPS_FIELD] || req.socket.isHttps) {//防止socket长连接导致新请求的头部无法加util.HTTPS_FIELD
		req.socket.isHttps = true;
		req.isHttps = true;
		delete req.headers[HTTPS_FIELD];
	} else {

...
var SNI_UA_RE = /WebKit|Gecko|Trident\/(?:5|6|7)\.0/i;
  ...
  server.on('connect', function(req, reqSocket, head) {//ws, wss, https proxy
    ...
    rules.resolveRulesFile(req, function() {
      ...
      pluginMgr.getTunnelRules(req, function(_rulesMgr) {
        ...
        if (isIntercept()) {
          var useSNI = !disable.sni && (isIPHost || req.enable.sni || filter.sni
            || SNI_UA_RE.test(req.headers['user-agent']));
        ...
        var resData = {headers: {}};
        var inspect = req.enable.inspect;

...
function tunnelProxy(server, proxy) {
  server.on('connect', function(req, reqSocket, head) {//ws, wss, https proxy
    ...
    var headers, inspect, customInspect, reqData, resData, res, rollBackTunnel, buf;
    req.isTunnel = true;
    ...
    rules.resolveRulesFile(req, function() {
      ...
      pluginMgr.getTunnelRules(req, function(_rulesMgr) {
        ...
        function handleTunnel() {
          ...
          inspect = enable.inspect;
          customInspect = enable.customInspect || enable.customInspector;
          headers = req.headers;
          ...
          pluginMgr.loadPlugin(plugin, function() {
            ...
            if(plugin) {
              ...
              pluginMgr.addRuleHeaders(req, _rules, null, customInspect);
            }
        ...
        function handleConnect() {
          ...
          reqSocket.inspectFrames = !customInspect && (useTunnelPolicy || inspect);
          if (customInspect) {
            var enable = req.enable;
            customInspect = ['inspect'];
            if (enable.pauseSend) {
              customInspect.push('pauseSend');
            } else if (enable.ignoreSend) {
              customInspect.push('ignoreSend');
            }
            if (enable.pauseReceive) {
              customInspect.push('pauseReceive');
            } else if (enable.ignoreReceive) {
              customInspect.push('ignoreReceive');
            }
            reqSocket.customInspect = customInspect.join();
          }
          if (buf) {
            resSocket.write(buf);
            buf = null;

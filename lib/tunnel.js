      ...
      pluginMgr.getTunnelRules(req, function(_rulesMgr) {
        ...
        function handleTunnel() {
          ...
          pluginMgr.loadPlugin(plugin, function(err, ports) {
            if (err) {
              return sendEstablished(500, err);
            }
        ...
        function sendEstablished(code, body) {
          if (res) {
            code = res.statusCode;
          } else {
            code = code || 200;
          ...
          pluginMgr.getResRules(req, res, function() {
            ...
            util.parseRuleJson(rollBackTunnel ? null : reqRules.resHeaders, function(newResHeaders) {
              if (rollBackTunnel) {
                ...
                res.rawHeaders.push('proxy-agent', 'Proxy-Agent', 'content-length', 'Content-Length');
                ...
                var rawData = [statusLine, getRawHeaders(formatHeaders(resHeaders, rawHeaderNames)), '\r\n', body].join('\r\n');
                if (code && code != 200) {

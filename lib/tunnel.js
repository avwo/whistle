    ...
    rules.resolveRulesFile(req, function() {
      ...
      pluginMgr.getTunnelRules(req, function(_rulesMgr) {
        ...
        function handleConnect() {
          var reqTransform = util.getEventTransform(proxy, 'tunnelRequest', tunnelUrl);
          if (reqTransform) {
            reqSocket.pipe(reqTransform).pipe(resSocket);
            var resTransform = util.getEventTransform(proxy, 'tunnelRequest', tunnelUrl);
            resSocket.pipe(resTransform).pipe(reqSocket);
          } else {

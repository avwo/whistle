...
function tunnelProxy(server, proxy) {
  server.on('connect', function(req, reqSocket, head) {//ws, wss, https proxy
    ...
    var headers, inspect, reqData, resData, res, paused, buf;
    req.isTunnel = true;
    ...
    rules.resolveRulesFile(req, function() {
      ...
      var isEnableIntercept = function() {
        ...
        if (isIPHost) {
          return options.port == 80 || options.port == 443;
        }
        return true;
      };
      ...
      var plugin = isIntercept() ? false : pluginMgr.resolveWhistlePlugins(req);
      ...
      pluginMgr.getTunnelRules(req, function(_rulesMgr) {
        ...
        if (isIntercept()) {
          ...
          reqSocket.write(ESTABLISHED_CTN);
          return;
        }
        ...
        function sendEstablished(code) {
          ...
          pluginMgr.getResRules(req, res, function() {
            ...
            util.parseRuleJson(paused ? null : reqRules.resHeaders, function(newResHeaders) {
              if (paused) {
                reqSocket.resume();
              } else {

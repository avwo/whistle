...
function tunnelProxy(server, proxy) {
  ...
  server.on('connect', function(req, reqSocket) {//ws, wss, https proxy
    ...
    var resSocket, proxyClient, responsed, reqEmitter, data, rulesMgr, originPort;
    ...
    rules.resolveRulesFile(req, function() {
      ...
      pluginMgr.getTunnelRules(req, function(_rulesMgr) {
        ...
        if (!reqSocket._hasError && !req.fromComposer && (req._forceCapture || isIntercept())) {
          ...
          reqSocket.sniCallbackRule = _rules && _rules.sniCallback;
        ...
        function handleTunnel() {
          ...
          if (!req.isPluginReq && config.captureData && (!filter.hide || disable.hide)) {
            ...
            if (retryConnect) {
              ...
              resSocket.on('error', function() {
                if (!retried) {
                  ...
                  !responsed && !reqSocket._hasError && retryConnect && retryConnect();
                }
              });
            }
          }, hostIp, hostPort);
          ...
          pluginMgr.getResRules(req, res, function() {
            ...
            util.parseRuleJson(rollBackTunnel ? null : reqRules.resHeaders, function(newResHeaders) {
              if (rollBackTunnel) {
                ...
                var rawData = [statusLine, getRawHeaders(formatHeaders(curHeaders, rawHeaderNames))].join('\r\n') + '\r\n\r\n';
                if (code != 200) {

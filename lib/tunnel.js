...
function tunnelProxy(server, proxy, type) {
  ...
  server.on('connect', function (req, reqSocket) {
    ...
    util.onSocketEnd(reqSocket, function (err) {
      ...
      var e = req.enable;
      var d = req.disable;
      if (config.captureData && (e.showCaptureError || d.hideCaptureError ||
        (!e.hideCaptureError && !d.showCaptureError)) && dispatch.getTunnelDataOnce(reqSocket)) {
        resData = {headers: {}};
        data = {
          id: req.reqId,
          url: options.host,
      ...
      pluginMgr.getTunnelRules(req, function (_rulesMgr) {
        ...
        function handleTunnel() {
          ...
          if (
            ...
            config.captureData &&
            ((!filter.hide && !disable.show) || enable.show || disable.hide)
          ) {

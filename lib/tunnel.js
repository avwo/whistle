...
function tunnelProxy(server, proxy) {
  ...
  server.on('connect', function(req, reqSocket) {//ws, wss, https proxy
    ...
    rules.resolveRulesFile(req, function() {
      ...
      var useHttpH2 = function() {
        return config.enableH2 && (policy === 'httpH2' || policy === 'http2');
      };
      ...
      var isCustomIntercept = function() {
        return filter.https || filter.capture || filter.intercept || policy === 'intercept' || policy === 'capture' || useHttpH2();
      };
      ...
      pluginMgr.getTunnelRules(req, function(_rulesMgr) {
        ...
        reqSocket.useHttpH2 = useHttpH2();
        if (!reqSocket._hasError && (req._forceCapture || isIntercept())) {

  ...
  server.on('connect', function(req, reqSocket, head) {//ws, wss, https proxy
    ...
    rules.resolveRulesFile(req, function() {
      ...
      pluginMgr.getTunnelRules(req, function(_rulesMgr) {
        ...
        if (!isICloundCKDB && !useTunnelPolicy && (policy === 'intercept' || isLocalUIUrl || ((filter.https || filter.capture ||
           filter.intercept || rulesUtil.properties.get('interceptHttpsConnects'))

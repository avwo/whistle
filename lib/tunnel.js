...
function emitAborted(data, reqEmitter) {
  data.reqError = true;
  data.res.statusCode = 'aborted';
...
function tunnelProxy(server, proxy, type) {
  ...
  server.on('connect', function (req, reqSocket) {
    ...
    util.onSocketEnd(reqSocket, function (err) {
      // 处理可能的证书校验出错
      if (dispatch.getTunnelDataOnce(reqSocket)) {
        data = {
          id: req.reqId,
          url: options.host,
          startTime: now,
            ...
            util.parseRuleJson(
              ...
              function (newResHeaders) {
                if (rollBackTunnel) {
                  ...
                  var delResHeaders =
                    util.parseDelProps(req).resHeaders;

    ...
    rules.resolveRulesFile(req, function() {
      ...
      pluginMgr.getTunnelRules(req, function(_rulesMgr) {
        ...
        function handleTunnel() {
          ...
          inspect = enable.inspect || util.iscustomParser(req);
          headers = req.headers;
          ...
          pluginMgr.loadPlugin(plugin, function() {
            ...
            if(plugin) {
              var ports = plugin.ports;
              if (ports) { 
                tunnelPort = ports.tunnelPort || ports.port;
              }
              if (!tunnelPort) {
              ...
              if (proxyUrl) {
                ...
                getServerIp(_url, function(ip) {
                  ...
                  util.parseRuleJson(_rules.reqHeaders, function(reqHeaders) {
                    reqHeaders = reqHeaders && util.lowerCaseify(reqHeaders, reqRawHeaderNames);
                    ...
                    if (disable.clientIp || disable.clientIP) {
                      ...
                      } else if (!reqHeaders['x-forwarded-for'] && !config.keepXFF  && util.isLocalAddress(headers[config.CLIENT_IP_HEAD])) {
                        delete headers[config.CLIENT_IP_HEAD];
                      }
                    }

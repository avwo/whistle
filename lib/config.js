...
exports.extend = function(newConf) {
  ...
  if (newConf) {
    var sniCallback = newConf.SNICallback || newConf.sniCallback;
    if (typeof sniCallback === 'function') {
      config.sniCallback = sniCallback;
    }
    ...
    if (typeof shadowRules === 'string') {
      ...
      if (typeof shadowRules[0] === 'string') {
        ...
        config.rules = extend({}, newConf.rules, shadowRules[0]);
        if (shadowRules[1]) {
      ...
      config.rules = extend({}, newConf.rules, shadowRules);
    }
    ...
    if (typeof newConf.mode === 'string') {
      ...
      mode.forEach(function(m) {
        m = m.trim();
        if (/^(pureProxy|debug|captureData|strict|proxyServer|encrypted|noGzip|disableUpdateTips|proxifier2?)$/.test(m)) {
          config[m] = true;
          ...
          config.pureProxy = true;
        } else if (m === 'keepXFF' || m === 'forwardedFor') {
          config.keepXFF = true;
      ...
      if (config.disableForwardedProto) {
        config.HTTPS_PROTO_HEADER = config.HTTPS_FIELD;
      }
    ...
    if (typeof newConf.middlewares == 'string') {
      config.middlewares = newConf.middlewares.trim().split(/\s*,\s*/);
    }
    ...
    var uiPort = !config.disableWebUI && getHostPort(newConf.uiport);
    if (uiPort) {
  ...
  var baseDir = config.baseDir ? path.resolve(getHomePath(config.baseDir), config.dataDirname) : getDataDir(config.dataDirname);

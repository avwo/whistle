...
if (hasDnsOrder) {
  config.verbatim = hasIPv6First ? 2 : 1;
}
...
var DNS_ORDERS_OPTS = ['', 'ipv4first', 'ipv4first', hasIPv6First ? 'ipv6first' : '', '', 'verbatim'];
...
function setDefaultResultOrder(order) {
  ...
  if (DNS_ORDERS_OPTS.indexOf(order) === -1) {
    order = DNS_ORDERS_OPTS[order] || DNS_ORDERS_OPTS[1];
  }
  ...
  try {
    if (!dns.getDefaultResultOrder || dns.getDefaultResultOrder() !== order) {
      dns.setDefaultResultOrder(order);
    }
    config.dnsOrder = getDnsOrder(DNS_ORDERS_OPTS.indexOf(order));
    return true;
  } catch (e) {}
}

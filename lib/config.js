...
exports.extend = function extend(newConf) {
    ...
    if (typeof pluginHost === 'string' && (pluginHost = pluginHost.trim())) {
      ...
      Object.keys(pluginHost).forEach(function(name) {
        ...
        if (typeof host === 'string' && (host = host.trim())) {
          config.pluginHostMap[host.toLowerCase()] = name;
        }
      });
    }
    var uploadUrl = newConf.uploadUrl;
  ...
  var isLocalUIUrl = function(url) {
    ...
    return LOCAL_UI_HOST_LIST.indexOf(host) !== -1 || !!config.pluginHostMap[host];
  };

...
exports.extend = function extend(newConf) {
  if (newConf) {
    ...
    if (newConf.values) {
      config.values = {};
      Object.keys(newConf.values).forEach(function(name) {
        var value = name ? newConf.values[name] : null;
    ...
    if (newConf.rules) {
      if (Array.isArray(newConf.rules) || typeof newConf.rules == 'string') {
        config.rules = {
          Default: {
            rules: getRules(newConf.rules)
          }
        };
      } else {
        config.rules = {};
        Object.keys(newConf.rules).forEach(function(name) {
          var item = newConf.rules[name];
          if (name && item) {
            config.rules[name] = {

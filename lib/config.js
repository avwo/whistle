...
exports.extend = function extend(newConf) {
  if (newConf) {
    ...
    if (newConf.rules) {
      ...
      if (typeof newConf.rules == 'string') {
  ...
  config.getWeinrePort = function(url) {
    var host = getHostname(url);
    if (!WEINRE_RE.test(host)) {
      return false;
    }
    var port = RegExp.$1 || config.weinreport;
    if (RegExp.$2 == 'weinre' && isLocalUIUrl(RegExp.$3)) {
      return port;
    }
    return false;
  };

...
function getHttpsAgent(options) {
  return getAgent(options, httpsAgents);
}

exports.getHttpsAgent = getHttpsAgent;

function getAgent(options, cache) {
  ...
  if (!agent) {
    ...
    agent = cache[key] = new tunnel.httpsOverHttp(options);
    agent.on('free', preventThrowOutError);
...
function getCacheKey(options) {
  ...
  var ip = options.clientIp || '';
  return ['https', options.host, options.port, auth, ip].join(':');
}

  ...
  if (newConf) {
    ...
    if (typeof newConf.mode === 'string') {
      ...
      mode.forEach(function(m) {
        if (/^(pureProxy|debug)$/.test(m)) {
          config[m] = true;
        }
      });
    }
    var visitor = newConf.visitor;
    if (visitor && typeof visitor === 'string') {
      var sepIndex = visitor.indexOf(':');
      var name = visitor.substring(0, sepIndex);
      var pass = visitor.substring(sepIndex + 1);
      if (name && pass) {
        config.visitor = {
          username: name,
          password: pass

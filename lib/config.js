...
exports.extend = function(newConf) {
  ...
  if (newConf) {
    ...
    if (dns.getServers && Array.isArray(dnsServer)) {
      ...
      dnsServer.forEach(function(ip) {
        if (ip === 'cur' || ip === 'current' || ip === 'custom') {
          newServers = newServers.concat(dns.getServers());
        } else if (net.isIP(ip)) {
          newServers.push(ip);
  ...
  if (typeof config.localUIHost === 'string') {
    var localHostList = config.localUIHost.toLowerCase().split(/[|,&]/);

...
function autoClearFreeSockets(agent) {
  if (!cacheAgents.length) {
    ...
    var cletSocket = function(socket) {
      if (now - socket._freeDateForWhistle > MAX_KEEP_ALIVE_AGE) {
        socket.destroy();
      }
    };
    ...
    var clearTimeoutSockets = function() {
      ...
      setTimeout(clearTimeoutSockets, MAX_KEEP_ALIVE_AGE);
    };
    setTimeout(clearTimeoutSockets, MAX_KEEP_ALIVE_AGE);
  }
...
exports.extend = function(newConf) {
  ...
  if (newConf) {
    ...
    if (/^\d+$/.test(newConf.port) && newConf.port > 0) {
      config.uiport = config.port = newConf.port;
    }
    if (/^\d+$/.test(newConf.uiport) && newConf.uiport > 0) {

...
var disableAgent = process.version.substring(1).split('.')[0] > 10;
...
function autoClearFreeSockets(agent) {
  if (!cacheAgents.length) {
    var now = 0;
    var cletSocket = function(socket) {
      if (now - socket._freeDateForWhistle >= KEEP_ALIVE_MSECS) {
        socket.destroy();
      }
    };
    var clearSockets = function(agent) {
      var freeSockets = agent.freeSockets;
      if (freeSockets) {
        Object.keys(freeSockets).forEach(function(name) {
          var list = freeSockets[name];
          Array.isArray(list) && list.forEach(cletSocket);
        });
      }
    };
    var interval = 6000;
    var clearTimeoutSockets = function() {
      now = Date.now();
      cacheAgents.forEach(clearSockets);
      setTimeout(clearTimeoutSockets, interval);
    };
    setTimeout(clearTimeoutSockets, interval);
  }
  cacheAgents.push(agent);
}
...
function createAgent(agentConfig, https) {
  ...
  agent.on('free', function(socket) {
    socket._freeDateForWhistle = Date.now();
    preventThrowOutError(socket);
  });
  autoClearFreeSockets(agent);
  return agent;
}
...
exports.setUIHost = function(host) {
  customUIHost = getHostname(host);
};

exports.setPluginUIHost = function(pluginName, host) {
  if (!pluginName || !WHISLTE_PLUGIN_RE.test(pluginName)) {
    return;
  }
  pluginName = RegExp.$1;
...
function connect(options, cb) {
  ...
  if (options.proxyAuth) {
    var auth = 'Basic ' + toBuffer(options.proxyAuth).toString('base64');
...
exports.extend = function(newConf) {
  ...
  if (newConf) {
    config.inspectMode = !!newConf.inspect || !!newConf.inspectBrk;
  ...
  var isWebUIHost = function(host) {
    if (host === 'local.wproxy.org' || uiHostList.indexOf(host) !== -1 || (customUIHost && host === customUIHost)) {
      return true;
    }
    if (config.pureProxy) {
      return false;

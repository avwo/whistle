...
var variableProperties = ['port', 'uiport', 'encrypted', 'sockets', 'timeout', 'dataDirname', 'storage', 'baseDir',
'username', 'password', 'uipath', 'debugMode', 'localUIHost', 'extra', 'rules', 'values', 'dnsCache'];

config.TUNNEL_KEY = 'x-whistle-tunnel-rules';
...
function getHttpsAgent(options) {
  ...
  if (!agent) {
    ...
    agent = new tunnel[isHttps ? 'httpsOverHttp' : 'httpOverHttp'](options);
...
function getCacheKey(options, isSocks) {
  ...
  return [options.isHttps ? 'https' : 'http', options.proxyHost, options.proxyPort, auth, ip].join(':');
}

...
config.REMOTE_ADDR_HEAD = 'x-whistle-remote-addr';
config.REMOTE_PORT_HEAD = 'x-whistle-remote-port';
  ...
  if (newConf) {
    ...
    if (typeof newConf.mode === 'string') {
      ...
      mode.forEach(function(m) {
        m = m.trim();
        if (/^(pureProxy|debug|master|captureData|headless|strict|proxyServer|encrypted|noGzip|disableUpdateTips|proxifier2?)$/.test(m)) {
          config[m] = true;

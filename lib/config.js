...
exports.extend = function (newConf) {
  ...
  if (newConf) {
    ...
    config.projectPluginPaths = getPaths(
      newConf.projectPluginPaths || newConf.projectPluginPath
    );
    ...
    if (typeof newConf.mode === 'string') {
      ...
      mode.forEach(function (m) {
        m = m.trim();
        if (
          /^(pureProxy|debug|master|captureData|headless|strict|proxyServer|encrypted|noGzip|disableUpdateTips|proxifier2?)$/.test(
            m
          )
        ) {
          config[m] = true;

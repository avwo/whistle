...
function autoClearFreeSockets(agent) {
  var maxAge = config.strict ? 6000 : 60000;
  if (!cacheAgents.length) {
    var now = 0;
    var cletSocket = function(socket) {
      if (now - socket._freeDateForWhistle > maxAge) {
        socket.destroy();
      }
    };
    ...
    var clearTimeoutSockets = function() {
      ...
      setTimeout(clearTimeoutSockets, maxAge);
    };
    setTimeout(clearTimeoutSockets, maxAge);
  }
...
exports.extend = function(newConf) {
  ...
  var agentConfig = {
    ...
    maxFreeSockets: disableAgent ? 0 : 60 // node 11及以上版本缓存连接有问题，先禁掉
  };
  config.disableAgent = disableAgent || config.debug;
  config.httpAgent = config.disableAgent ? false : createAgent(agentConfig);
  config.httpsAgent = config.disableAgent ? false : createAgent(agentConfig, true);

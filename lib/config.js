  ...
  if (newConf) {
    ...
    if (typeof newConf.mode === 'string') {
      ...
      mode.forEach(function (m) {
        m = m.trim();
        if (
          /^(pureProxy|debug|master|client|diagnose|disableAuthUI|captureData|headless|strict|proxyServer|encrypted|noGzip|disableUpdateTips|proxifier2?)$/.test(m)
        ) {
          config[m] = true;
    ...
    if (typeof newConf.installPlugins === 'function' && typeof newConf.uninstallPlugins === 'function') {
      config.installPlugins = newConf.installPlugins;
      config.uninstallPlugins = newConf.uninstallPlugins;
      config.epm = true;
    }

...
function connect(options, cb) {
  ...
  if (options.proxyAuth) {
    proxyOptions.headers['proxy-authorization'] = 'Basic ' +
new Buffer(options.proxyAuth + '').toString('base64');
  }
...
exports.extend = function extend(newConf) {
  ...
  config.localUIHost = getHostname(config.localUIHost) || 'local.whistlejs.com';
  config.WEINRE_HOST = 'weinre.' + config.localUIHost;
  if (LOCAL_UI_HOST_LIST.indexOf(config.localUIHost) == -1) {

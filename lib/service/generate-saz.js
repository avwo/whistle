...
module.exports = function(body) {
  ...
  var getName = function() {
    ...
    return paddingCount <= 0 ? name : new Array(paddingCount + 1).join('0') + name;
  };
  ...
  sessions.map(function(item, index) {
    ...
    var res = !item.res || item.res.statusCode == null ? Buffer.from('') : util.getResRaw(item.res);
    var name = getName();
    item.index = index;
    zip.add('raw/' + name + '_c.txt', req);
    zip.add('raw/' + name + '_m.xml', Buffer.from(getFiddler2Meta(item)));
    zip.add('raw/' + name + '_s.txt', res);
    zip.add('raw/' + name + '_whistle.json', Buffer.from(JSON.stringify({

...
function getResponse(statusCode, headers, isTest) {
  var result = { statusCode: statusCode };
  if (isTest) {
    result.reqId = headers ? headers['x-whistle-req-id'] : undefined;
  } else {
...
function handleConnect(options, cb, count, isTest) {
  ...
  for (var i = 0; i < count; i++) {
    ...
    common.connectInner({
      ...
      execCb && execCb(null, getResponse(svrRes.statusCode, svrRes.headers, isTest));
...
function handleWebSocket(options, cb, count, isTest) {
  count = getReqCount(count);
    ...
    common.connect(PROXY_OPTS, function(err, socket) {
      if (err) {
        ...
        parseReq(socket, function(e) {
          ...
          execCb && execCb(null, getResponse(statusCode, socket.headers || {}, isTest));
        }, true);
      }
    });
  }
...
function handleHttp(options, cb, count, reqId, isTest) {
  count = getReqCount(count);
...
exports.handleRequest = function(req, res) {
  ...
  if (!reqBody.noStore && !isTest && reqBody.needResponse && common.checkHistory(reqBody)) {
  ...
  getBody(function(err) {
    ...
    if (isWs) {
      options.method = 'GET';
      handleWebSocket(options, handleResponse, count, isTest);
    } else if (isConn) {
      handleConnect(options, handleResponse, count, isTest);
    } else  {
      handleHttp(options, handleResponse, count, reqBody.reqId, isTest);
    }
    if (!handleResponse) {

...
var MAX_AGE = 1000 * 60 * 60 * 24 * 10; // 10 days
...
function checkLogin(whistleId, token, cb) {
  if (!whistleId || !token) {
    return;
  }
...
function readWhistleLoginFile(file) {
  try {
    ...
    if (text.length === 3 && Date.now() - (parseInt(text[2], 10) || 0) < MAX_AGE) {
      return text;
    }
  } catch (e) {}
}
...
function checkWhistleId(cb) {
  var text = readWhistleLoginFile(whistleIdFile) || (config.storage && readWhistleLoginFile(path.join(config.baseDir, '.whistle_id')));

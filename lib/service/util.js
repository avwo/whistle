...
function getResRaw(res) {
  ...
  var statusCode = /^\d+$/.test(res.statusCode) ? res.statusCode : 0;
  headers.unshift(['HTTP/1.1', statusCode, res.statusMessage || STATUS_CODES[statusCode] || 'unknown'].join(' '));
  return headers.join('\r\n') + '\r\n\r\n' + (res.body || '');
}
...
function foratRawData(raw) {
  ...
  var body = '';
  if (offset) {
    body = raw.substring(offset[1]).replace(BODY_SEP, '');

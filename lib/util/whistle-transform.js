...
var LT_RE = /^\s*</;
function startWithLT(chunk) {
  return chunk && LT_RE.test(chunk.toString());
}
...
function WhistleTransform(options) {
  ...
  this._strictHtml = options.strictHtml && (this._body || this._top || this._bottom);
}
...
WhistleTransform.prototype._transform = function(chunk, encoding, callback) {
  var self = this;
  var cb = function() {
    if (self._canInject && self._ended && self._bottom) {
  ...
  if (!self._inited) {
    self._canInject = !self._strictHtml || startWithLT(chunk);
    self._inited = true;
    if (self._canInject) {
      if (self._body) {
        self._ended = true;

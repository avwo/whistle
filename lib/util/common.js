...
function bufferToString(buffer) {
  if (Buffer.isBuffer(buffer) && !isUtf8(buffer)) {
    try {
      buffer = iconv.encode(buffer, 'GB18030');
    } catch (e) {}
  }
...
exports.readStream = function(stream, callback) {
  ...
  stream.once('end', function() {
    ...
    var getText = function(cb, ecd) {
      ...
      if (!ecd || typeof ecd !== 'string') {
        ecd = encoding;
      }
      zlib.unzip(ecd, buffer, function(e, ctn) {
        err = e;
        text = bufferToString(ctn);
        cb(err, text);
      });
    };

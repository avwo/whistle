...
function readFileFromMap(path, fileMap) {
  if (!isString(path)) {
    return '';
  }
  path = path.split('|');
  return joinBuffer(path.map(function(file) {
    return fileMap[file || ''];
  }));
}
...
function readFileList(list, callback) {
  readFileMap(list, function(fileMap) {
    ...
    var result = [];
    list.forEach(function(file) {
      result.push(readFileFromMap(file, fileMap));
    });
    callback(result);
  });
}

...
function clearCache() {
  ...
  var len = ids.length;
  if (config.clusterMode ? !len : len <= MAX_LENGTH) {
    return;
  }
  ...
  var isTimeout = function(curData, i) {
    if (i < overflow || (config.clusterMode && curData.endTime
      && now - curData.endTime > CLUSTER_CACHE_TIME)) {
      return true;
    }
    if (i >= preserveLen) {
      return;
    }
    return curData.endTime ? now - curData.endTime > CACHE_TIME : now - curData.startTime > MAX_CACHE_TIME;
  };
  for (var i = 0; i < len; i++) {
    var id = ids[i];

...
function decodeData(data) {
  var result = [];
  data.bin.forEach(function(b) {
    result.push(b.toString(16));
  });
  data.bin = result.join('');
  return data;
}
function handleFrame(data) {
  frames.push(decodeData(data));
}
...
function getFrames(curReqId, lastFrameId) {
  ...
  lastFrameId = lastFrameId || String(Date.now() - 5000);
  var result = [];
  var count = 16;

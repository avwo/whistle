...
function getBase64Data(res) {
  return 'data:' + res.headers['content-type'].split(';')[0] + ';base64,';
}
...
function decodeData(frame) {
  if (frame.base64 == null) {
    if (frame.charset && !UTF8_RE.test(frame.charset)) {
      try {
        frame.bin = SafeBuffer.from(iconv.decode(frame.bin, frame.charset));
      } catch(e) {}
    }

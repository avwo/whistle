...
function getLastFrameId(curReqId) {
  for (var i = frames.length - 1; i >= 0; i--) {
    var frame = frames[i];
    if (frame.reqId === curReqId && !frame.closed && !frame.err) {
...
*/
  ...
  proxy.getData = function(options, clientIp) {
    ...
    var stopRecordFrames = options.lastFrameId == -3;
    ...
    return {
      ...
      endId: endId,
      frames: stopRecordFrames ? [] : getFrames(options.curReqId, options.lastFrameId),
      lastFrameId: stopRecordFrames ? getLastFrameId(options.curReqId) : undefined,

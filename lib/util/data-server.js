...
var CACHE_TIME = 1000 * (config.strict ? 36 : 120);
...
*/
  ...
  function checkHeader(text, keyword, exact) {
    ...
    if (exact) {
      return text === keyword || text === util.encodeURIComponent(keyword).toLowerCase();
    }
  ...
  function checkItem(item, filter) {
    ...
    if (filter.filterKey && item.req.headers[filter.filterKey] != filter.filterValue) {
      return false;
    }
    if (filter.url && item.url.toLowerCase().indexOf(filter.url) === -1) {
      return false;
    }
    if (filter.name && !checkHeader(item.req.headers[filter.name], filter.value, filter.exact)) {
      return false;
    }
    ...
    if (headers) {
      for (var i = 0, len = headers.length; i < len; i++) {
        ...
        if (!checkHeader(item.req.headers[header.name], header.value, filter.exact)) {
          return false;
        }
      }
    }
  ...
  proxy.getData = function(options, clientIp, key, value) {
    options = options || {};
    ...
    var hasNew = (lastNewId && endId !== lastNewId) || (lastNewFrameId && lastNewFrameId !== lastFrameId);

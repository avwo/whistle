...
// function isEnableCors(name) {
//   return name === 'enable' || name === 'use-credentials' || name === 'credentials';
// }
...
exports.setReqCors = function(data, cors) {
  ...
  Object.keys(cors).forEach(function(name) {
    ...
    if (value === '') {
      if (name === '*') {
        return setHeader(data, 'origin', '*');
      }
      if (URL_RE.test(name)) {
        return setHeader(data, 'origin', parseOrigin(name));
      }
    } else if (typeof value !== 'string') {
      return;
    }
    if (name === 'origin') {
      return setHeader(data, 'origin', value);
    }
    if (name === 'method') {
      return setHeader(data, 'access-control-request-method', value);
    }
    if (name === 'headers') {
      setHeader(data, 'access-control-request-headers', value);
    }
  });
};

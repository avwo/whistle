...
exports.indexOfBuffer = function(buf, subBuf, start) {
	start = start || 0;
	if (buf.indexOf) {
	...
	if (subLen) {
		for (var i = start, len = buf.length; i < len; i++) {
			var j = 0;
			for (; j < subLen; j++) {
				if (subBuf[j] !== buf[i + j]) {
					break;
...
function isUrlEncoded(req) {
	...
	return /^post$/i.test(req.method) && /application\/x-www-form-urlencoded/i.test(req.headers && req.headers['content-type']);
}

exports.isUrlEncoded = isUrlEncoded;

function isUploadForm(req) {
	return /^post$/i.test(req.method) && /multipart\/(?:form-data|related)/i.test(req.headers && req.headers['content-type']);
}

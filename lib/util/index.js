...
function mergeRuleProps(origin, add) {
  if (origin) {
    add.list = add.list || [add];
  }
  if (add) {
    origin.list = origin.list || [origin];
  }
  if (origin && add) {
...
function mergeRules(req, add, isResRules) {
  ...
  mergeRuleProps(add && add['delete'], origin['delete']);
  mergeRuleProps(add && add.filter, origin.filter);
  mergeRuleProps(add && add.disable, origin.disable);
  if (isResRules && add) {
  ...
  Object.keys(req.filter).forEach(function(name) {
    if (!isResRules || protoMgr.resProtocols.indexOf(name) != -1) {
      delete origin[name];
    }
  });

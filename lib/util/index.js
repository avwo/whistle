...
exports.getSessionId = function() {
  ...
  return Date.now() +'-' + (sIndex++) + '/' + Math.floor(Math.random() * 10000);
};
...
exports.connect = function(options, callback) {
  ...
  var handleError = function(err) {
    ...
    socket.destroy(err);
    clearTimeout(timer);
    timer = setTimeout(handleTimeout, 16000);
    try {
    ...
    socket.on('close', function(err) {
      socket && execCallback(err || new Error('closed'));
    });
  };
  ...
  timer = setTimeout(handleTimeout, config.CONN_TIMEOUT);
  try {
    socket = sockMgr.connect(options, handleConnect);
  } catch (e) {
    return execCallback(e);

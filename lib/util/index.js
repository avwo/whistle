...
exports.getCgiUrl = function(url) {
...
exports.getHintList = function(conf) {
  ...
  hintList.forEach(function(hint) {
    if (typeof hint === 'string') {
      ...
      var label = hint.label || hint.display;
      if (typeof label !== 'string') {
        label = '';
      }
      if (hint.value.length + help.length + label.length <= MAX_HINT_LEN) {
        result = result || [];
        result.push({
          text: hint.value.trim(),
          displayText: label.trim(),
          help: help.trim()
        });
      }
    }
  });
  var hintPosition = conf.hintPosition;
  return result && {
    hintList: result,
    hintPosition: hintPosition === 'cursor' || hintPosition === 'tail' ? hintPosition : undefined,
    hintOffset: parseInt(conf.hintOffset, 10) || 0
  };
};

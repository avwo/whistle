...
function readFiles(files, callback) {
	...
	files.forEach(function(file, i) {
		var handle = function(err, data) {
			files[i] = err ? null : data;
			--len <= 0 && callback(err, files);
		};
		file && typeof file == 'string' ? fs.readFile(path, {encoding: 'utf8'}, handle) : handle();
	});
}
...
function readInjectFiles(data, callback) {
	data ? util.readFiles([data.prepend, data.replace, data.append], function(ctns) {
		data.prepend = ctns[0];
		data.replace = ctns[2];
		data.append = ctns[3];
		callback(data);
...
function parseRuleToJson(rule, callback) {
	...
	parseFileToJson(getMatcher(rule), callback);
}

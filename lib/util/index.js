...
exports.sendStatusCodeError = function(cltRes, svrRes) {
...
function lowerCaseify(obj) {
	...
	for (var i in obj) {
		var value = obj[i];
		if (value !== undefined) {
			result[i.toLowerCase()] = Array.isArray(value) ? value : value + '';
		}
	}
...
function _getRuleValue(rule, callback) {
	...
	if (rule.root) {
		filePath = path.join(rule.root, filePath);
	}
...
function getRuleFiles(rule) {
	...
	return files.map(function(file) {
		return root ? path.join(root, decodePath(file)) : decodePath(file);
	});
}
...
function getRuleFile(rule) {
	...
	return rule.root ?  path.join(rule.root, decodePath(filePath)) : decodePath(filePath);
}
...
exports.rule = {
		...
		getProxy: function getProxy(rule) {
			...
			return protoMgr.isProxyProtocol(getProtocol(proxyUrl)) ? proxyUrl : null;
		}
};
...
function handlePluginRules(req, rulesMgr, isResRules) {
	if (!rulesMgr) {
		return;
	}
	...
	req.disable && extend(req.disable, rulesMgr.resolveDisable(fullUrl));
	if (filter) {
		extend(filter, rulesMgr.resolveFilter(fullUrl));
	...
	if (!isResRules && !_rules.host) {
		var rules = rulesMgr.resolveRules(fullUrl);
		if (rules.host) {
			_rules.host = rules.host;
		}
	}
	
}

exports.handlePluginRules = handlePluginRules;


var PLUGIN_RULE_RE = /^([a-z\d_\-]+)(?:\((.*)\))?$/;
var PLUGIN_RULE_RE2 = /^([a-z\d_\-]+)(?:\:\/\/(.*))?$/;

function resolvePluginRule(plugin) {
	var value = getMatcherValue(plugin);
...
exports.resolvePluginRule = resolvePluginRule;

...
var now = Date.now();
var DATA_DIR;
var ASSESTS_PATH = exports.ASSESTS_PATH = path.join(__dirname, '../../assets');
var LOCAL_DATA_PATH = exports.LOCAL_DATA_PATH = process.env.WHISTLE_PATH || path.join(__dirname, '../../../', config.dataDirname);
var OLD_LOCAL_DATA_PATH = exports.OLD_LOCAL_DATA_PATH = path.join(LOCAL_DATA_PATH, '../.whistle data');
...
function homedir() {
	...
	return process.env[process.platform == 'win32' ? 'USERPROFILE' : 'HOME'] || path.join(__dirname, '../../../');
}

exports.homedir = homedir;

function getWhistlePath() {
	var whistlePath = process.env.WHISTLE_PATH;
	return whistlePath ? whistlePath : path.join(homedir(), '.WhistleAppData');
}
...
function getDataDir(dirname) {
	...
	DATA_DIR = path.join(getWhistlePath(), dirname || (config.dataDirname || '.' + config.name));
...
function parseLinesToJSON(text) {
...
function readInjectFiles(data, callback) {
	...
	readFiles([data.prepend, data.replace, data.append], function(ctns) {
		if (ctns[0] != null) {
			data.top = ctns[0];
		} 
		if (ctns[1] != null) {
...
function parseFilesToJson(files, callback) {
	Array.isArray(files) && files.length ?
			readFiles(files, function(ctns) {
				callback.apply(null, ctns.map(parseJSON));
			}) : callback();
}

exports.parseFilesToJson = parseFilesToJson;

function parseRuleToJson(rules, callback) {
	...
	rules.forEach(function(rule, i) {
		getRuleValue(rule, function(value) {
			values[i] = value;
			if (--length <= 0) {
				callback.apply(null, values.map(parseJSON));
			}
		});
	});
}

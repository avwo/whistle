...
function parseInlineJSON(text) {
	...
	var result = {};
	text.split(/\||&/g).forEach(function(keyValue) {
		if (keyValue) {
			var speratorIndex = keyValue.indexOf(':');
			if (speratorIndex == -1) {
				speratorIndex = keyValue.indexOf('=');
			}
			if (speratorIndex == -1) {
				result[keyValue] = '';
			} else {
				result[keyValue.substring(0, speratorIndex)] = keyValue.substring(speratorIndex + 1);
			}
		}
	});
	return result;
...
function parseJSON(data) {
	...
	return parseInlineJSON(data) || parseLinesJSON(data);
}
...
function parseRuleJson(rules, callback) {
	...
	Q.all(rules.map(function(rule) {
		var defer = Q.defer();
		var json = parseInlineJSON(rule && removeProtocol(getMatcher(rule), true));
		if (json) {
			defer.resolve(json);
		} else {

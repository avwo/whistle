...
function getEventTransform(proxy, eventName, url) {
  if (!listenerCount(proxy, eventName)) {
    return;
  }
  var transform = new PassThrough();
  transform.on('error', noop);
...
exports.getEventTransform = getEventTransform;
...
function updateClientIp() {
  ...
  for (var i = 0, len = keys.length; i < len; i++) {
    var ip = getIpv4(interfaces[keys[i]]);
    if (ip) {
      clientIp = ip;
      return;
    }
...
function getClientIp(req) {
  ...
  try {
    ip = getForwardedIp(headers) ||
    req.connection.remoteAddress ||
    req.socket.remoteAddress;
  } catch(e) {}
  ip = removeIPV6Prefix(ip);
  return (ip.length < 7 || ip === LOCALHOST) ? clientIp : ip;
}
...
function ignoreRules(rules, ignore, isResRules) {
  ...
  Object.keys(ignore).forEach(function(name) {
    ...
    if (!isResRules || protoMgr.resProtocols.indexOf(name) != -1) {
      ...
      var realName = aliasProtocols[name];
      if (realName) {
        name = realName;
      } else {
        var rule = rules[name];
        ...
        if (!rule || isProxy) {
          ...
          if (!rule || rule.url.indexOf(name + ':') !== 0) {
            return;
          }
...
function transformReq(req, res, port, weinre) {
  ...
  if(req.clientIp || !req.headers[config.CLIENT_IP_HEAD]) {
    req.headers[config.CLIENT_IP_HEAD] = req.clientIp || getClientIp(req) || LOCALHOST;
  }
...
//TODO: 暂时支持reqHeaders://(value)，该头部设置主要给插件whistle.nohost
//TODO: whistle v2.0将全面支持对tunnel、websocket的操作
function parseReqHeaders(reqHeaders) {
  reqHeaders = reqHeaders && reqHeaders.value;
  try {
    reqHeaders = removeProtocol(reqHeaders, true);
    reqHeaders = reqHeaders && lowerCaseify(parseJSON(reqHeaders));
    if (reqHeaders && !Object.keys(reqHeaders)) {
      return;
    }
    return reqHeaders;
  } catch(e) {}
...
exports.parseReqHeaders = parseReqHeaders;
...
exports.hasHeaderRules = hasHeaderRules;

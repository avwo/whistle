...
function transformReq(req, res, port, host) {
  ...
  if(req.clientIp || !net.isIP(req.headers[config.CLIENT_IP_HEAD])) {
    var clientIp = req.clientIp || getClientIp(req);
    if (isLocalAddress(clientIp)) {
      delete req.headers[config.CLIENT_IP_HEAD];
    } else {
      req.headers[config.CLIENT_IP_HEAD] = clientIp;
    }
  }
...
var REQ_HEADER_RE = /^req(?:H|\.h)eaders\.(.+)$/;
var RES_HEADER_RE = /^res(?:H|\.h)eaders\.(.+)$/;
...
exports.setClientCert = function(options, key, cert, isPfx) {
  if (isPfx) {
    if (cert) {
      options.pfx = cert;
      if (key) {

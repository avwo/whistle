...
var MAX_BODY_LENGTH = MAX_RULES_SIZE * 8;
  ...
  var client = (options.protocol === 'https:' ? https : http).get(options, function(res) {
    ...
    res.on('data', function(data) {
      ...
      if (body.length > maxLength) {
        client.abort();
        callbackHandler(isRulesText ? null : new Error('The size of response body exceeds 2m'));
      } else {
        clearTimeout(timer);
...
function ignoreProxy(rules, name, exclude) {
  ...
  if (name !== 'http-proxy' && name !== 'socks') {
    return;
  }
  var pName = getProtocolName(rules.proxy.url);
  if ((name === pName || (name === 'http-proxy' && pName === 'proxy'))
    && !exclude[pName]) {
    delete rules.proxy;
  }
  return true;
...
function checkTlsError(err) {
  ...
  return stack.indexOf('at TLSSocket.onHangUp') !== -1;
}

exports.checkTlsHandUp = function(req, ip, err) {
  return req.isHttps && !req.disable.auto2http && (req.enable.auto2http
    || req.rules.host || isLocalAddress(ip)) && checkTlsError(err);
};

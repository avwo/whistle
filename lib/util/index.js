...
function mergeRules(req, add, isResRules) {
  ...
  extend(req.filter, req.enable);
  var ignore = extend(extend({}, req.filter), req.ignore);
  Object.keys(ignore).forEach(function(name) {
    if (!isResRules || protoMgr.resProtocols.indexOf(name) != -1) {
      if (ignorePlugins(origin, name)) {
        return;
      }
      var rule = origin[name];
      var isProxy = name === 'socks' || name === 'proxy';
      if (!rule || isProxy) {
        var protocol = name + ':';
        rule = isProxy ? origin.proxy : origin.rule;
        if ((!isProxy && !protoMgr.isFileProxy(protocol)) || !rule
            || rule.matcher.indexOf(protocol) !== 0) {
          return;
        }
...
var upperCaseRE = /^([a-z])|-([a-z])/g;
var firstCharToUpperCase = function(all, first, dash) {
  if (first) {
...
function formatHeaders(headers) {
  ...
  Object.keys(headers).forEach(function(name) {
    newHeaders[name.replace(upperCaseRE, firstCharToUpperCase)] = headers[name];
  });
  return newHeaders;
}

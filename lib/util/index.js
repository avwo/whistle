...
var G_NON_LATIN1_RE = /\s|[^\x00-\xFF]/ug;
...
function getFullUrl(req) {
  ...
  return _getProtocol(req.isHttps) + fullUrl;
}
...
function getRemoteAddr(req) {
  try {
    ...
    if (!socket._remoteAddr) {
      var ip;
      if (req.isPluginReq && !req._isProxyReq && req.headers) {
        ip = req.headers[config.REMOTE_ADDR_HEAD];
        delete req.headers[config.REMOTE_ADDR_HEAD];
      }
      socket._remoteAddr = ip || removeIPV6Prefix(socket.remoteAddress) || LOCALHOST;
    }
...
function getRemotePort(req) {
  try {
    ...
    if (socket._remotePort == null) {
      var port;
      if (req.isPluginReq && !req._isProxyReq && req.headers) {
        port = req.headers[config.REMOTE_PORT_HEAD];
...
exports.getStaticDir = function(conf) {
  ...
  if (!staticDir || typeof staticDir !== 'string' ||
    !/^[\w./-]+$/.test(staticDir) || staticDir.length > 100) {
    return;
  }

...
function ignorePlugins(rules, name, exclude) {
  ...
  for (var i = list.length - 1; i >= 0; i--) {
    if (list[i].matcher.indexOf(protocol) === 0 && !exclude[name]) {
      list.splice(i, 1);
    }
  }
  if (!list.length) {
...
function ignoreRules(rules, ignore, isResRules) {
  ...
  keys.forEach(function(name) {
    ...
    ignoreAll = name === 'allRules' || name === 'allProtocols' || name === 'All';
    if (ignoreAll || name === 'filter' || name === 'ignore') {
      return;
    }
    ignore[aliasProtocols[name] || name] = 1;
  });
  keys = Object.keys(ignoreAll ? rules : ignore);
  if (ignoreAll && !exclude) {
    keys.forEach(function(name) {
  ...
  keys.forEach(function(name) {
    if (name === 'filter' || name === 'ignore' || (exclude && exclude[name])) {
      return;
    }
    if (!isResRules || protoMgr.resProtocols.indexOf(name) != -1) {

...
function resolveInlineValues(str, inlineValues) {
...
function parsePathReplace(urlPath, params, delPaths) {
  if ((!params && !delPaths) || !/^(?:ws|http)s?:/.test(urlPath)) {
    return;
  }
...
function wrapResponse(res) {
  ...
  passThrough.headers['x-server'] = config.appName;
  res.body != null &&
...
exports.getRemoteRulesResolver = function(inlineValues) {
  ...
  return function(text) {
    return text.replace(REMOTE_RULES_RE, function (_, apo, rulesUrl) {
      ...
      ++index;
      return resolveInlineValues(getRemoteRules(apo, rulesUrl), inlineValues);
    });
  };
};

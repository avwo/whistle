...
function getFullUrl(req) {
  ...
  if (!isString(host)) {
    ...
    if (headers.host !== host) {
      if (isString(headers.host)) {
        req._forwardedHost = headers.host;
      }
      headers.host = host;
    }
  }
  if (hasProtocol(req.url)) {
    ...
    if (options.host) {
      if (hostRule === false) {
        req._forwardedHost = req._forwardedHost || options.host;
...
exports.initClientId = function(req) {
  req.clientId = req.headers[config.CLIENT_ID_HEADER] || clientId;
};

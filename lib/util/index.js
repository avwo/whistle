...
function parseRuleJson(rules, callback) {
  ...
  Q.all(rules.map(function(rule) {
    ...
    if (json) {
      ...
      readRuleValue(rule, function(data) {
...
function readRuleValue(rule, callback) {
  if (!rule) {
    return callback();
  }
  if (rule.value) {
  ...
  var fn = protoMgr.isBinProtocol(rule.name) ? 'readFile' : 'readFileText';
  fm[fn](filePath, callback);
}

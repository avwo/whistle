...
exports.checkSkip = function(skip, rule) {
  if (skip['*'] && !notSkip('*')) {
    return true;
  }
  if (skip[rule.name] && !notSkip(rule.name)) {
    return true;
  }
  var protocol = getProtocolName(rule.url || rule.matcher);
  return skip[protocol] && !notSkip(protocol);
};

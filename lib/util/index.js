...
var addressList = [];
var hashData = ['x-whistle-client-id', Date.now(), Math.random(), Math.random()];
...
Object.keys(interfaces).forEach(function(ifname) {
  interfaces[ifname].forEach(function (iface) {
    hashData.push(iface.address);
  });
});

var clientId = crypto.createHmac('sha256', '' + Date.now()).update(hashData.join()).digest('hex');
exports.setClientId = function(headers) {
  headers['x-whistle-client-id'] = clientId;
};
exports.isEnableClientId = function(req) {
  var enable = req.enable;
  return enable && (enable.clientId || enable.clientID || enable.clientid);
};

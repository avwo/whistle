...
function getFileWriter(file, callback, force, index) {
  if (!file || (!force && pendingFiles[file])) {
    return callback();
  }
  ...
  var getWriter = function() {
    fse.ensureFile(file, function(err) {
      execCb(err ? null : fs.createWriteStream(file).on('error', logger.error));
      logger.error(err);
    });
  };
  if (force) {
...
function getFileWriters(files, callback, force, index) {

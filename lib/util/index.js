...
function readInjectFiles(data, callback) {
  ...
  fm.readFilesText([data.prepend, data.replace, data.append], function(result) {
    if (result[0]) {
      data.top = result[0];
    }
    if (result[1]) {
...
function readRuleList(rule, callback, isJson) {
  ...
  var execCallback = function() {
    ...
    if (isJson) {
      ...
      callback(fm.joinData(result, !isBin));
    }
  };
...
exports.decodeBuffer = fm.decode;
...
function resolveProperties(list) {
  var result = {};
  if (list) {
    list.map(getMatcherValue).filter(noop).forEach(function(action) {
      result[action] = true;
    });
  }
  return result;
...
exports.setResponseFor = function(rules, headers, serverIp) {
  var responseFor = getMatcherValue(rules.responseFor);
  if (!responseFor) {
    return;
  }
  if (RESPONSE_FOR_NAME.test(responseFor)) {
    responseFor = headers[RegExp.$1.toLowerCase()] || '';
    responseFor += (responseFor && ', ') + (serverIp || '127.0.0.1');
  }

...
function parseInlineJSON(text) {
	if (!text || typeof text != 'string' || /\s/.test(text) || /\\|\//.test(text)) {
		return;
	}
	
	var result = {};
	text.split('&').forEach(function(keyValue) {
		if (keyValue) {
			var speratorIndex = keyValue.indexOf('=');
			if (speratorIndex == -1) {
				result[_decodeURIComponent(keyValue)] = '';
			} else {
				result[_decodeURIComponent(keyValue.substring(0, speratorIndex))] 
							= _decodeURIComponent(keyValue.substring(speratorIndex + 1));
			}
		}
	});
	return result;
...
function parseJSON(data) {
	...
	return _parseJSON(data) || parseLinesJSON(data);
}
...
function replaceUrlQueryString(url, queryString) {
	...
	return url.replace(/\?.*$/, '') + '?' 
					+  replaceQueryString(getQueryString(url), queryString) + hashString;
}

...
var OPERATOR_RE = /^\s*#\s*operators?/i;
function isRulesContent(ctn) {
  return COMMENT_RE.test(ctn) || !SCRIPT_RE.test(ctn);
}
exports.isRulesContent = isRulesContent;
exports.isOperatorContent = function(ctn) {
  return OPERATOR_RE.test(ctn);
};
exports.opToRules = function(pattern, ctn) {
  ctn = ctn.split(CRLF_RE).map(function(op) {
    return pattern + ' ' + op;
  });
  return ctn.join('\n');
};

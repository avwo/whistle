...
if (startTime !== false && startUsage !== false) {
  ...
  setInterval(function() {
    ...
    var newTunnelReqs = procData.totalTunnelRequests - preData.totalTunnelRequests;
    var newWsReqs = procData.totalWsRequests - preData.totalWsRequests;
    var newHttpUIReqs = procData.totalHttpUIRequests - preData.totalHttpUIRequests;
    var newWsUIReqs = procData.totalWsUIRequests - preData.totalWsUIRequests;
    preData.now = curTime;
    ...
    preData.totalHttpUIRequests = procData.totalHttpUIRequests;
    preData.totalWsUIRequests = procData.totalWsUIRequests;
    ...
    procData.httpUIQps = Math.floor(newHttpUIReqs * 100000 / costTime);
    procData.wsUIQps = Math.floor(newWsUIReqs * 100000 / costTime);
    var totalQps = procData.httpQps + procData.tunnelQps + procData.wsQps;
    var totalUIQps = procData.httpUIQps + procData.wsUIQps;
    procData.totalQps = totalQps;
    procData.totalUIQps = totalUIQps;
    if (procData.maxQps < totalQps) {
      procData.maxQps = totalQps;
    }
    if (procData.maxUIQps < totalUIQps) {
      procData.maxUIQps = totalUIQps;
    }
  }, 1000);
}

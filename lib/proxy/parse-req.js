...
module.exports = function parseReq(socket, callback, buffer, neadModifyHeaders) {
	...
	function parseData(data) {
		...
		if (neadModifyHeaders) {
			req.addHeaders = function(headers) {
				var rawData = [firstLine];
				Object.keys(headers || {}).forEach(function(key) {
					var value = headers[key];
					value && rawData.push(key + ':' +  value);
				});
				req.write(rawData.join('\r\n'));
				req.write(buffer.slice(endIndex));
			};
		} else {

...
function parseRes(str) {
	...
	var firstLine = headers.shift().trim().split(/\s+/g);
	var _headers = {};
...
function getRawResByValue(body) {
	...
	if (HEADERS_SEP_RE.test(body)) {
		...
		headers = body.substring(0, i);
		body = body.substring(i + crlfStr.length);
	}
	reader = parseRes(headers);
	reader.body = body;
	return reader;
}
	...
	if (isRawFileProtocol(protocol)) {
		...
		var response = function(err, crlf) {
			...
			if (err) {
				...
				if (crlf) {
					crlf = new Buffer(crlf);
					var index = util.indexOfList(buffer, crlf);
					if (index != -1) {
						extend(reader, parseRes(buffer.slice(0, index) + ''));
